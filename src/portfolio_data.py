"""Pillar-4 US portfolio data — generated by extract_data.py from index.html.
Edit values here, then run gen_dashboard.py to regenerate the dashboard.
Do NOT change ticker/name/ex/g or POS (positions) during fundamentals refreshes.
"""

ASOF = ('Aug 7, 2026 · close — weekly refresh & integrity audit Aug 10: all 152 names now at the verified Aug 7 close. The 14 '
 "stragglers left stale by Friday's daily job were advanced: AFRM 75.25 (its disputed Aug 4 base 79.56 abandoned; "
 'stockanalysis history Aug 4=78.12, Aug 7=75.25), TE 5.85 (+6.9% vs stored — the previously disputed Aug 6 close now '
 'reads 5.55 on stockanalysis, agreeing with Google; MarketBeat 2nd-sourced 5.88@3:59PM print), ZETA 26.64, YUM 150.76 '
 '(quote page at-close; its history table is STILL frozen at Jul 31 — stuck feed, flagged), AXON 571.01 (+9.3%, '
 'marketscreener exact match; the bogus Aug 6 565.00 history row is gone, Aug 6 now reads 522.46 as MarketBeat '
 'confirmed), VEEV 230.47 (+5.8%, at-close quote with prev-close chain intact), RBRK 90.07 (+6.5%, macrotrends exact), '
 'LWLG 8.11 (+6.9%, marketscreener exact), XE 22.65 (+8.8%, marketscreener exact), UUUU 14.14 (+9.6%, MarketBeat '
 'exact), EOSE 4.15 (+5.3%, the formerly Google-only print now confirmed by stockanalysis history), HNGE 89.33 (+11.6% '
 '— Aug 7 rebound after Aug 5 earnings; MarketBeat 89.04@3:59PM confirms magnitude), TMDX 83.89, LMND 52.94. All seven '
 '≥6% moves were confirmed against an independent dated second source and allowed through the gate (TE, AXON, RBRK, '
 'LWLG, XE, UUUU, HNGE). Spot-checks NVDA 223.96 / MSFT 499.99 / GOOGL 354.30 match stored values. Market caps scaled '
 'with price; hi52 extended where breached. AUDIT FIXES: (1) MONTHLY 2026-08 was missing the four fills already '
 'recorded in TRADES on Aug 5-7 (TSM 1,873.23, NVDA 1,975.05, CRM 1,853.28, ISRG 1,887.98) — rolled up, buyUSD '
 '10,045.56 -> 17,635.10, lots 6 -> 10, TOTALS.investedEverUSD 588,820.83 -> 596,410.37 (THB at stored fx 33.75). (2) '
 'Group attribution made consistent: REALIZED g set for TTD/APP (SaaS), SOFI/HOOD (Digital Fintech); SOLD g set for '
 'HOOD/SOFI (Digital Fintech), MELI (Digital Applications), APP (SaaS); MONTHLY[].byGroup re-attributed from byStock '
 'to the CURRENT group map (it still reflected pre-move groups for TSLA->Energy, MPWR->Power, IBM->Quantum, the '
 'Renewal Energy and Nuclear moves, and used -1 for re-added tickers). Deleted tickers keep their sold-time groups '
 '(SNPS/TTD -> SaaS). POS and TRADES untouched. AUDIT NOTES (not fixed): TRADES amounts are inconsistent about '
 'commission — the nine fills Jul 28-Aug 4 include the 8.56 fee in amount (META 8.54) while Aug 5-7 fills exclude it '
 'per the stated convention; Salee to decide which convention TRADES should carry. Implied share counts vs the Aug 3 '
 'weekly baseline: no drift >2% — clean. invested-ever minus sold-cost (554,795 pre-fix) vs summed POS cost 568,038 '
 'differs by ~13.2k; ~7.6k was the missing Aug roll-up (now fixed), the rest is commissions embedded in POS costs plus '
 'the mixed TRADES convention — expected, not an error. | Aug 10 addition: NXPI (NXP Semiconductors N.V., NASDAQ) '
 "added to Computing Chips at the Aug 7, 2026 close 239.71 (+3.42%), stamp 'At close: Aug 7, 2026, 4:00 PM EDT', "
 'second-sourced by MarketBeat. Watchlist only - no position, no budget. NXP makes the chips that run CARS (engine '
 'control, driver-assist radar, digital car keys), factory machines and connected devices, plus the secure chips '
 'behind contactless payment. THE KEY POINT: it is coming OUT of the automotive-chip downcycle. FY2025 was the trough '
 '(revenue -2.73%, EPS -17.36%), but TTM revenue is now +8.77% and Q2 2026 (reported Jul 28) grew +19% YoY with '
 'Industrial & IoT +38% and Automotive +12%; Q3 guidance is +15-21% YoY. Profitable and shareholder-friendly: gross '
 'margin 56.04%, net margin 22.56%, EPS 11.72 (+39.69%), ROE 27.92%, ROIC 17.99%, free cash flow 2.81B, dividend yield '
 '1.69% plus buybacks. Valuation is the cheapest in the Computing Chips group: trailing P/E 20.46, forward P/E 14.33, '
 'PEG 0.74, EV/EBITDA 13.66 - and it trades ~29.5% BELOW its 52-week high of 339.95 despite being up 16.41% over 52 '
 'weeks. 30 analysts, consensus Buy, avg target 311.10 (190-400) = ~30% implied upside. CAUTION: the Buy consensus is '
 'thinning at the margin - UBS DOWNGRADED to Hold on Aug 3 (target 305 -> 270) and Bernstein stayed Hold the same day '
 '(270 -> 290). Debt/equity 0.93 with 10.98B total debt vs 3.22B cash is the highest leverage among the chip names '
 "here, and beta 1.82 makes it volatile. Next earnings Oct 27, 2026 - COMPANY-CONFIRMED on NXP's IR events page "
 "(MarketBeat's Oct 26 is only an estimate). NOT FOUND: jan2, all-time high (52-wk high 339.95 stored as ath proxy), "
 'the dates of the 52-week high/low, and any explicit China revenue-exposure percentage. CONFLICTS on record: forward '
 'P/E 14.33 (stockanalysis) vs 15.92 (GuruFocus) vs 17.46 (MarketBeat) - treat forward P/E as a 14-17.5 range; '
 'trailing EPS 11.72 (stockanalysis TTM to Jun 28) vs 9.00 (GuruFocus, lagged basis), which drives a trailing P/E gap '
 'of 20.46 vs 26.39; analyst panel 30 / avg 311.10 (stockanalysis) vs 26 / 305.43 (MarketBeat). stockanalysis taken '
 'throughout. | Aug 10: FOUR names added to Photonic & Optic Fiber at the Aug 7, 2026 close (all watchlist only, all '
 "stamped 'At close: Aug 7, 2026, 4:00 PM EDT'; jan2 NOT FOUND on all four). GROUP-FIT GRADING WAS DONE ON DISCLOSED "
 "SEGMENT REVENUE, not on the company's marketing. (1) CIEN (Ciena, NYSE) at 412.39 - STRONG FIT, a genuine optical "
 'pure play: Optical Networking alone was 1,099.8M of 1,570.7M revenue = 70.0% in FQ2 2026, with coherent optics '
 '(WaveLogic 6 Extreme, 1.6 Tb/s per wavelength) the core technology. Revenue +30.6% TTM, EPS 3.01 (+311.9%), ROIC '
 '17.90%, FCF 833M. Trailing P/E 137 is distorted by weak pre-inflection quarters - forward P/E 52.87 and PEG 0.81 '
 'tell the real story. Backlog 7.7B; cloud customers ~1/3 of revenue; TWO customers = 34% of revenue (concentration '
 'risk). Sits 35% below its Jun 3 2026 high of 637.51 (stored as ath). WARNING: 637.51 is the 52-WEEK high, NOT the '
 'all-time high - a second-tier source puts the dot-com-era closing peak near 1,046 (Oct 2000) but the split basis '
 'could not be verified, so the recent high was stored and the true drawdown is far larger. Next earnings Sep 3, 2026 '
 '- COMPANY-CONFIRMED. (2) MTSI (MACOM, NASDAQ) at 310.82 - HYBRID FIT: Data Center 40.2% + Telecom 20.8% = 61.0% of '
 'FQ3 2026 revenue sits in optical-facing end markets (EML/Mach-Zehnder laser drivers, TIAs, PAM4 drivers for '
 '800G/1.6T/3.2T transceivers), but 39.0% is Industrial & Defense RF/microwave/GaN - so treat it as ~60% optical, not '
 'a pure play, and note even Data Center includes non-photonic parts. FQ3 (Aug 6) was a blowout: revenue 342.2M '
 '+35.8%, adj EPS 1.40 vs 1.35, and Q4 guidance 415-425M vs 364M consensus - the stock rose 14.49% that day. revG and '
 'r40 left BLANK: a clean TTM growth figure was NOT FOUND (Q3 was +35.8% YoY; nine-month +27.9%). CAUTION: trailing '
 'net margin 20.70% EXCEEDS operating margin 18.19% because interest income on a 663M cash pile and a ~3% non-GAAP tax '
 'rate flatter the bottom line - do not read trailing P/E, ROE or net margin as operating quality. Next earnings NOT '
 'ANNOUNCED. (3) CSCO (Cisco, NASDAQ) at 121.43 - WEAK FIT as photonics: Cisco does NOT break out optical revenue at '
 'all; FQ3 2026 was Networking 56%, Services 23%, Security 13%, Collaboration 6%, Observability 2%, with optics only a '
 'component inside its AI data-centre systems alongside Silicon One switching silicon. It belongs in a broader '
 'networking bucket - though note ANET, APH and GLW already sit in this group, so the group label is the real problem, '
 'not CSCO. Quality is high: revenue 60.75B +9.2%, EPS 3.00 +22.5%, ROIC 18.98%, FCF 11.79B, dividend 1.38%; AI '
 'infrastructure ORDERS raised to ~9B for FY26 (associated revenue ~4B). ath 130.37 is the Jun 4 2026 intraday high '
 'and is plausibly a true all-time high (a second-tier source shows the highest-ever close at 130.00 across '
 '1990-2026). WARNING: CSCO reports FQ4 FY2026 on AUG 12, 2026 - COMPANY-CONFIRMED, two days after this snapshot - so '
 'every multiple stored here is pre-print. (4) VIAV (VIAVI Solutions, NASDAQ) at 39.14 - WEAKEST FIT: only the OSP '
 'segment (335.4M of 1,518.3M FY2026 revenue = 22.1%) is optical, and that is optical COATINGS - anti-counterfeiting '
 'banknote pigments and 3D-sensing filters - NOT fibre or photonic components. The other 77.9% (NSE) is network test '
 'and monitoring instrumentation, whose disclosed FY26 growth drivers were the data-centre ecosystem and '
 'aerospace/defence. It is GAAP LOSS-MAKING on a TTM basis (EPS -0.13, net margin -2.00%, ROE -2.73%, ROIC -16.13%) '
 'while non-GAAP FY26 EPS was 1.00 - so trailing P/E and PEG are blank and EV/EBITDA 91.17 / P-FCF 210.77 are '
 'near-meaningless; judge it on forward P/E 23.82. Revenue did grow 40.0% TTM and FQ4 revenue 443.1M was +52.5%. It '
 'fell 3.57% on Aug 7 despite the beat and sits 35% below its 52-wk high. NOTE the 52-week LOW conflicts three ways '
 '(10.02 stockanalysis / 10.30 GuruFocus / 16.98 marketscreener) and the analyst panel conflicts hard (8 analysts avg '
 '61.43 Strong Buy on stockanalysis vs 9 analysts avg 42.88 Moderate Buy on MarketBeat, whose 11.00 low target is '
 'stale); stockanalysis taken throughout. Next earnings NOT COMPANY-CONFIRMED (late Oct/early Nov implied). NOT ADDED '
 '- SIVE: there is NO US-listed security with ticker SIVE. SIVE is Sivers Semiconductors AB on NASDAQ STOCKHOLM; its '
 'only US quotation is the OTC line SIVEF, whose price feed is delayed and was stamped Aug 6 rather than carrying a '
 'valid 4:00 PM EDT close - unusable for this dashboard. Sivers announced on Apr 16, 2026 that it is pursuing a Nasdaq '
 'New York dual listing but NO US ticker or timeline has been assigned. Held pending the correct ticker from the user. '
 'RECOMMENDATION ON RECORD: this group now holds ANET, APH, GLW, CSCO and VIAV alongside true photonics names, so '
 "renaming it to something like 'Optical & Networking' would describe its contents far better than 'Photonic & Optic "
 "Fiber'. Photonic & Optic Fiber 12 -> 16 names. | Aug 11, 2026 broker sync (POEMS screen stamped Aug 11, 12:20 PM "
 'BKK): TWO NEW FILLS from the Aug 10 US session - MSFT 4 sh (cash 2,060.52 incl 8.56 fee = 512.99/sh; Aug 10 range '
 '502.27-513.73) and ANET 10 sh (1,935.56 incl fee = 192.70/sh; range 187.03-195.70). Both fills independently '
 'confirmed reachable against sourced Aug 10 OHLC. POSITIONS NOW TIE TO THE BROKER TO THE CENT: 50 holdings, total '
 'cost 572,051.56 exactly matching POEMS, market value 692,279.64 vs POEMS 692,279.64, unrealized +120,228.08 '
 '(+21.02%). Two silent cost corrections POEMS surfaced: NVDA 77,332.40 -> 77,340.96 and CRM 3,741.35 -> 3,749.91, '
 'each the 8.56 commission on its Aug 6 fill that the stored TRADES amount had excluded. MONTHLY 2026-08 buyUSD '
 '17,635.10 -> 21,648.30 (lots 10 -> 12) and TOTALS.investedEverUSD 596,410.37 -> 600,423.57 accordingly; from this '
 'entry on, Aug fills carry commission INSIDE the amount, matching POEMS. Cash 11,327.06 -> 7,330.98 (11,327.06 - '
 '2,060.52 - 1,935.56), stamped Aug 11 and stated pre-settlement as POEMS always does. PRICES: the 50 HELD names '
 'advanced to the verified Aug 10, 2026 close, taken from the POEMS Current Price column and cross-checked two ways - '
 "every row's Market Value / Open Qty reproduces the quoted price and every Open Qty Cost / Open Qty reproduces the "
 'quoted average (50 rows, zero arithmetic mismatches, both exchange subtotals and the grand total matching POEMS to '
 'the cent), then 20 of the 50 were independently re-sourced against stockanalysis history plus a second source, all '
 '20 MATCH. Market caps scaled with price; 52-week highs/lows extended where breached. THE 107 WATCHLIST NAMES ARE '
 'STILL AT THE AUG 7 CLOSE - the overnight job (commit 647bb82) refreshed only the RKLB/ASTS/KEEL earnings text, not '
 "prices, so the dashboard is deliberately mixed-date and each card's price badge shows which date it carries. THE "
 "STORY OF AUG 10: the optics complex gave back most of Friday's melt-up - COHR -14.24% to 325.15, LITE -8.61% to "
 '813.51, FN -6.28% to 527.07, ALAB -5.07%, CRDO -3.98%, AAOI -2.08%. NO ban reversal and NO downgrade could be found '
 'dated Aug 8-11; sourced commentary attributes it to profit-taking after parabolic gains (COHR +105% YTD, +44% on the '
 'week alone), stretched multiples (COHR 159.96x trailing, LITE 145.91x) and pre-earnings de-risking, with put/call '
 'running 1.22-1.33 into the Aug 14 expiry. Coherent fell -19.61% on the day of its prior-year Q4 miss, which explains '
 'the hedging. Offsetting it, PANW +5.82% to 385.04, CRWD +5.01% to 225.16 and ISRG +3.85% to 393.38. Indexes eased '
 'off the record: S&P 500 7,753.11 -0.1%, Nasdaq 26,605.36 -0.3%, with Intel -4.06% after saying it may sell 15B of '
 'stock. EARNINGS IMMEDIATELY AHEAD: LITE reports fiscal Q4 FY2026 TONIGHT, Tue Aug 11 after the close '
 '(COMPANY-CONFIRMED, call 5:00 PM ET) - consensus EPS 2.62 against company revenue guidance 960M-1.01B; COHR reports '
 'fiscal Q4 FY2026 Wed Aug 12 after the close (COMPANY-CONFIRMED, call 4:30 PM ET). Both are held positions and both '
 'stored valuations are pre-print. ZETA DATA BUG FIXED: the stored all-time high of 26.64 sat BELOW the current price, '
 'which is impossible. Three sources (stockanalysis, GuruFocus, MarketBeat) agree the 52-week range is 14.37-28.50, so '
 'lo52 14.37 -> 14.37, hi52 27.07 -> 28.50 and ath 26.64 -> 28.50 (the 52-week high used as a proxy). The TRUE '
 "all-time high remains NOT FOUND on any permitted source - Forbes describes ZETA falling 'from levels of around 37 to "
 "18' after the Culper Research report in Nov 2024, so the real record is materially higher and the stored drawdown "
 'understates it. NOTE a 1%% close conflict on ZETA for Aug 10: 27.54 on the POEMS broker screen, GuruFocus and '
 'MarketBeat (27.53) versus 27.25 on the stockanalysis history table; the broker figure was taken. IREN -6.04%% was '
 'second-sourced and is sector-driven, not company news - Bitcoin fell ~2.1%% on Aug 10 and the miners fell with it '
 '(Bitdeer -14%%, HIVE -6%%); no IREN release or rating change dated Aug 8-11 could be found. | Aug 11, 2026 additions '
 '- FIVE NEW WATCHLIST NAMES at the verified Aug 10, 2026 close (no positions, no budgets; jan2 NOT FOUND on all '
 'five). TSEM (Tower Semiconductor, NASDAQ) 252.55 into Computing Chips - an Israeli ANALOG SPECIALTY FOUNDRY, so it '
 'sits beside TSM which is also a foundry; Q2 2026 (Aug 4) revenue 460M +24%, adj EPS 0.88 vs 0.76 consensus, Q3 '
 'guided to a record 520M midpoint (+31% YoY), 2028 model raised to 3.6B revenue described as fully spoken for by '
 'customers. THE REASON TO OWN IT IS SILICON PHOTONICS: annualised run rate 180M in Q2 2025 -> 680M in Q2 2026 (+270% '
 'YoY), targeting 1B by Q4 2026, with 1.3B of signed 2027 contracts. ROIC 14.84%, net cash (D/E 0.05). Trailing P/E '
 '99.86 but forward 49.70 and PEG 1.14. RISKS: fabs in Israel with geopolitical risk disclosed in the Q2 release, and '
 '100x trailing earnings. ath 319.94 is the 52-WEEK high, not the record - companiesmarketcap shows 598.12 in March '
 '2000, split basis unverified. FCF/capex CONFLICT within one source (FCF 411.96M/capex 453.93M on the financials page '
 'vs 300.61M/565.29M on statistics, OCF 865.90M on both); financials pair taken. Next earnings ESTIMATED Nov 9 - '
 "Tower's IR page confirms no date. FOUR ADDED TO CHIPS MANUFACTURING. (1) GFS (GlobalFoundries, NASDAQ) 50.05 - a "
 'mature-node foundry, 73% owned by Mubadala, with up to 1.5B of CHIPS Act money plus 550M from New York State and a '
 'July 2026 300M silicon photonics award under which the US GOVERNMENT TOOK A ~1% EQUITY STAKE. Cheapest of the five '
 'on forward P/E 21.87 with PEG 0.81 and EV/EBITDA 12.61, but the growth is not there: TTM revenue +1.40% and ROIC '
 'only 6.10%. FELL 7.19% on Aug 10 and sits 46% below its 92.55 high. Q2 (Aug 5) beat at 0.46 vs 0.43 but UBS cut to '
 '55 and TD Cowen from 85 to 75 the next day. NOTE: GFS is a foundry like TSM and TSEM, so on business model it '
 'belongs beside them in Computing Chips rather than with the equipment makers - placed as instructed. (2) FORM '
 '(FormFactor, NASDAQ) 112.47 - probe cards, 81% of revenue, plus 19% lab Systems. THE BEST OPERATOR OF THE FOUR: '
 'gross margin 46.18%, ROIC 16.50%, essentially no debt (D/E 0.03), TTM revenue +18.01% and EPS +160.71%. Q2 (Jul 29) '
 'revenue 258.2M +31.9%, non-GAAP EPS 0.82 vs 0.61. HBM was about two thirds of record DRAM revenue and co-packaged '
 'optics should pass 20M in 2026. But it trades at 82x trailing / 34x forward and is 30% below a 160.27 high. (3) COHU '
 '(Cohu, NASDAQ) 49.47 - back-end test handlers and contactors, 53% recurring revenue. WARNING: GAAP LOSS-MAKING on a '
 'trailing basis (EPS -0.83, net margin -7.43%, ROIC -5.31%), so trailing P/E and PEG are blank and EV/EBITDA 254.16 '
 'is meaningless - judge it on forward P/E 35.02. The turn is real though: Q2 revenue 149M +38%, EPS 0.26 vs 0.14, Q3 '
 'guided 170M (+35% YoY), computing 46% of system orders (+150% YoY) and the HPC estimate raised to 100-110M. '
 'Automotive orders still -24% YoY. Fell 6.45% on Aug 10. (4) VECO (Veeco Instruments, NASDAQ) 49.34 - see the RED '
 'FLAG below. *** RED FLAG ON VECO: IT IS A PENDING TAKEOVER TARGET, NOT AN INDEPENDENT INVESTMENT. Veeco agreed to be '
 'acquired by AXCELIS TECHNOLOGIES in an ALL-STOCK merger at a FIXED 0.3575 Axcelis shares per Veeco share (~4.4B; '
 "Veeco holders end up with ~42% of the combined company). Every regulator except China's SAMR has cleared it, closing "
 "is targeted for 2H 2026 and the OUTSIDE DATE IS SEPTEMBER 30, 2026. Because the ratio is fixed, VECO's price is now "
 "mostly a function of AXCELIS's price, not its own results - which is exactly why its stored figures look incoherent "
 '(trailing P/E 134.49 against forward 20.07, a 52-week range of 23.31 to 86.63, and four analyst panels that disagree '
 'wildly: 4 analysts avg 61.33, 6 analysts avg 49.33 with a 30 low, 4 analysts Outperform avg 60.33, and 7 analysts '
 'target 40.60). Fundamentals are the weakest of the five anyway - TTM revenue -2.5%, EPS -63.6%, ROIC 3.98%, net '
 'margin 3.40% - and there is a 230M convertible note due 2029 struck at 29.22 that is deeply in the money, roughly '
 "7.87M shares of dilution. Buying VECO today is a bet on the Axcelis deal closing and on Axcelis's own share price, "
 'not on Veeco. ALSO NOTE its stockanalysis statistics page was still priced off the Aug 7 close of 52.79 when the '
 'ratios were taken, so the stored multiples are marginally stale. *** BUDGETS ALLOCATED at 5,000 each on FN, AAOI, '
 'COHR and IBM (all four previously had no budget). Against existing cost that leaves FN 3,395.44, AAOI 3,661.44, COHR '
 '3,671.44 and IBM 3,286.93 of headroom - 14,015.25 of new commitments against 7,330.98 of cash, so these are '
 'intentions rather than funded orders. All four fell on Aug 10 (COHR -14.24%, FN -6.28%) and COHR reports tomorrow. '
 'ath NOT FOUND on FORM, COHU and VECO - the 52-week highs 160.27, 74.60 and 86.63 were stored as proxies; Veeco dates '
 'to 1945 so its true record is certainly far higher. Chips Manufacturing 6 -> 10 names, Computing Chips 11 -> 12, '
 'total 157 -> 162. | Aug 11, 2026 (evening, after the Aug 11 close): THREE STRUCTURAL CHANGES, all user-directed. (1) '
 'GFS MOVED from Chips Manufacturing to Computing Chips, accepting the note filed with its addition - GlobalFoundries '
 'is a foundry that builds chips other companies design, the same business as TSM and TSEM, while Chips Manufacturing '
 'is otherwise equipment and test makers. Computing Chips 12 -> 13, Chips Manufacturing 10 -> 8 (after the VECO '
 'removal below). GFS carried no position, no budget, no trades and no realised history, so nothing needed '
 're-attributing. (2) VECO DELETED as advised. Veeco is not an independent equity: it agreed to be acquired by Axcelis '
 "Technologies in an ALL-STOCK merger at a FIXED 0.3575 Axcelis shares per Veeco share, all regulators except China's "
 'SAMR have cleared it, and the outside date is Sep 30, 2026 - so its price tracks Axcelis rather than its own '
 'results, which is why its stored multiples were incoherent (134x trailing vs 20x forward; four analyst panels '
 'averaging 61.33, 60.33, 49.33 and 40.60). It was watchlist-only with no position, no budget and no trade history, so '
 'the deletion is clean; sanity_check flags it as VANISHED by design and that single flag was verified as the ONLY one '
 'before pushing. Its overview and weekly-notes entries were removed with it to avoid orphans. If Salee wants exposure '
 'to this combination the instrument is ACLS, not VECO. (3) CHKP (Check Point Software Technologies Ltd, NASDAQ) added '
 'to Cybersecurity at the Aug 10, 2026 close 131.14 (+2.60%), second-sourced; watchlist only, jan2 NOT FOUND. It is '
 'the deliberate opposite of the two cybersecurity names already held: CRWD and PANW are fast growers on rich '
 'multiples, CHKP is a cash machine that barely grows. Gross margin 87.49%, operating margin 29.02%, net margin '
 '37.93%, ROIC 36.04%, FCF 1.10B on 2.76B revenue (39.91% FCF margin), trailing P/E 13.36 and forward 12.16 - the '
 'cheapest quality name anywhere on this dashboard. NO DIVIDEND; it returns cash through buybacks instead, with a 2B '
 'expansion authorised May 11, 2026, ~325M repurchased in Q2 and the share count down 4.38% YoY. THE PROBLEM IS '
 'GROWTH: TTM revenue +4.6% and Q2 2026 (Jul 30) revenue 673.6M only +1% YoY; non-GAAP EPS 2.55 beat but revenue '
 'missed and the stock fell 8.19% that day. The sell side is walking away - JPMorgan to Neutral Jul 27, Raymond James '
 'to Market Perform Jul 15, Roth MKM target to 105 Jul 31, Goldman 168 -> 150 Aug 1, Citi 125 -> 130 Aug 3. Israel '
 'risk is disclosed explicitly in the FY2025 20-F (war involving Israel, the US and Iran; hostilities with Hezbollah, '
 'Hamas and Yemen; reservist call-ups). CONFLICTS on record: analyst panel 38 / Buy / avg 146.32 / high 201 '
 '(stockanalysis) vs 28 / Hold / avg 149.67 / high 220 (MarketBeat), low agrees at 105; 52-week high 210.66 '
 '(stockanalysis) vs 222.22 (GuruFocus), low agrees at 112.23; ROIC 36.04% (stockanalysis) vs an unverified 17.99% '
 '(GuruFocus, not fetched). ath 233.47 dated Jun 6, 2025 is a CLOSING-basis figure from a single non-preferred source '
 "(companiesmarketcap) - no intraday record was found. Next earnings ESTIMATED Oct 27 - Check Point's IR page could "
 'not be read and MarketScreener lists no Q3 date. TIMING NOTE: the Aug 11, 2026 session has now CLOSED, so the whole '
 'dashboard is one session behind - CHKP itself closed Aug 11 at 129.12 (-1.54%). CHKP was stored at the Aug 10 close '
 'for consistency with the 50 held names and the four names added earlier today; the next daily job should advance '
 'everything to Aug 11 together. Total 162 -> 162 (one added, one deleted). | Aug 11, 2026: CHKP REMOVED from '
 "Cybersecurity at Salee's request, hours after being added. It was watchlist only - no position, no budget, no trades "
 'and no realised history - so the removal is clean and leaves nothing orphaned; its overview and weekly-notes entries '
 'were deleted with it. sanity_check flags it as VANISHED by design and that single flag was verified as the ONLY '
 'failure, with zero warnings, before pushing. The research stands on record in this log should it ever be reinstated: '
 'Check Point closed Aug 10 at 131.14 and Aug 11 at 129.12, with 87.49%% gross margin, 29.02%% operating margin, '
 '36.04%% ROIC, 1.10B FCF on 2.76B revenue, trailing P/E 13.36 and forward 12.16, no dividend but a 2B buyback '
 'expansion authorised May 11 2026 - against TTM revenue growth of only 4.6%% and Q2 revenue up 1%% YoY, an 8.19%% '
 'fall on the Jul 30 print, and downgrades from JPMorgan, Raymond James, Roth MKM and Goldman between Jul 15 and Aug '
 '1. Cybersecurity 7 -> 6 names (CRWD, PANW, ZS, FTNT, NET, RBRK). Total 162 -> 161.')

GROUPS = ['Hyperscalers',
 'Computing Chips',
 'Chips Manufacturing',
 'Memory & Storage',
 'Photonic & Optic Fiber',
 'Energy',
 'Robotic / Automation / EV',
 'Space',
 'Drone & Defense',
 'Quantum',
 'SaaS',
 'Consumer / F&B',
 'Healthcare',
 'Neocloud Watch',
 'Digital Fintech',
 'Cybersecurity',
 'Nuclear Energy',
 'Power and Electrification',
 'Renewal Energy',
 'Digital Applications',
 'Industrial STX']

STOCKS = [{'t': 'GOOGL',
  'name': 'Alphabet',
  'ex': 'NASDAQ',
  'g': 0,
  'price': 357.52,
  'jan2': 316.9,
  'mcapB': 4377.0189,
  'ath': 408.61,
  'fvMin': 350,
  'fvAvg': 423.44,
  'fvMax': 475,
  'an': 83,
  'peg': 1.72,
  'ps': 10.02,
  'fpe': 28.01,
  'pe': 27.04,
  'gm': 60.37,
  'pm': 37.92,
  'revB': 422.5,
  'revG': 17.5,
  'eps': 13.11,
  'epsG': 46.4,
  'roi': 28.34,
  'roe': 38.88,
  'fcfB': 64.43,
  'capexB': 109.92,
  'r40': 32.8,
  'budget': 60000,
  'lo52': 187.82,
  'hi52': 408.61,
  'pxd': '2026-08-10'},
 {'t': 'AMZN',
  'name': 'Amazon',
  'ex': 'NASDAQ',
  'g': 0,
  'price': 278.09,
  'jan2': 231.34,
  'mcapB': 3000.2328,
  'ath': 284.02,
  'fvMin': 175,
  'fvAvg': 311.76,
  'fvMax': 335,
  'an': 94,
  'peg': 1.29,
  'ps': 3.5,
  'fpe': 29.55,
  'pe': 27.35,
  'gm': 50.77,
  'pm': 12.1,
  'revB': 775.68,
  'revG': 19.62,
  'eps': 8.61,
  'epsG': 16.07,
  'roi': 13.49,
  'roe': 17.02,
  'fcfB': -7.6,
  'capexB': 169.01,
  'r40': 18.64,
  'budget': 50000,
  'lo52': 196.0,
  'hi52': 284.02,
  'pxd': '2026-08-10'},
 {'t': 'MSFT',
  'name': 'Microsoft',
  'ex': 'NASDAQ',
  'g': 0,
  'price': 506.06,
  'jan2': 482.24,
  'mcapB': 3769.5529,
  'ath': 555.45,
  'fvMin': 400,
  'fvAvg': 549.08,
  'fvMax': 680,
  'an': 82,
  'peg': 1.16,
  'ps': 8.77,
  'fpe': 21.17,
  'pe': 21.76,
  'gm': 67.94,
  'pm': 40.3,
  'revB': 331.84,
  'revG': 18.0,
  'eps': 17.95,
  'epsG': 23.0,
  'roi': 21.72,
  'roe': 35.39,
  'fcfB': 71.61,
  'capexB': 64.55,
  'r40': 39.6,
  'budget': 60000,
  'lo52': 349.2,
  'hi52': 555.45,
  'pxd': '2026-08-10'},
 {'t': 'META',
  'name': 'Meta Platforms',
  'ex': 'NASDAQ',
  'g': 0,
  'price': 594.92,
  'jan2': 649.25,
  'mcapB': 1517.4572,
  'ath': 796.25,
  'fvMin': 700,
  'fvAvg': 804.17,
  'fvMax': 880,
  'an': 65,
  'peg': 2.85,
  'ps': 7.16,
  'fpe': 17.93,
  'pe': 22.28,
  'gm': 81.75,
  'pm': 29.84,
  'revB': 228.25,
  'revG': 28.0,
  'eps': 26.55,
  'epsG': -13.4,
  'roi': 19.75,
  'roe': 26.07,
  'fcfB': 40.98,
  'capexB': 89.33,
  'r40': 45.9,
  'budget': 50000,
  'lo52': 520.26,
  'hi52': 796.25,
  'pxd': '2026-08-10'},
 {'t': 'AAPL',
  'name': 'Apple',
  'ex': 'NASDAQ',
  'g': 0,
  'price': 308.26,
  'jan2': 270.51,
  'mcapB': 4500.5128,
  'ath': 334.99,
  'fvMin': 253,
  'fvAvg': 342.11,
  'fvMax': 400,
  'an': 111,
  'peg': 3.0,
  'ps': 10.29,
  'fpe': 34.41,
  'pe': 38.05,
  'gm': 48.65,
  'pm': 27.62,
  'revB': 466.82,
  'revG': 16.36,
  'eps': 8.76,
  'epsG': 28.66,
  'roi': 104.33,
  'roe': 119.91,
  'fcfB': 136.68,
  'capexB': 10.04,
  'r40': 45.64,
  'budget': 25000,
  'lo52': 201.5,
  'hi52': 343.67,
  'pxd': '2026-08-10'},
 {'t': 'NVDA',
  'name': 'NVIDIA',
  'ex': 'NASDAQ',
  'g': 1,
  'price': 217.55,
  'jan2': 189.6,
  'mcapB': 5266.7124,
  'ath': 236.54,
  'fvMin': 218,
  'fvAvg': 319.48,
  'fvMax': 500,
  'an': 79,
  'peg': 0.46,
  'ps': 19.38,
  'fpe': 20.34,
  'pe': 31.06,
  'gm': 74.15,
  'pm': 62.97,
  'revB': 253.49,
  'revG': 70.7,
  'eps': 6.53,
  'epsG': 110.6,
  'roi': 104.67,
  'roe': 114.29,
  'fcfB': 119.08,
  'capexB': 6.57,
  'r40': 117.7,
  'budget': 80000,
  'lo52': 164.07,
  'hi52': 236.54,
  'pxd': '2026-08-10'},
 {'t': 'TSM',
  'name': 'Taiwan Semiconductor',
  'ex': 'NYSE',
  'g': 1,
  'price': 418.47,
  'jan2': 311.98,
  'mcapB': 2018.5914,
  'ath': 479.0,
  'fvMin': 500,
  'fvAvg': 596,
  'fvMax': 700,
  'an': 25,
  'peg': None,
  'ps': 13.3,
  'fpe': 18.8,
  'pe': 26.64,
  'gm': 64.23,
  'pm': 49.92,
  'revB': 139.57,
  'revG': 30.6,
  'eps': 13.44,
  'epsG': 53.4,
  'roi': 54.61,
  'roe': 39.97,
  'fcfB': 35.28,
  'capexB': 46.87,
  'r40': 55.8,
  'budget': 50000,
  'lo52': 223.7,
  'hi52': 479.0,
  'pxd': '2026-08-10'},
 {'t': 'AVGO',
  'name': 'Broadcom',
  'ex': 'NASDAQ',
  'g': 1,
  'price': 422.4,
  'jan2': 352.78,
  'mcapB': 2007.3988,
  'ath': 495.0,
  'fvMin': 215.88,
  'fvAvg': 523.73,
  'fvMax': 650,
  'an': 48,
  'peg': 0.52,
  'ps': 23.38,
  'fpe': 23.53,
  'pe': 61.72,
  'gm': 68.28,
  'pm': 38.85,
  'revB': 75.47,
  'revG': 32.3,
  'eps': 6.01,
  'epsG': 124.7,
  'roi': 24.22,
  'roe': 37.28,
  'fcfB': 32.76,
  'capexB': 0.86,
  'r40': 75.7,
  'budget': 40000,
  'lo52': 273.0,
  'hi52': 495.0,
  'pxd': '2026-08-10'},
 {'t': 'AMD',
  'name': 'Advanced Micro Devices',
  'ex': 'NASDAQ',
  'g': 1,
  'price': 469.56,
  'jan2': 218.9,
  'mcapB': 765.6607,
  'ath': 584.73,
  'fvMin': 260,
  'fvAvg': 574.88,
  'fvMax': 1250,
  'an': 70,
  'peg': 0.95,
  'ps': 20.47,
  'fpe': 56.08,
  'pe': 133.31,
  'gm': 53.06,
  'pm': 15.57,
  'revB': 41.3,
  'revG': 39.6,
  'eps': 3.89,
  'epsG': 122.1,
  'roi': 7.83,
  'roe': 9.57,
  'fcfB': 8.95,
  'capexB': 1.68,
  'r40': 61.3,
  'budget': 25000,
  'lo52': 149.22,
  'hi52': 584.73,
  'pxd': '2026-08-10'},
 {'t': 'MRVL',
  'name': 'Marvell Technology',
  'ex': 'NASDAQ',
  'g': 1,
  'price': 208.56,
  'jan2': 86.74,
  'mcapB': 182.648,
  'ath': 329.88,
  'fvMin': 110,
  'fvAvg': 252.56,
  'fvMax': 385,
  'an': 43,
  'peg': 1.17,
  'ps': 18.96,
  'fpe': 41.55,
  'pe': 65.17,
  'gm': 51.5,
  'pm': 28.99,
  'revB': 8.72,
  'revG': 34.1,
  'eps': 2.89,
  'epsG': None,
  'roi': 6.87,
  'roe': 16.03,
  'fcfB': 1.67,
  'capexB': 0.39,
  'r40': 53.2,
  'budget': 25000,
  'lo52': 61.44,
  'hi52': 329.88,
  'pxd': '2026-08-10'},
 {'t': 'INTC',
  'name': 'Intel',
  'ex': 'NASDAQ',
  'g': 1,
  'price': 97.52,
  'jan2': 37.77,
  'mcapB': 491.8973,
  'ath': 142.35,
  'fvMin': 60,
  'fvAvg': 110.04,
  'fvMax': 200,
  'an': 85,
  'peg': 1.18,
  'ps': 8.88,
  'fpe': 87.49,
  'pe': None,
  'gm': 37.2,
  'pm': -5.9,
  'revB': 53.76,
  'revG': 1.4,
  'eps': -0.67,
  'epsG': None,
  'roi': 1.47,
  'roe': -2.91,
  'fcfB': -3.12,
  'capexB': 13.1,
  'r40': -4.4,
  'budget': 5000,
  'lo52': 18.97,
  'hi52': 142.35,
  'pxd': '2026-08-10'},
 {'t': 'QCOM',
  'name': 'Qualcomm',
  'ex': 'NASDAQ',
  'g': 1,
  'price': 162.17,
  'jan2': 173.8,
  'mcapB': 173.2117,
  'ath': 259.92,
  'fvMin': 100,
  'fvAvg': 221.9,
  'fvMax': 314,
  'an': 36,
  'peg': 5.98,
  'ps': 4.07,
  'fpe': 17.39,
  'pe': 18.77,
  'gm': 54.8,
  'pm': 22.31,
  'revB': 44.49,
  'revG': 5.2,
  'eps': 9.15,
  'epsG': -6.3,
  'roi': 32.94,
  'roe': 36.08,
  'fcfB': 12.5,
  'capexB': 1.78,
  'r40': 33.3,
  'budget': 5000,
  'lo52': 121.99,
  'hi52': 259.92,
  'pxd': '2026-08-10'},
 {'t': 'ARM',
  'name': 'Arm Holdings',
  'ex': 'NASDAQ',
  'g': 1,
  'price': 282.57,
  'jan2': 112.83,
  'mcapB': 301.8131,
  'ath': 452.7,
  'fvMin': 125,
  'fvAvg': 301.81,
  'fvMax': 500,
  'an': 40,
  'peg': 3.58,
  'ps': 58.53,
  'fpe': 124.44,
  'pe': 318.55,
  'gm': 97.54,
  'pm': 18.37,
  'revB': 4.92,
  'revG': 22.8,
  'eps': 0.85,
  'epsG': 12.0,
  'roi': 14.34,
  'roe': 11.95,
  'fcfB': 0.98,
  'capexB': 0.55,
  'r40': 42.7,
  'budget': None,
  'lo52': 100.02,
  'hi52': 452.7,
  'pxd': '2026-08-07'},
 {'t': 'ON',
  'name': 'ON Semiconductor',
  'ex': 'NASDAQ',
  'g': 1,
  'price': 81.17,
  'jan2': None,
  'mcapB': 31.6,
  'ath': 134.92,
  'fvMin': 85,
  'fvAvg': 108.88,
  'fvMax': 150,
  'an': 29,
  'peg': 0.62,
  'ps': 5.1,
  'fpe': 20.92,
  'pe': 52.01,
  'gm': 42.78,
  'pm': 10.17,
  'revB': 6.2,
  'revG': None,
  'eps': 1.56,
  'epsG': 43.5,
  'roi': 15.6,
  'roe': 8.31,
  'fcfB': 1.5,
  'capexB': 0.1716,
  'r40': None,
  'budget': None,
  'lo52': 44.56,
  'hi52': 134.92,
  'pxd': '2026-08-07'},
 {'t': 'MCHP',
  'name': 'Microchip Technology',
  'ex': 'NASDAQ',
  'g': 1,
  'price': 84.69,
  'jan2': None,
  'mcapB': 45.99,
  'ath': 105.91,
  'fvMin': 80,
  'fvAvg': 109.21,
  'fvMax': 135,
  'an': 25,
  'peg': 0.47,
  'ps': 8.98,
  'fpe': 21.46,
  'pe': 124.83,
  'gm': 60.19,
  'pm': 9.34,
  'revB': 5.122,
  'revG': 20.93,
  'eps': 0.67,
  'epsG': None,
  'roi': 5.87,
  'roe': 7.19,
  'fcfB': 1.111,
  'capexB': 0.0871,
  'r40': 42.6,
  'budget': None,
  'lo52': 48.52,
  'hi52': 105.91,
  'pxd': '2026-08-07'},
 {'t': 'NXPI',
  'name': 'NXP Semiconductors',
  'ex': 'NASDAQ',
  'g': 1,
  'price': 239.71,
  'jan2': None,
  'mcapB': 60.45,
  'ath': 339.95,
  'fvMin': 190,
  'fvAvg': 311.1,
  'fvMax': 400,
  'an': 30,
  'peg': 0.74,
  'ps': 4.58,
  'fpe': 14.33,
  'pe': 20.46,
  'gm': 56.04,
  'pm': 22.56,
  'revB': 13.185,
  'revG': 8.77,
  'eps': 11.72,
  'epsG': 39.69,
  'roi': 17.99,
  'roe': 27.92,
  'fcfB': 2.81,
  'capexB': 0.323,
  'r40': 30.08,
  'budget': None,
  'lo52': 183.0,
  'hi52': 339.95,
  'pxd': '2026-08-07'},
 {'t': 'TSEM',
  'name': 'Tower Semiconductor',
  'ex': 'NASDAQ',
  'g': 1,
  'price': 252.55,
  'jan2': None,
  'mcapB': 28.47,
  'ath': 319.94,
  'fvMin': 278,
  'fvAvg': 321.32,
  'fvMax': 355,
  'an': 7,
  'peg': 1.14,
  'ps': 16.65,
  'fpe': 49.7,
  'pe': 99.86,
  'gm': 26.87,
  'pm': 16.94,
  'revB': 1.71,
  'revG': 14.9,
  'eps': 2.53,
  'epsG': 45.4,
  'roi': 14.84,
  'roe': 9.98,
  'fcfB': 0.412,
  'capexB': 0.454,
  'r40': 39.0,
  'budget': None,
  'lo52': 45.51,
  'hi52': 319.94,
  'pxd': '2026-08-10'},
 {'t': 'GFS',
  'name': 'GlobalFoundries',
  'ex': 'NASDAQ',
  'g': 1,
  'price': 50.05,
  'jan2': None,
  'mcapB': 27.46,
  'ath': 92.55,
  'fvMin': 55,
  'fvAvg': 76.76,
  'fvMax': 140,
  'an': 21,
  'peg': 0.81,
  'ps': 3.96,
  'fpe': 21.87,
  'pe': 39.09,
  'gm': 27.14,
  'pm': 10.32,
  'revB': 6.938,
  'revG': 1.4,
  'eps': 1.28,
  'epsG': None,
  'roi': 6.1,
  'roe': 6.17,
  'fcfB': 0.796,
  'capexB': 1.12,
  'r40': 12.9,
  'budget': None,
  'lo52': 31.51,
  'hi52': 92.55,
  'pxd': '2026-08-10'},
 {'t': 'ASML',
  'name': 'ASML Holding',
  'ex': 'NASDAQ',
  'g': 2,
  'price': 1733.48,
  'jan2': 1133.76,
  'mcapB': 674.2601,
  'ath': 1999.96,
  'fvMin': 885.04,
  'fvAvg': 2122.44,
  'fvMax': 2859,
  'an': 44,
  'peg': None,
  'ps': 16.77,
  'fpe': 31.47,
  'pe': 55.68,
  'gm': 52.73,
  'pm': 30.11,
  'revB': 40.29,
  'revG': 9.8,
  'eps': 31.41,
  'epsG': 14.5,
  'roi': 65.98,
  'roe': 53.94,
  'fcfB': 11.49,
  'capexB': 1.65,
  'r40': 38.4,
  'budget': 30000,
  'lo52': 683.48,
  'hi52': 1999.96,
  'pxd': '2026-08-10'},
 {'t': 'AMAT',
  'name': 'Applied Materials',
  'ex': 'NASDAQ',
  'g': 2,
  'price': 522.12,
  'jan2': 267.08,
  'mcapB': 414.5443,
  'ath': 739.67,
  'fvMin': 358,
  'fvAvg': 623.06,
  'fvMax': 900,
  'an': 39,
  'peg': 1.28,
  'ps': 14.49,
  'fpe': 35.46,
  'pe': 49.82,
  'gm': 48.96,
  'pm': 29.31,
  'revB': 29.02,
  'revG': 3.3,
  'eps': 10.63,
  'epsG': 29.6,
  'roi': 33.23,
  'roe': 39.69,
  'fcfB': 5.34,
  'capexB': 2.65,
  'r40': 21.7,
  'budget': 20000,
  'lo52': 154.47,
  'hi52': 739.67,
  'pxd': '2026-08-10'},
 {'t': 'LRCX',
  'name': 'Lam Research',
  'ex': 'NASDAQ',
  'g': 2,
  'price': 306.4,
  'jan2': 177.85,
  'mcapB': 383.3926,
  'ath': 438.5,
  'fvMin': 220,
  'fvAvg': 366.48,
  'fvMax': 500,
  'an': 35,
  'peg': 1.87,
  'ps': 18.07,
  'fpe': 41.37,
  'pe': 59.18,
  'gm': 49.98,
  'pm': 30.94,
  'revB': 21.68,
  'revG': 26.5,
  'eps': 5.29,
  'epsG': 47.6,
  'roi': 73.88,
  'roe': 66.76,
  'fcfB': 6.0,
  'capexB': 0.95,
  'r40': 54.2,
  'budget': 15000,
  'lo52': 90.94,
  'hi52': 438.5,
  'pxd': '2026-08-10'},
 {'t': 'KLAC',
  'name': 'KLA Corporation',
  'ex': 'NASDAQ',
  'g': 2,
  'price': 192.74,
  'jan2': 125.97,
  'mcapB': 251.7697,
  'ath': 307.37,
  'fvMin': 150,
  'fvAvg': 234.57,
  'fvMax': 325,
  'an': 29,
  'peg': 1.75,
  'ps': 19.34,
  'fpe': 44.29,
  'pe': 49.27,
  'gm': 61.3,
  'pm': 35.57,
  'revB': 13.58,
  'revG': 15.21,
  'eps': 3.66,
  'epsG': 14.29,
  'roi': 70.73,
  'roe': 76.08,
  'fcfB': 3.77,
  'capexB': 0.38,
  'r40': 42.97,
  'budget': 10000,
  'lo52': 83.22,
  'hi52': 307.37,
  'pxd': '2026-08-10'},
 {'t': 'AMKR',
  'name': 'Amkor Technology',
  'ex': 'NASDAQ',
  'g': 2,
  'price': 52.87,
  'jan2': 41.05,
  'mcapB': 13.1344,
  'ath': 96.68,
  'fvMin': 60,
  'fvAvg': 78.0,
  'fvMax': 90,
  'an': 10,
  'peg': 1.4,
  'ps': 2.21,
  'fpe': 28.95,
  'pe': 36.2,
  'gm': 14.43,
  'pm': 6.17,
  'revB': 7.07,
  'revG': 12.7,
  'eps': 1.74,
  'epsG': 35.8,
  'roi': 10.71,
  'roe': 10.01,
  'fcfB': 0.17,
  'capexB': 1.05,
  'r40': 15.1,
  'budget': 5000,
  'lo52': 20.5,
  'hi52': 96.68,
  'pxd': '2026-08-10'},
 {'t': 'AEHR',
  'name': 'Aehr Test Systems',
  'ex': 'NASDAQ',
  'g': 2,
  'price': 103.07,
  'jan2': 26.32,
  'mcapB': 3.3502,
  'ath': 126.62,
  'fvMin': 110,
  'fvAvg': 115.0,
  'fvMax': 125,
  'an': 4,
  'peg': 0.32,
  'ps': 49.58,
  'fpe': 101.76,
  'pe': None,
  'gm': 38.5,
  'pm': -14.25,
  'revB': 0.05,
  'revG': -15.21,
  'eps': -0.23,
  'epsG': None,
  'roi': -11.15,
  'roe': -4.16,
  'fcfB': -0.01,
  'capexB': 0.0,
  'r40': -25.97,
  'budget': None,
  'lo52': 15.94,
  'hi52': 126.62,
  'pxd': '2026-08-07'},
 {'t': 'FORM',
  'name': 'FormFactor',
  'ex': 'NASDAQ',
  'g': 2,
  'price': 112.47,
  'jan2': None,
  'mcapB': 9.3,
  'ath': 160.27,
  'fvMin': 64,
  'fvAvg': 138.63,
  'fvMax': 175,
  'an': 10,
  'peg': None,
  'ps': 10.24,
  'fpe': 34.46,
  'pe': 82.03,
  'gm': 46.18,
  'pm': 12.8,
  'revB': 0.902,
  'revG': 18.01,
  'eps': 1.46,
  'epsG': 160.71,
  'roi': 16.5,
  'roe': 10.99,
  'fcfB': 0.136,
  'capexB': 0.044,
  'r40': 33.1,
  'budget': None,
  'lo52': 27.0,
  'hi52': 160.27,
  'pxd': '2026-08-10'},
 {'t': 'COHU',
  'name': 'Cohu',
  'ex': 'NASDAQ',
  'g': 2,
  'price': 49.47,
  'jan2': None,
  'mcapB': 2.4,
  'ath': 74.6,
  'fvMin': 65,
  'fvAvg': 70.88,
  'fvMax': 80,
  'an': 8,
  'peg': None,
  'ps': 4.65,
  'fpe': 35.02,
  'pe': None,
  'gm': 43.95,
  'pm': -7.43,
  'revB': 0.5226,
  'revG': 32.66,
  'eps': -0.83,
  'epsG': None,
  'roi': -5.31,
  'roe': -4.84,
  'fcfB': 0.035,
  'capexB': 0.012,
  'r40': 39.3,
  'budget': None,
  'lo52': 18.67,
  'hi52': 74.6,
  'pxd': '2026-08-10'},
 {'t': 'MU',
  'name': 'Micron Technology',
  'ex': 'NASDAQ',
  'g': 3,
  'price': 861.0,
  'jan2': 295.13,
  'mcapB': 972.4058,
  'ath': 1255.0,
  'fvMin': 361,
  'fvAvg': 1486,
  'fvMax': 2200,
  'an': 45,
  'peg': 0.03,
  'ps': 10.62,
  'fpe': 5.92,
  'pe': 19.16,
  'gm': 72.57,
  'pm': 55.91,
  'revB': 90.27,
  'revG': 167.0,
  'eps': 44.31,
  'epsG': 695.9,
  'roi': 67.64,
  'roe': 66.64,
  'fcfB': 26.17,
  'capexB': 25.26,
  'r40': 196.0,
  'budget': 30000,
  'lo52': 103.38,
  'hi52': 1255.0,
  'pxd': '2026-08-10'},
 {'t': 'SKHY',
  'name': "SK hynix (NASDAQ ADR, listed Jul 10 '26)",
  'ex': 'NASDAQ',
  'g': 3,
  'price': 135.29,
  'jan2': None,
  'mcapB': 820.025,
  'ath': 194.8,
  'fvMin': 281.67,
  'fvAvg': 281.67,
  'fvMax': 281.67,
  'an': 3,
  'peg': None,
  'ps': 10.4,
  'fpe': 4.88,
  'pe': 18.31,
  'gm': 68.34,
  'pm': 56.89,
  'revB': 86.77,
  'revG': 85.0,
  'eps': 9.39,
  'epsG': 185.7,
  'roi': 51.79,
  'roe': 61.17,
  'fcfB': 28.3,
  'capexB': 19.6,
  'r40': 116.6,
  'budget': 10000,
  'lo52': 124.8,
  'hi52': 194.8,
  'pxd': '2026-08-10'},
 {'t': 'SNDK',
  'name': 'SanDisk',
  'ex': 'NASDAQ',
  'g': 3,
  'price': 1212.21,
  'jan2': 244.35,
  'mcapB': 179.5121,
  'ath': 1804.0,
  'fvMin': 1000,
  'fvAvg': 2144,
  'fvMax': 3250,
  'an': 23,
  'peg': 0.05,
  'ps': 17.85,
  'fpe': 8.62,
  'pe': 53.5,
  'gm': 56.04,
  'pm': 34.19,
  'revB': 13.18,
  'revG': 169.0,
  'eps': 29.71,
  'epsG': None,
  'roi': 48.64,
  'roe': 39.3,
  'fcfB': 4.46,
  'capexB': 0.179,
  'r40': 202.8,
  'budget': None,
  'lo52': 40.1,
  'hi52': 2354.39,
  'pxd': '2026-08-07'},
 {'t': 'WDC',
  'name': 'Western Digital',
  'ex': 'NASDAQ',
  'g': 3,
  'price': 434.3,
  'jan2': 177.2,
  'mcapB': 149.701,
  'ath': 602.54,
  'fvMin': 415,
  'fvAvg': 633.83,
  'fvMax': 1050,
  'an': 26,
  'peg': 0.46,
  'ps': 16.05,
  'fpe': 33.9,
  'pe': 30.03,
  'gm': 45.43,
  'pm': 55.29,
  'revB': 11.78,
  'revG': 35.2,
  'eps': 18.35,
  'epsG': None,
  'roi': 41.87,
  'roe': 85.92,
  'fcfB': 2.91,
  'capexB': 0.381,
  'r40': 59.9,
  'budget': None,
  'lo52': 69.3,
  'hi52': 799.87,
  'pxd': '2026-08-07'},
 {'t': 'STX',
  'name': 'Seagate Technology',
  'ex': 'NASDAQ',
  'g': 3,
  'price': 812.76,
  'jan2': 286.835,
  'mcapB': 183.8993,
  'ath': 1145.0,
  'fvMin': 680,
  'fvAvg': 1016,
  'fvMax': 1600,
  'an': 25,
  'peg': 0.38,
  'ps': 13.86,
  'fpe': 25.48,
  'pe': 53.76,
  'gm': 45.58,
  'pm': 26.11,
  'revB': 12.2,
  'revG': 34.1,
  'eps': 13.9,
  'epsG': 105.3,
  'roi': 89.43,
  'roe': 371.53,
  'fcfB': 3.11,
  'capexB': 0.569,
  'r40': 60.2,
  'budget': None,
  'lo52': 138.3,
  'hi52': 1145.0,
  'pxd': '2026-08-07'},
 {'t': 'DELL',
  'name': 'Dell Technologies',
  'ex': 'NYSE',
  'g': 13,
  'price': 453.77,
  'jan2': 120.62,
  'mcapB': 293.2021,
  'ath': 469.47,
  'fvMin': 360,
  'fvAvg': 501.04,
  'fvMax': 700,
  'an': 27,
  'peg': 0.91,
  'ps': 2.11,
  'fpe': 23.5,
  'pe': 34.97,
  'gm': 19.21,
  'pm': 6.28,
  'revB': 134.0,
  'revG': 38.57,
  'eps': 12.51,
  'epsG': 97.03,
  'roi': 23.97,
  'roe': None,
  'fcfB': 9.44,
  'capexB': 3.03,
  'r40': 45.61,
  'budget': None,
  'lo52': 110.22,
  'hi52': 469.47,
  'pxd': '2026-08-07'},
 {'t': 'HPE',
  'name': 'Hewlett Packard Enterprise',
  'ex': 'NYSE',
  'g': 13,
  'price': 53.22,
  'jan2': 22.17,
  'mcapB': 70.4762,
  'ath': 64.25,
  'fvMin': 28,
  'fvAvg': 64.13,
  'fvMax': 80,
  'an': 22,
  'peg': 0.42,
  'ps': 1.63,
  'fpe': 12.36,
  'pe': 44.5,
  'gm': 33.78,
  'pm': 4.01,
  'revB': 38.79,
  'revG': 22.58,
  'eps': 1.07,
  'epsG': 8.08,
  'roi': 5.58,
  'roe': 6.31,
  'fcfB': 3.99,
  'capexB': 2.37,
  'r40': 32.87,
  'budget': None,
  'lo52': 17.49,
  'hi52': 64.25,
  'pxd': '2026-08-07'},
 {'t': 'SMCI',
  'name': 'Super Micro Computer',
  'ex': 'NASDAQ',
  'g': 13,
  'price': 31.13,
  'jan2': 30.16,
  'mcapB': 20.141,
  'ath': 56.64,
  'fvMin': 15,
  'fvAvg': 37.81,
  'fvMax': 58,
  'an': 19,
  'peg': 0.3,
  'ps': 0.58,
  'fpe': 8.85,
  'pe': 15.78,
  'gm': 8.39,
  'pm': 3.7,
  'revB': 33.7,
  'revG': 56.24,
  'eps': 1.91,
  'epsG': 4.42,
  'roi': 10.43,
  'roe': 17.88,
  'fcfB': -6.85,
  'capexB': 0.16,
  'r40': 35.91,
  'budget': None,
  'lo52': 19.48,
  'hi52': 62.36,
  'pxd': '2026-08-07'},
 {'t': 'ANET',
  'name': 'Arista Networks',
  'ex': 'NYSE',
  'g': 4,
  'price': 191.52,
  'jan2': 134.05,
  'mcapB': 239.5942,
  'ath': 191.52,
  'fvMin': 164,
  'fvAvg': 191.75,
  'fvMax': 220,
  'an': 30,
  'peg': 2.22,
  'ps': 22.61,
  'fpe': 44.53,
  'pe': 60.29,
  'gm': 63.54,
  'pm': 38.33,
  'revB': 10.54,
  'revG': 32.6,
  'eps': 3.16,
  'epsG': 24.8,
  'roi': 237.43,
  'roe': 27.3,
  'fcfB': 5.16,
  'capexB': 0.16,
  'r40': 81.5,
  'budget': 20000,
  'lo52': 114.52,
  'hi52': 197.31,
  'pxd': '2026-08-10'},
 {'t': 'CRDO',
  'name': 'Credo Technology',
  'ex': 'NASDAQ',
  'g': 4,
  'price': 239.935,
  'jan2': 148.4,
  'mcapB': 44.7454,
  'ath': 308.67,
  'fvMin': 184,
  'fvAvg': 271.92,
  'fvMax': 350,
  'an': 19,
  'peg': 0.73,
  'ps': 28.31,
  'fpe': 33.07,
  'pe': 80.75,
  'gm': 68.04,
  'pm': 35.37,
  'revB': 1.34,
  'revG': 205.7,
  'eps': 2.51,
  'epsG': 765.5,
  'roi': 73.1,
  'roe': 34.41,
  'fcfB': 0.41,
  'capexB': 0.06,
  'r40': 236.2,
  'budget': 15000,
  'lo52': 86.49,
  'hi52': 308.67,
  'pxd': '2026-08-10'},
 {'t': 'LITE',
  'name': 'Lumentum',
  'ex': 'NASDAQ',
  'g': 4,
  'price': 813.51,
  'jan2': 379.38,
  'mcapB': 63.2853,
  'ath': 1085.68,
  'fvMin': 600,
  'fvAvg': 1103.49,
  'fvMax': 1400,
  'an': 25,
  'peg': 0.41,
  'ps': 22.91,
  'fpe': 46.0,
  'pe': 139.31,
  'gm': 40.84,
  'pm': 17.68,
  'revB': 2.49,
  'revG': 69.0,
  'eps': 5.26,
  'epsG': None,
  'roi': 8.3,
  'roe': 22.83,
  'fcfB': 0.11,
  'capexB': 0.34,
  'r40': 73.6,
  'budget': 20000,
  'lo52': 101.61,
  'hi52': 1085.68,
  'pxd': '2026-08-10'},
 {'t': 'ALAB',
  'name': 'Astera Labs',
  'ex': 'NASDAQ',
  'g': 4,
  'price': 317.23,
  'jan2': 171.69,
  'mcapB': 54.3752,
  'ath': 499.48,
  'fvMin': 155,
  'fvAvg': 281.56,
  'fvMax': 460,
  'an': 26,
  'peg': 1.77,
  'ps': 51.58,
  'fpe': 89.54,
  'pe': 205.5,
  'gm': 75.99,
  'pm': 26.56,
  'revB': 1.2,
  'revG': 98.5,
  'eps': 1.76,
  'epsG': 197.1,
  'roi': 79.74,
  'roe': 18.5,
  'fcfB': 0.28,
  'capexB': 0.06,
  'r40': 121.5,
  'budget': 15000,
  'lo52': 97.89,
  'hi52': 499.48,
  'pxd': '2026-08-10'},
 {'t': 'APH',
  'name': 'Amphenol',
  'ex': 'NYSE',
  'g': 4,
  'price': 167.7,
  'jan2': 137.12,
  'mcapB': 206.7759,
  'ath': 178.52,
  'fvMin': 165,
  'fvAvg': 187.83,
  'fvMax': 215,
  'an': 18,
  'peg': 1.76,
  'ps': 7.18,
  'fpe': 29.81,
  'pe': 43.5,
  'gm': 37.92,
  'pm': 17.24,
  'revB': 25.9,
  'revG': 54.4,
  'eps': 3.48,
  'epsG': 68.9,
  'roi': 21.63,
  'roe': 36.82,
  'fcfB': 4.63,
  'capexB': 1.1,
  'r40': 72.3,
  'budget': 10000,
  'lo52': 102.76,
  'hi52': 178.52,
  'pxd': '2026-08-10'},
 {'t': 'GLW',
  'name': 'Corning',
  'ex': 'NYSE',
  'g': 4,
  'price': 165.68,
  'jan2': 88.91,
  'mcapB': 142.7215,
  'ath': 271.78,
  'fvMin': 155,
  'fvAvg': 213.07,
  'fvMax': 270,
  'an': 16,
  'peg': 1.58,
  'ps': 8.26,
  'fpe': 45.43,
  'pe': 73.71,
  'gm': 36.38,
  'pm': 11.09,
  'revB': 16.32,
  'revG': 21.4,
  'eps': 2.08,
  'epsG': 27.1,
  'roi': 10.41,
  'roe': 16.74,
  'fcfB': 1.5,
  'capexB': 1.41,
  'r40': 30.6,
  'budget': None,
  'lo52': 54.37,
  'hi52': 271.78,
  'pxd': '2026-08-07'},
 {'t': 'FN',
  'name': 'Fabrinet',
  'ex': 'NYSE',
  'g': 4,
  'price': 527.07,
  'jan2': 464.74,
  'mcapB': 18.8893,
  'ath': 748.89,
  'fvMin': 635,
  'fvAvg': 749.11,
  'fvMax': 850,
  'an': 9,
  'peg': None,
  'ps': 4.18,
  'fpe': 30.16,
  'pe': 42.45,
  'gm': 12.04,
  'pm': 9.94,
  'revB': 4.24,
  'revG': 34.6,
  'eps': 11.64,
  'epsG': 35.8,
  'roi': 30.23,
  'roe': 19.99,
  'fcfB': 0.05,
  'capexB': 0.21,
  'r40': 35.7,
  'budget': 5000,
  'lo52': 272.49,
  'hi52': 748.89,
  'pxd': '2026-08-10'},
 {'t': 'AAOI',
  'name': 'Applied Optoelectronics',
  'ex': 'NASDAQ',
  'g': 4,
  'price': 132.81,
  'jan2': 36.35,
  'mcapB': 10.6588,
  'ath': 233.67,
  'fvMin': 57.5,
  'fvAvg': 151.3,
  'fvMax': 220,
  'an': 6,
  'peg': None,
  'ps': 16.73,
  'fpe': 68.28,
  'pe': None,
  'gm': 28.92,
  'pm': -9.57,
  'revB': 0.596,
  'revG': 61.85,
  'eps': -0.78,
  'epsG': None,
  'roi': -5.58,
  'roe': -5.45,
  'fcfB': -0.592,
  'capexB': 0.46,
  'r40': -37.5,
  'budget': 5000,
  'lo52': 15.29,
  'hi52': 233.67,
  'pxd': '2026-08-10'},
 {'t': 'POET',
  'name': 'POET Technologies',
  'ex': 'NASDAQ',
  'g': 4,
  'price': 8.91,
  'jan2': None,
  'mcapB': 1.5422,
  'ath': 20.81,
  'fvMin': None,
  'fvAvg': None,
  'fvMax': None,
  'an': 0,
  'peg': None,
  'ps': 854.73,
  'fpe': None,
  'pe': None,
  'gm': 100.0,
  'pm': None,
  'revB': 0.00141,
  'revG': 607.6,
  'eps': -0.74,
  'epsG': None,
  'roi': -12.72,
  'roe': -33.94,
  'fcfB': -0.03508,
  'capexB': 0.00417,
  'r40': None,
  'budget': None,
  'lo52': 3.87,
  'hi52': 20.81,
  'pxd': '2026-08-07'},
 {'t': 'COHR',
  'name': 'Coherent',
  'ex': 'NYSE',
  'g': 4,
  'price': 325.15,
  'jan2': 194.33,
  'mcapB': 63.6078,
  'ath': 440.0,
  'fvMin': 230.0,
  'fvAvg': 391.45,
  'fvMax': 465.0,
  'an': 22,
  'peg': 0.89,
  'ps': 9.59,
  'fpe': 43.26,
  'pe': 135.14,
  'gm': 36.99,
  'pm': 7.1,
  'revB': 6.602,
  'revG': 18.0,
  'eps': 2.4,
  'epsG': None,
  'roi': 6.05,
  'roe': 4.72,
  'fcfB': -0.538,
  'capexB': 0.679,
  'r40': 9.8,
  'budget': 5000,
  'lo52': 84.35,
  'hi52': 440.0,
  'pxd': '2026-08-10'},
 {'t': 'LWLG',
  'name': 'Lightwave Logic',
  'ex': 'NASDAQ',
  'g': 4,
  'price': 8.11,
  'jan2': None,
  'mcapB': 1.2494,
  'ath': 19.56,
  'fvMin': None,
  'fvAvg': None,
  'fvMax': None,
  'an': 0,
  'peg': None,
  'ps': 5040.18,
  'fpe': None,
  'pe': None,
  'gm': 97.48,
  'pm': None,
  'revB': 0.000243,
  'revG': 175.9,
  'eps': -0.16,
  'epsG': None,
  'roi': -24.28,
  'roe': -38.84,
  'fcfB': -0.01525,
  'capexB': 0.000929,
  'r40': None,
  'budget': None,
  'lo52': 1.99,
  'hi52': 18.71,
  'pxd': '2026-08-07'},
 {'t': 'AXTI',
  'name': 'AXT Inc',
  'ex': 'NASDAQ',
  'g': 4,
  'price': 88.58,
  'jan2': None,
  'mcapB': 5.6549,
  'ath': 143.16,
  'fvMin': 55,
  'fvAvg': 91.6,
  'fvMax': 125,
  'an': 5,
  'peg': None,
  'ps': 34.88,
  'fpe': 43.26,
  'pe': 957.34,
  'gm': 32.18,
  'pm': 3.23,
  'revB': 0.12551,
  'revG': 45.8,
  'eps': 0.07,
  'epsG': None,
  'roi': 0.53,
  'roe': 1.01,
  'fcfB': -0.02798,
  'capexB': 0.00686,
  'r40': 23.51,
  'budget': None,
  'lo52': 1.96,
  'hi52': 143.16,
  'pxd': '2026-08-07'},
 {'t': 'CSCO',
  'name': 'Cisco Systems',
  'ex': 'NASDAQ',
  'g': 4,
  'price': 121.43,
  'jan2': None,
  'mcapB': 478.61,
  'ath': 130.37,
  'fvMin': 112,
  'fvAvg': 131.36,
  'fvMax': 150,
  'an': 26,
  'peg': 1.84,
  'ps': 7.88,
  'fpe': 25.87,
  'pe': 40.47,
  'gm': 64.3,
  'pm': 19.69,
  'revB': 60.75,
  'revG': 9.2,
  'eps': 3.0,
  'epsG': 22.5,
  'roi': 18.98,
  'roe': 25.23,
  'fcfB': 11.79,
  'capexB': 1.24,
  'r40': 28.61,
  'budget': None,
  'lo52': 65.75,
  'hi52': 130.37,
  'pxd': '2026-08-07'},
 {'t': 'CIEN',
  'name': 'Ciena',
  'ex': 'NYSE',
  'g': 4,
  'price': 412.39,
  'jan2': None,
  'mcapB': 58.38,
  'ath': 637.51,
  'fvMin': 270,
  'fvAvg': 565.71,
  'fvMax': 720,
  'an': 20,
  'peg': 0.81,
  'ps': 10.48,
  'fpe': 52.87,
  'pe': 137.12,
  'gm': 43.05,
  'pm': 7.87,
  'revB': 5.57,
  'revG': 30.6,
  'eps': 3.01,
  'epsG': 311.9,
  'roi': 17.9,
  'roe': 15.46,
  'fcfB': 0.83266,
  'capexB': 0.20011,
  'r40': 45.55,
  'budget': None,
  'lo52': 84.41,
  'hi52': 637.51,
  'pxd': '2026-08-07'},
 {'t': 'MTSI',
  'name': 'MACOM Technology',
  'ex': 'NASDAQ',
  'g': 4,
  'price': 310.82,
  'jan2': None,
  'mcapB': 23.74,
  'ath': 418.9,
  'fvMin': 300,
  'fvAvg': 401.54,
  'fvMax': 475,
  'an': 14,
  'peg': 0.98,
  'ps': 20.39,
  'fpe': 37.71,
  'pe': 98.99,
  'gm': 56.54,
  'pm': 20.7,
  'revB': 1.16,
  'revG': None,
  'eps': 3.14,
  'epsG': None,
  'roi': 14.5,
  'roe': 17.18,
  'fcfB': 0.20408,
  'capexB': 0.06711,
  'r40': None,
  'budget': None,
  'lo52': 118.16,
  'hi52': 418.9,
  'pxd': '2026-08-07'},
 {'t': 'VIAV',
  'name': 'VIAVI Solutions',
  'ex': 'NASDAQ',
  'g': 4,
  'price': 39.14,
  'jan2': None,
  'mcapB': 9.59,
  'ath': 60.43,
  'fvMin': 44,
  'fvAvg': 61.43,
  'fvMax': 70,
  'an': 8,
  'peg': None,
  'ps': 6.32,
  'fpe': 23.82,
  'pe': None,
  'gm': 61.73,
  'pm': -2.0,
  'revB': 1.52,
  'revG': 40.0,
  'eps': -0.13,
  'epsG': None,
  'roi': -16.13,
  'roe': -2.73,
  'fcfB': 0.0455,
  'capexB': 0.0255,
  'r40': 43.0,
  'budget': None,
  'lo52': 10.02,
  'hi52': 60.43,
  'pxd': '2026-08-07'},
 {'t': 'CEG',
  'name': 'Constellation Energy',
  'ex': 'NASDAQ',
  'g': 16,
  'price': 270.43,
  'jan2': 358.0,
  'mcapB': 96.5687,
  'ath': 412.7,
  'fvMin': 296,
  'fvAvg': 357.81,
  'fvMax': 441,
  'an': 23,
  'peg': 1.31,
  'ps': 3.05,
  'fpe': 21.5,
  'pe': 21.6,
  'gm': 23.26,
  'pm': 12.69,
  'revB': 29.87,
  'revG': 23.4,
  'eps': 11.71,
  'epsG': 21.3,
  'roi': 8.03,
  'roe': 16.1,
  'fcfB': 1.12,
  'capexB': 3.43,
  'r40': 27.2,
  'budget': 10000,
  'lo52': 228.63,
  'hi52': 412.7,
  'pxd': '2026-08-10'},
 {'t': 'BWXT',
  'name': 'BWX Technologies',
  'ex': 'NYSE',
  'g': 16,
  'price': 168.88,
  'jan2': 174.53,
  'mcapB': 15.6257,
  'ath': 241.82,
  'fvMin': 200,
  'fvAvg': 238.79,
  'fvMax': 290,
  'an': 15,
  'peg': 2.71,
  'ps': 4.38,
  'fpe': 33.31,
  'pe': 43.19,
  'gm': 22.08,
  'pm': 10.11,
  'revB': 3.51,
  'revG': 22.67,
  'eps': 3.86,
  'epsG': 20.63,
  'roi': 10.78,
  'roe': 28.29,
  'fcfB': 0.32,
  'capexB': 0.2,
  'r40': 32.8,
  'budget': 5000,
  'lo52': 136.18,
  'hi52': 241.82,
  'pxd': '2026-08-10'},
 {'t': 'OKLO',
  'name': 'Oklo',
  'ex': 'NYSE',
  'g': 16,
  'price': 48.42,
  'jan2': 105.31,
  'mcapB': 8.4294,
  'ath': 163.39,
  'fvMin': 14,
  'fvAvg': 86.2,
  'fvMax': 140,
  'an': 25,
  'peg': None,
  'ps': 6841.73,
  'fpe': None,
  'pe': None,
  'gm': 40.41,
  'pm': None,
  'revB': 0.00121,
  'revG': None,
  'eps': -0.93,
  'epsG': None,
  'roi': -6.84,
  'roe': -7.7,
  'fcfB': -0.27582,
  'capexB': 0.1589,
  'r40': None,
  'budget': None,
  'lo52': 36.61,
  'hi52': 193.84,
  'pxd': '2026-08-07'},
 {'t': 'SMR',
  'name': 'NuScale Power',
  'ex': 'NYSE',
  'g': 16,
  'price': 9.82,
  'jan2': 20.51,
  'mcapB': 3.6043,
  'ath': 46.77,
  'fvMin': 6,
  'fvAvg': 14.57,
  'fvMax': 25,
  'an': 18,
  'peg': None,
  'ps': 320.69,
  'fpe': None,
  'pe': None,
  'gm': 23.55,
  'pm': -3888.6,
  'revB': 0.011,
  'revG': -81.0,
  'eps': -2.92,
  'epsG': None,
  'roi': -11.21,
  'roe': -55.18,
  'fcfB': -0.78,
  'capexB': 0.0,
  'r40': None,
  'budget': None,
  'lo52': 7.21,
  'hi52': 57.42,
  'pxd': '2026-08-07'},
 {'t': 'NNE',
  'name': 'Nano Nuclear Energy',
  'ex': 'NASDAQ',
  'g': 16,
  'price': 18.85,
  'jan2': 32.01,
  'mcapB': 1.0122,
  'ath': 34.73,
  'fvMin': 22,
  'fvAvg': 41.4,
  'fvMax': 50,
  'an': 6,
  'peg': None,
  'ps': None,
  'fpe': None,
  'pe': None,
  'gm': None,
  'pm': None,
  'revB': 0.0,
  'revG': None,
  'eps': -0.69,
  'epsG': None,
  'roi': -7.8,
  'roe': -8.61,
  'fcfB': -0.04,
  'capexB': 0.01,
  'r40': None,
  'budget': None,
  'lo52': None,
  'hi52': 60.87,
  'pxd': '2026-08-07'},
 {'t': 'CCJ',
  'name': 'Cameco',
  'ex': 'NYSE',
  'g': 16,
  'price': 97.39,
  'jan2': 107.56,
  'mcapB': 42.4751,
  'ath': 124.04,
  'fvMin': 83.97,
  'fvAvg': 131.31,
  'fvMax': 168.9,
  'an': 25,
  'peg': None,
  'ps': 15.37,
  'fpe': 53.24,
  'pe': 151.07,
  'gm': 35.11,
  'pm': 10.21,
  'revB': 2.45,
  'revG': -2.66,
  'eps': 0.57,
  'epsG': -33.61,
  'roi': 5.05,
  'roe': 5.12,
  'fcfB': 0.4,
  'capexB': 0.28,
  'r40': 13.5,
  'budget': None,
  'lo52': 68.96,
  'hi52': 135.24,
  'pxd': '2026-08-07'},
 {'t': 'VST',
  'name': 'Vistra',
  'ex': 'NYSE',
  'g': 16,
  'price': 142.87,
  'jan2': 163.89,
  'mcapB': 48.173,
  'ath': 219.82,
  'fvMin': 99,
  'fvAvg': 222.89,
  'fvMax': 320,
  'an': 20,
  'peg': 0.38,
  'ps': 2.78,
  'fpe': 15.79,
  'pe': 26.3,
  'gm': 38.64,
  'pm': 11.52,
  'revB': 19.45,
  'revG': 7.4,
  'eps': 5.92,
  'epsG': -6.1,
  'roi': 9.96,
  'roe': 42.9,
  'fcfB': 1.8,
  'capexB': 2.87,
  'r40': 16.7,
  'budget': 10000,
  'lo52': 132.66,
  'hi52': 219.82,
  'pxd': '2026-08-10'},
 {'t': 'XE',
  'name': "X-energy (IPO Apr 24 '26)",
  'ex': 'NASDAQ',
  'g': 16,
  'price': 22.65,
  'jan2': 23.0,
  'mcapB': 9.2072,
  'ath': 37.1,
  'fvMin': 22,
  'fvAvg': 37.86,
  'fvMax': 57,
  'an': 8,
  'peg': None,
  'ps': 52.76,
  'fpe': None,
  'pe': None,
  'gm': -69.12,
  'pm': None,
  'revB': 0.1171,
  'revG': -3.9,
  'eps': -1.34,
  'epsG': None,
  'roi': None,
  'roe': None,
  'fcfB': -0.324,
  'capexB': 0.149,
  'r40': None,
  'budget': None,
  'lo52': 13.29,
  'hi52': 37.1,
  'pxd': '2026-08-07'},
 {'t': 'UUUU',
  'name': 'Energy Fuels',
  'ex': 'NYSE American',
  'g': 16,
  'price': 14.14,
  'jan2': None,
  'mcapB': 3.5295,
  'ath': 27.9,
  'fvMin': 16,
  'fvAvg': 26.13,
  'fvMax': 32.5,
  'an': 7,
  'peg': None,
  'ps': 30.4,
  'fpe': None,
  'pe': None,
  'gm': 40.85,
  'pm': -77.3,
  'revB': 0.10576,
  'revG': None,
  'eps': -0.34,
  'epsG': None,
  'roi': -16.12,
  'roe': -11.45,
  'fcfB': -0.12043,
  'capexB': 0.05797,
  'r40': None,
  'budget': None,
  'lo52': 8.16,
  'hi52': 27.9,
  'pxd': '2026-08-07'},
 {'t': 'LEU',
  'name': 'Centrus Energy',
  'ex': 'NYSE American',
  'g': 16,
  'price': 191.37,
  'jan2': None,
  'mcapB': 3.82,
  'ath': 464.25,
  'fvMin': 170,
  'fvAvg': 260.87,
  'fvMax': 390,
  'an': 17,
  'peg': None,
  'ps': 7.9,
  'fpe': 78.38,
  'pe': 88.06,
  'gm': 23.65,
  'pm': 10.23,
  'revB': 0.4739,
  'revG': None,
  'eps': 2.17,
  'epsG': None,
  'roi': 0.42,
  'roe': 8.05,
  'fcfB': -0.1638,
  'capexB': 0.1088,
  'r40': None,
  'budget': None,
  'lo52': 142.13,
  'hi52': 464.25,
  'pxd': '2026-08-07'},
 {'t': 'TE',
  'name': 'T1 Energy',
  'ex': 'NYSE',
  'g': 5,
  'price': 5.85,
  'jan2': 6.94,
  'mcapB': 1.6413,
  'ath': 12.49,
  'fvMin': 8,
  'fvAvg': 10.07,
  'fvMax': 16,
  'an': 7,
  'peg': None,
  'ps': 1.8,
  'fpe': None,
  'pe': None,
  'gm': 7.6,
  'pm': -42.3,
  'revB': 0.88,
  'revG': 1459.55,
  'eps': -1.88,
  'epsG': None,
  'roi': -23.54,
  'roe': -111.06,
  'fcfB': -0.04,
  'capexB': 0.11,
  'r40': None,
  'budget': None,
  'lo52': 1.15,
  'hi52': 12.49,
  'pxd': '2026-08-07'},
 {'t': 'EOSE',
  'name': 'Eos Energy Enterprises',
  'ex': 'NASDAQ',
  'g': 5,
  'price': 4.15,
  'jan2': 15.28,
  'mcapB': 1.473,
  'ath': 19.99,
  'fvMin': 5,
  'fvAvg': 8.78,
  'fvMax': 18,
  'an': 11,
  'peg': None,
  'ps': 8.19,
  'fpe': None,
  'pe': None,
  'gm': -101.88,
  'pm': -296.13,
  'revB': 0.16,
  'revG': 631.79,
  'eps': -5.51,
  'epsG': None,
  'roi': -139.71,
  'roe': None,
  'fcfB': -0.39,
  'capexB': 0.08,
  'r40': None,
  'budget': None,
  'lo52': 3.36,
  'hi52': 19.86,
  'pxd': '2026-08-07'},
 {'t': 'ENPH',
  'name': 'Enphase Energy',
  'ex': 'NASDAQ',
  'g': 5,
  'price': 41.87,
  'jan2': 35.22,
  'mcapB': 5.5319,
  'ath': 73.74,
  'fvMin': 27,
  'fvAvg': 48.28,
  'fvMax': 85,
  'an': 31,
  'peg': None,
  'ps': 3.46,
  'fpe': 18.3,
  'pe': 35.98,
  'gm': 27.18,
  'pm': 9.64,
  'revB': 1.4,
  'revG': 10.72,
  'eps': 1.02,
  'epsG': 72.0,
  'roi': 12.14,
  'roe': 14.11,
  'fcfB': 0.15,
  'capexB': 0.05,
  'r40': 20.36,
  'budget': None,
  'lo52': 25.78,
  'hi52': 73.74,
  'pxd': '2026-08-07'},
 {'t': 'AMPX',
  'name': 'Amprius Technologies',
  'ex': 'NYSE',
  'g': 5,
  'price': 11.85,
  'jan2': None,
  'mcapB': 1.7154,
  'ath': 24.23,
  'fvMin': 18,
  'fvAvg': 23.0,
  'fvMax': 30,
  'an': 10,
  'peg': None,
  'ps': 15.35,
  'fpe': None,
  'pe': None,
  'gm': 18.13,
  'pm': -43.98,
  'revB': 0.09026,
  'revG': 172.6,
  'eps': -0.31,
  'epsG': None,
  'roi': -11.79,
  'roe': -44.16,
  'fcfB': -0.05875,
  'capexB': 0.00447,
  'r40': 107.51,
  'budget': None,
  'lo52': 6.07,
  'hi52': 24.23,
  'pxd': '2026-08-07'},
 {'t': 'FSLR',
  'name': 'First Solar',
  'ex': 'NASDAQ',
  'g': 5,
  'price': 250.05,
  'jan2': None,
  'mcapB': 26.92,
  'ath': 320.95,
  'fvMin': 150,
  'fvAvg': 255.26,
  'fvMax': 330,
  'an': 35,
  'peg': 0.56,
  'ps': 5.33,
  'fpe': 11.68,
  'pe': 15.05,
  'gm': 44.02,
  'pm': 32.47,
  'revB': 5.38,
  'revG': 23.8,
  'eps': 16.22,
  'epsG': 38.6,
  'roi': 20.58,
  'roe': 18.51,
  'fcfB': 1.5,
  'capexB': 0.65562,
  'r40': 51.7,
  'budget': None,
  'lo52': 176.47,
  'hi52': 320.95,
  'pxd': '2026-08-07'},
 {'t': 'TSLA',
  'name': 'Tesla',
  'ex': 'NASDAQ',
  'g': 5,
  'price': 330.88,
  'jan2': 457.8,
  'mcapB': 1307.7403,
  'ath': 498.83,
  'fvMin': 370,
  'fvAvg': 431.36,
  'fvMax': 508,
  'an': 81,
  'peg': 3.95,
  'ps': 14.61,
  'fpe': 168.32,
  'pe': 370.36,
  'gm': 19.07,
  'pm': 3.95,
  'revB': 97.88,
  'revG': 2.3,
  'eps': 1.09,
  'epsG': -37.4,
  'roi': 6.34,
  'roe': 4.9,
  'fcfB': 7.0,
  'capexB': 9.53,
  'r40': 9.4,
  'budget': 10000,
  'lo52': 297.82,
  'hi52': 498.83,
  'pxd': '2026-08-10'},
 {'t': 'ISRG',
  'name': 'Intuitive Surgical',
  'ex': 'NASDAQ',
  'g': 6,
  'price': 393.38,
  'jan2': 566.78,
  'mcapB': 138.9682,
  'ath': 603.88,
  'fvMin': 324,
  'fvAvg': 504.15,
  'fvMax': 685,
  'an': 33,
  'peg': 1.93,
  'ps': 11.09,
  'fpe': 30.8,
  'pe': 39.61,
  'gm': 66.77,
  'pm': 28.45,
  'revB': 11.03,
  'revG': 20.7,
  'eps': 8.72,
  'epsG': 21.8,
  'roi': 25.25,
  'roe': 17.42,
  'fcfB': 2.83,
  'capexB': 0.53,
  'r40': 47.4,
  'budget': 20000,
  'lo52': 328.57,
  'hi52': 603.88,
  'pxd': '2026-08-10'},
 {'t': 'CGNX',
  'name': 'Cognex',
  'ex': 'NASDAQ',
  'g': 6,
  'price': 64.36,
  'jan2': 36.43,
  'mcapB': 10.7171,
  'ath': 72.88,
  'fvMin': 62,
  'fvAvg': 77.6,
  'fvMax': 94,
  'an': 21,
  'peg': None,
  'ps': 10.63,
  'fpe': 45.41,
  'pe': 68.0,
  'gm': 68.87,
  'pm': 16.05,
  'revB': 1.089,
  'revG': 17.14,
  'eps': 1.03,
  'epsG': 45.07,
  'roi': 12.65,
  'roe': 11.15,
  'fcfB': 0.24,
  'capexB': 0.01,
  'r40': 39.3,
  'budget': 10000,
  'lo52': 33.45,
  'hi52': 72.88,
  'pxd': '2026-08-10'},
 {'t': 'TER',
  'name': 'Teradyne',
  'ex': 'NASDAQ',
  'g': 6,
  'price': 365.1,
  'jan2': 199.29,
  'mcapB': 57.4998,
  'ath': 487.91,
  'fvMin': 270,
  'fvAvg': 423.41,
  'fvMax': 550,
  'an': 18,
  'peg': 1.4,
  'ps': 13.33,
  'fpe': 46.58,
  'pe': 59.86,
  'gm': 58.7,
  'pm': 22.55,
  'revB': 3.79,
  'revG': 30.3,
  'eps': 5.39,
  'epsG': 52.5,
  'roi': 31.29,
  'roe': 28.75,
  'fcfB': 0.55,
  'capexB': 0.22,
  'r40': 44.9,
  'budget': 10000,
  'lo52': 102.42,
  'hi52': 487.91,
  'pxd': '2026-08-10'},
 {'t': 'OUST',
  'name': 'Ouster',
  'ex': 'NASDAQ',
  'g': 6,
  'price': 43.35,
  'jan2': 22.29,
  'mcapB': 2.9227,
  'ath': 46.05,
  'fvMin': 33,
  'fvAvg': 52.14,
  'fvMax': 75,
  'an': 7,
  'peg': None,
  'ps': 14.97,
  'fpe': None,
  'pe': None,
  'gm': 47.69,
  'pm': -26.0,
  'revB': 0.205,
  'revG': 30.0,
  'eps': -0.87,
  'epsG': None,
  'roi': -13.87,
  'roe': -18.01,
  'fcfB': -0.0825,
  'capexB': 0.0269,
  'r40': -7.4,
  'budget': None,
  'lo52': 16.4,
  'hi52': 49.39,
  'pxd': '2026-08-07'},
 {'t': 'ROK',
  'name': 'Rockwell Automation',
  'ex': 'NYSE',
  'g': 6,
  'price': 441.04,
  'jan2': 392.0,
  'mcapB': 49.0757,
  'ath': 497.36,
  'fvMin': 282,
  'fvAvg': 473.61,
  'fvMax': 555,
  'an': 28,
  'peg': 2.3,
  'ps': 5.79,
  'fpe': 33.21,
  'pe': 47.58,
  'gm': 48.92,
  'pm': 12.36,
  'revB': 8.8,
  'revG': 7.5,
  'eps': 9.63,
  'epsG': None,
  'roi': 19.07,
  'roe': 27.18,
  'fcfB': 1.34,
  'capexB': 0.196,
  'r40': 22.7,
  'budget': None,
  'lo52': 305.44,
  'hi52': 480.98,
  'pxd': '2026-08-07'},
 {'t': 'RKLB',
  'name': 'Rocket Lab',
  'ex': 'NASDAQ',
  'g': 7,
  'price': 80.04,
  'jan2': 70.63,
  'mcapB': 47.8785,
  'ath': 151.0,
  'fvMin': 77,
  'fvAvg': 116.57,
  'fvMax': 150,
  'an': 16,
  'peg': None,
  'ps': 64.42,
  'fpe': None,
  'pe': None,
  'gm': 37.27,
  'pm': -21.52,
  'revB': 0.77,
  'revG': 45.8,
  'eps': -0.28,
  'epsG': None,
  'roi': -8.06,
  'roe': -13.55,
  'fcfB': -0.32,
  'capexB': 0.15,
  'r40': -0.7,
  'budget': 10000,
  'lo52': 37.57,
  'hi52': 151.0,
  'pxd': '2026-08-10'},
 {'t': 'ASTS',
  'name': 'AST SpaceMobile',
  'ex': 'NASDAQ',
  'g': 7,
  'price': 71.94,
  'jan2': 74.34,
  'mcapB': 27.9175,
  'ath': 133.86,
  'fvMin': 41.2,
  'fvAvg': 83.32,
  'fvMax': 108,
  'an': 12,
  'peg': None,
  'ps': 242.26,
  'fpe': None,
  'pe': None,
  'gm': 36.42,
  'pm': None,
  'revB': 0.12,
  'revG': 1732.1,
  'eps': -2.15,
  'epsG': None,
  'roi': -6.78,
  'roe': -37.75,
  'fcfB': -1.3,
  'capexB': 1.21,
  'r40': None,
  'budget': None,
  'lo52': 22.47,
  'hi52': 129.89,
  'pxd': '2026-08-07'},
 {'t': 'LUNR',
  'name': 'Intuitive Machines',
  'ex': 'NASDAQ',
  'g': 7,
  'price': 16.4,
  'jan2': 18.56,
  'mcapB': 3.5617,
  'ath': 46.75,
  'fvMin': 11,
  'fvAvg': 40.78,
  'fvMax': 75,
  'an': 9,
  'peg': None,
  'ps': 8.39,
  'fpe': None,
  'pe': None,
  'gm': 9.7,
  'pm': -32.69,
  'revB': 0.33,
  'revG': 53.8,
  'eps': -0.87,
  'epsG': None,
  'roi': -20.33,
  'roe': -30.29,
  'fcfB': -0.13,
  'capexB': 0.05,
  'r40': 13.75,
  'budget': None,
  'lo52': 7.78,
  'hi52': 46.75,
  'pxd': '2026-08-07'},
 {'t': 'SPCX',
  'name': 'SpaceX',
  'ex': 'NASDAQ',
  'g': 7,
  'price': 133.11,
  'jan2': None,
  'mcapB': 1755.4063,
  'ath': 225.64,
  'fvMin': 62,
  'fvAvg': 240.04,
  'fvMax': 800,
  'an': 33,
  'peg': 0.29,
  'ps': 76.14,
  'fpe': 47.74,
  'pe': None,
  'gm': 51.87,
  'pm': -35.66,
  'revB': 23.04,
  'revG': 33.2,
  'eps': -2.27,
  'epsG': None,
  'roi': None,
  'roe': None,
  'fcfB': -32.52,
  'capexB': 42.42,
  'r40': None,
  'budget': None,
  'lo52': 108.27,
  'hi52': 225.64,
  'pxd': '2026-08-07'},
 {'t': 'PL',
  'name': 'Planet Labs',
  'ex': 'NYSE',
  'g': 7,
  'price': 23.93,
  'jan2': 20.41,
  'mcapB': 8.5322,
  'ath': 51.78,
  'fvMin': 25.0,
  'fvAvg': 40.1,
  'fvMax': 53.0,
  'an': 11,
  'peg': None,
  'ps': 24.24,
  'fpe': None,
  'pe': None,
  'gm': 55.58,
  'pm': -111.17,
  'revB': 0.33561,
  'revG': 34.15,
  'eps': -1.17,
  'epsG': None,
  'roi': -8.03,
  'roe': -83.98,
  'fcfB': 0.04655,
  'capexB': -0.08591,
  'r40': 48.02,
  'budget': None,
  'lo52': 6.1,
  'hi52': 51.76,
  'pxd': '2026-08-07'},
 {'t': 'SIDU',
  'name': 'Sidus Space',
  'ex': 'NASDAQ',
  'g': 7,
  'price': 2.24,
  'jan2': None,
  'mcapB': 0.23,
  'ath': 6.79,
  'fvMin': None,
  'fvAvg': None,
  'fvMax': None,
  'an': 1,
  'peg': None,
  'ps': 64.26,
  'fpe': None,
  'pe': None,
  'gm': -145.92,
  'pm': None,
  'revB': 0.0035,
  'revG': -9.2,
  'eps': -0.77,
  'epsG': None,
  'roi': -36.39,
  'roe': -84.25,
  'fcfB': -0.02947,
  'capexB': 0.00888,
  'r40': None,
  'budget': None,
  'lo52': 0.628,
  'hi52': 6.79,
  'pxd': '2026-08-07'},
 {'t': 'PLTR',
  'name': 'Palantir',
  'ex': 'NASDAQ',
  'g': 8,
  'price': 175.23,
  'jan2': 181.3,
  'mcapB': 420.0779,
  'ath': 207.52,
  'fvMin': 138,
  'fvAvg': 185.36,
  'fvMax': 230,
  'an': 26,
  'peg': 1.62,
  'ps': 48.96,
  'fpe': 83.32,
  'pe': 107.39,
  'gm': 84.07,
  'pm': 49.0,
  'revB': 6.15,
  'revG': 78.8,
  'eps': 1.17,
  'epsG': 290.0,
  'roi': 306.8,
  'roe': 32.59,
  'fcfB': 3.34,
  'capexB': 0.04,
  'r40': 133.1,
  'budget': 10000,
  'lo52': 106.37,
  'hi52': 207.52,
  'pxd': '2026-08-10'},
 {'t': 'AVAV',
  'name': 'AeroVironment',
  'ex': 'NASDAQ',
  'g': 8,
  'price': 189.15,
  'jan2': 248.45,
  'mcapB': 9.53,
  'ath': 417.86,
  'fvMin': 166,
  'fvAvg': 231.68,
  'fvMax': 326,
  'an': 20,
  'peg': 1.6,
  'ps': 3.63,
  'fpe': 43.95,
  'pe': None,
  'gm': 25.33,
  'pm': -13.41,
  'revB': 1.98,
  'revG': 140.9,
  'eps': -5.4,
  'epsG': None,
  'roi': -1.55,
  'roe': -10.03,
  'fcfB': -0.14,
  'capexB': 0.06,
  'r40': 133.8,
  'budget': 5000,
  'lo52': 135.2,
  'hi52': 417.86,
  'pxd': '2026-08-10'},
 {'t': 'ONDS',
  'name': 'Ondas Holdings',
  'ex': 'NASDAQ',
  'g': 8,
  'price': 9.31,
  'jan2': 10.14,
  'mcapB': 5.3055,
  'ath': 15.28,
  'fvMin': 16,
  'fvAvg': 19.81,
  'fvMax': 25,
  'an': 8,
  'peg': None,
  'ps': 38.49,
  'fpe': None,
  'pe': 14.92,
  'gm': 44.85,
  'pm': 251.92,
  'revB': 0.1,
  'revG': 793.2,
  'eps': 0.44,
  'epsG': None,
  'roi': -9.11,
  'roe': 42.95,
  'fcfB': -0.09,
  'capexB': 0.003,
  'r40': 703.6,
  'budget': None,
  'lo52': 0.83,
  'hi52': 15.28,
  'pxd': '2026-08-10'},
 {'t': 'AXON',
  'name': 'Axon Enterprise',
  'ex': 'NASDAQ',
  'g': 8,
  'price': 571.01,
  'jan2': None,
  'mcapB': 46.0231,
  'ath': 885.0,
  'fvMin': 409.68,
  'fvAvg': 683.4,
  'fvMax': 830,
  'an': 21,
  'peg': 2.31,
  'ps': 13.04,
  'fpe': 58.25,
  'pe': 217.29,
  'gm': 59.53,
  'pm': 6.19,
  'revB': 3.22,
  'revG': None,
  'eps': 2.4,
  'epsG': -40.5,
  'roi': 1.18,
  'roe': 6.22,
  'fcfB': 0.133,
  'capexB': 0.1326,
  'r40': None,
  'budget': None,
  'lo52': 339.01,
  'hi52': 885.0,
  'pxd': '2026-08-07'},
 {'t': 'RCAT',
  'name': 'Red Cat Holdings',
  'ex': 'NASDAQ',
  'g': 8,
  'price': 9.21,
  'jan2': None,
  'mcapB': 1.4,
  'ath': 18.78,
  'fvMin': 20,
  'fvAvg': 22,
  'fvMax': 25,
  'an': 6,
  'peg': None,
  'ps': 19.59,
  'fpe': None,
  'pe': None,
  'gm': 9.75,
  'pm': -136.26,
  'revB': 0.07154,
  'revG': None,
  'eps': -0.81,
  'epsG': None,
  'roi': -22.25,
  'roe': -34.4,
  'fcfB': -0.1577,
  'capexB': 0.01862,
  'r40': None,
  'budget': None,
  'lo52': 5.77,
  'hi52': 18.78,
  'pxd': '2026-08-07'},
 {'t': 'LMT',
  'name': 'Lockheed Martin',
  'ex': 'NYSE',
  'g': 8,
  'price': 587.95,
  'jan2': None,
  'mcapB': 135.69,
  'ath': 692.0,
  'fvMin': 503,
  'fvAvg': 629.53,
  'fvMax': 756,
  'an': 21,
  'peg': 0.99,
  'ps': 1.76,
  'fpe': 18.99,
  'pe': 21.66,
  'gm': 11.8,
  'pm': 8.16,
  'revB': 77.01,
  'revG': 7.2,
  'eps': 27.14,
  'epsG': 52.5,
  'roi': 27.42,
  'roe': 89.16,
  'fcfB': 8.73,
  'capexB': 1.67,
  'r40': 18.5,
  'budget': None,
  'lo52': 423.91,
  'hi52': 692.0,
  'pxd': '2026-08-07'},
 {'t': 'RTX',
  'name': 'RTX Corp',
  'ex': 'NYSE',
  'g': 8,
  'price': 223.03,
  'jan2': None,
  'mcapB': 300.59,
  'ath': 225.65,
  'fvMin': 180,
  'fvAvg': 230.68,
  'fvMax': 265,
  'an': 23,
  'peg': 2.78,
  'ps': 3.21,
  'fpe': 30.24,
  'pe': 39.26,
  'gm': 20.35,
  'pm': 8.28,
  'revB': 93.5,
  'revG': 11.8,
  'eps': 5.68,
  'epsG': 24.9,
  'roi': 9.4,
  'roe': 12.27,
  'fcfB': 11.41,
  'capexB': 2.8,
  'r40': 24.0,
  'budget': None,
  'lo52': 150.61,
  'hi52': 225.65,
  'pxd': '2026-08-07'},
 {'t': 'NOC',
  'name': 'Northrop Grumman',
  'ex': 'NYSE',
  'g': 8,
  'price': 571.58,
  'jan2': None,
  'mcapB': 81.2,
  'ath': 774.0,
  'fvMin': 538,
  'fvAvg': 643.28,
  'fvMax': 815,
  'an': 23,
  'peg': 3.28,
  'ps': 1.89,
  'fpe': 19.56,
  'pe': 18.15,
  'gm': 20.06,
  'pm': 10.48,
  'revB': 42.89,
  'revG': None,
  'eps': 31.5,
  'epsG': None,
  'roi': 15.02,
  'roe': 26.96,
  'fcfB': 3.65,
  'capexB': 1.43,
  'r40': None,
  'budget': None,
  'lo52': 479.02,
  'hi52': 774.0,
  'pxd': '2026-08-07'},
 {'t': 'IONQ',
  'name': 'IonQ',
  'ex': 'NYSE',
  'g': 9,
  'price': 44.43,
  'jan2': 46.01,
  'mcapB': 16.5826,
  'ath': 84.64,
  'fvMin': 44.78,
  'fvAvg': 68.79,
  'fvMax': 100,
  'an': 13,
  'peg': None,
  'ps': 69.38,
  'fpe': None,
  'pe': 37.75,
  'gm': 36.11,
  'pm': 174.88,
  'revB': 0.19,
  'revG': 334.6,
  'eps': 0.92,
  'epsG': None,
  'roi': -16.68,
  'roe': 11.29,
  'fcfB': -0.42,
  'capexB': 0.02,
  'r40': 108.2,
  'budget': None,
  'lo52': 25.89,
  'hi52': 84.64,
  'pxd': '2026-08-07'},
 {'t': 'ORCL',
  'name': 'Oracle',
  'ex': 'NYSE',
  'g': 10,
  'price': 151.05,
  'jan2': 193.84,
  'mcapB': 435.1006,
  'ath': 345.72,
  'fvMin': 155,
  'fvAvg': 251.85,
  'fvMax': 400,
  'an': 43,
  'peg': 0.55,
  'ps': 5.41,
  'fpe': 15.76,
  'pe': 21.68,
  'gm': 65.82,
  'pm': 25.37,
  'revB': 67.36,
  'revG': 17.4,
  'eps': 5.83,
  'epsG': 34.3,
  'roi': 11.48,
  'roe': 53.38,
  'fcfB': -23.69,
  'capexB': 55.66,
  'r40': -17.8,
  'budget': 5000,
  'lo52': 114.75,
  'hi52': 345.72,
  'pxd': '2026-08-10'},
 {'t': 'ZETA',
  'name': 'Zeta Global',
  'ex': 'NYSE',
  'g': 10,
  'price': 27.54,
  'jan2': 20.54,
  'mcapB': 6.8606,
  'ath': 28.5,
  'fvMin': 22,
  'fvAvg': 28.68,
  'fvMax': 44,
  'an': 14,
  'peg': 0.73,
  'ps': 3.85,
  'fpe': 26.09,
  'pe': None,
  'gm': 60.14,
  'pm': -0.14,
  'revB': 1.57,
  'revG': 35.7,
  'eps': -0.01,
  'epsG': None,
  'roi': 1.98,
  'roe': -0.24,
  'fcfB': 0.22,
  'capexB': 0.01,
  'r40': 50.0,
  'budget': None,
  'lo52': 14.37,
  'hi52': 28.5,
  'pxd': '2026-08-10'},
 {'t': 'CDNS',
  'name': 'Cadence Design Systems',
  'ex': 'NASDAQ',
  'g': 10,
  'price': 331.91,
  'jan2': 315.7,
  'mcapB': 91.5209,
  'ath': 416.69,
  'fvMin': 275,
  'fvAvg': 394.79,
  'fvMax': 470,
  'an': 26,
  'peg': 2.81,
  'ps': 17.22,
  'fpe': 40.51,
  'pe': 76.9,
  'gm': 86.08,
  'pm': 21.18,
  'revB': 5.53,
  'revG': 13.4,
  'eps': 4.29,
  'epsG': 8.9,
  'roi': 19.7,
  'roe': 20.66,
  'fcfB': 1.43,
  'capexB': 0.17,
  'r40': 39.3,
  'budget': None,
  'lo52': 262.75,
  'hi52': 416.69,
  'pxd': '2026-08-10'},
 {'t': 'CRM',
  'name': 'Salesforce',
  'ex': 'NYSE',
  'g': 10,
  'price': 197.51,
  'jan2': 265.0,
  'mcapB': 161.7575,
  'ath': 274.0,
  'fvMin': 160,
  'fvAvg': 245.41,
  'fvMax': 475,
  'an': 52,
  'peg': 0.76,
  'ps': 3.27,
  'fpe': 12.26,
  'pe': 19.82,
  'gm': 77.64,
  'pm': 18.73,
  'revB': 42.83,
  'revG': 11.0,
  'eps': 8.61,
  'epsG': 34.8,
  'roi': 11.07,
  'roe': 16.91,
  'fcfB': 14.66,
  'capexB': 0.56,
  'r40': 45.2,
  'budget': None,
  'lo52': 146.32,
  'hi52': 274.0,
  'pxd': '2026-08-10'},
 {'t': 'IBM',
  'name': 'IBM',
  'ex': 'NYSE',
  'g': 9,
  'price': 236.31,
  'jan2': 297.56,
  'mcapB': 222.6416,
  'ath': 332.46,
  'fvMin': 175,
  'fvAvg': 273.75,
  'fvMax': 350,
  'an': 24,
  'peg': 2.6,
  'ps': 2.9,
  'fpe': 17.6,
  'pe': 18.84,
  'gm': 58.36,
  'pm': 15.61,
  'revB': 68.91,
  'revG': 9.7,
  'eps': 11.31,
  'epsG': 94.2,
  'roi': 14.97,
  'roe': 35.77,
  'fcfB': 12.91,
  'capexB': 1.08,
  'r40': 28.4,
  'budget': 5000,
  'lo52': 199.19,
  'hi52': 332.46,
  'pxd': '2026-08-10'},
 {'t': 'ADBE',
  'name': 'Adobe',
  'ex': 'NASDAQ',
  'g': 10,
  'price': 265.21,
  'jan2': 333.3,
  'mcapB': 105.434,
  'ath': 688.37,
  'fvMin': 190,
  'fvAvg': 269.61,
  'fvMax': 380,
  'an': 40,
  'peg': 0.66,
  'ps': 3.75,
  'fpe': 9.17,
  'pe': 13.6,
  'gm': 89.4,
  'pm': 28.69,
  'revB': 25.2,
  'revG': 11.5,
  'eps': 17.48,
  'epsG': 11.84,
  'roi': 60.74,
  'roe': 62.95,
  'fcfB': 10.28,
  'capexB': 0.201,
  'r40': 52.3,
  'budget': None,
  'lo52': 190.12,
  'hi52': 376.16,
  'pxd': '2026-08-07'},
 {'t': 'NOW',
  'name': 'ServiceNow',
  'ex': 'NYSE',
  'g': 10,
  'price': 124.88,
  'jan2': 147.45,
  'mcapB': 129.1013,
  'ath': 234.08,
  'fvMin': 72,
  'fvAvg': 140.25,
  'fvMax': 248,
  'an': 49,
  'peg': 0.95,
  'ps': 7.41,
  'fpe': 23.3,
  'pe': 65.95,
  'gm': 74.77,
  'pm': 11.34,
  'revB': 14.73,
  'revG': 22.2,
  'eps': 1.6,
  'epsG': 0.38,
  'roi': 10.28,
  'roe': 14.24,
  'fcfB': 4.58,
  'capexB': 0.728,
  'r40': 53.3,
  'budget': None,
  'lo52': 81.24,
  'hi52': 210.2,
  'pxd': '2026-08-07'},
 {'t': 'SNOW',
  'name': 'Snowflake',
  'ex': 'NYSE',
  'g': 10,
  'price': 330.37,
  'jan2': 216.71,
  'mcapB': 114.5064,
  'ath': 401.89,
  'fvMin': 110,
  'fvAvg': 297.6,
  'fvMax': 500,
  'an': 51,
  'peg': 4.13,
  'ps': 18.8,
  'fpe': 129.11,
  'pe': None,
  'gm': 67.15,
  'pm': -23.79,
  'revB': 5.03,
  'revG': 31.1,
  'eps': -3.51,
  'epsG': None,
  'roi': -16.48,
  'roe': -54.87,
  'fcfB': 1.17,
  'capexB': 0.067,
  'r40': 54.3,
  'budget': None,
  'lo52': 118.3,
  'hi52': 330.37,
  'pxd': '2026-08-07'},
 {'t': 'DDOG',
  'name': 'Datadog',
  'ex': 'NASDAQ',
  'g': 10,
  'price': 233.93,
  'jan2': 133.77,
  'mcapB': 83.2518,
  'ath': 292.72,
  'fvMin': 139,
  'fvAvg': 265.53,
  'fvMax': 330,
  'an': 47,
  'peg': 3.22,
  'ps': 20.58,
  'fpe': 87.34,
  'pe': 462.27,
  'gm': 79.53,
  'pm': 4.48,
  'revB': 3.97,
  'revG': 31.52,
  'eps': 0.5,
  'epsG': 42.86,
  'roi': 2.71,
  'roe': 4.7,
  'fcfB': 1.18,
  'capexB': 0.047,
  'r40': 61.3,
  'budget': None,
  'lo52': 98.01,
  'hi52': 292.72,
  'pxd': '2026-08-07'},
 {'t': 'MDB',
  'name': 'MongoDB',
  'ex': 'NASDAQ',
  'g': 10,
  'price': 396.71,
  'jan2': 422.5,
  'mcapB': 31.9167,
  'ath': 444.72,
  'fvMin': 272.64,
  'fvAvg': 396.21,
  'fvMax': 545,
  'an': 40,
  'peg': 3.95,
  'ps': 9.43,
  'fpe': 48.21,
  'pe': None,
  'gm': 71.97,
  'pm': -1.12,
  'revB': 2.6,
  'revG': 19.0,
  'eps': -0.37,
  'epsG': None,
  'roi': -2.19,
  'roe': -0.97,
  'fcfB': 0.591,
  'capexB': 0.0057,
  'r40': 41.7,
  'budget': None,
  'lo52': 196.0,
  'hi52': 444.72,
  'pxd': '2026-08-07'},
 {'t': 'APP',
  'name': 'AppLovin',
  'ex': 'NASDAQ',
  'g': 10,
  'price': 346.8,
  'jan2': 618.32,
  'mcapB': 116.5079,
  'ath': 745.61,
  'fvMin': 406,
  'fvAvg': 656.2,
  'fvMax': 860,
  'an': 32,
  'peg': 0.64,
  'ps': 22.14,
  'fpe': 23.44,
  'pe': 35.32,
  'gm': 88.37,
  'pm': 64.29,
  'revB': 6.16,
  'revG': 66.4,
  'eps': 11.64,
  'epsG': 110.4,
  'roi': 119.64,
  'roe': 266.44,
  'fcfB': 4.44,
  'capexB': None,
  'r40': 138.5,
  'budget': None,
  'lo52': 346.8,
  'hi52': 745.61,
  'pxd': '2026-08-07'},
 {'t': 'VEEV',
  'name': 'Veeva Systems',
  'ex': 'NYSE',
  'g': 10,
  'price': 230.47,
  'jan2': None,
  'mcapB': 37.4348,
  'ath': 341.0,
  'fvMin': 165,
  'fvAvg': 244.14,
  'fvMax': 350,
  'an': 30,
  'peg': 1.08,
  'ps': 10.45,
  'fpe': 23.11,
  'pe': 37.86,
  'gm': 75.02,
  'pm': 28.37,
  'revB': 3.32,
  'revG': 16.25,
  'eps': 5.64,
  'epsG': 20,
  'roi': 31.44,
  'roe': 13.93,
  'fcfB': 1.64,
  'capexB': 0.02497,
  'r40': 65.65,
  'budget': None,
  'lo52': 148.05,
  'hi52': 310.5,
  'pxd': '2026-08-07'},
 {'t': 'TEAM',
  'name': 'Atlassian',
  'ex': 'NASDAQ',
  'g': 10,
  'price': 149.07,
  'jan2': None,
  'mcapB': 37.8324,
  'ath': 189.69,
  'fvMin': 95,
  'fvAvg': 140.33,
  'fvMax': 480,
  'an': 32,
  'peg': 0.87,
  'ps': 4.25,
  'fpe': 18.03,
  'pe': None,
  'gm': 84.83,
  'pm': -0.82,
  'revB': 6.57,
  'revG': 26.0,
  'eps': -0.21,
  'epsG': None,
  'roi': -2.48,
  'roe': -4.48,
  'fcfB': 1.32,
  'capexB': 0.0341,
  'r40': 46.0,
  'budget': None,
  'lo52': 56.01,
  'hi52': 189.69,
  'pxd': '2026-08-07'},
 {'t': 'SBUX',
  'name': 'Starbucks',
  'ex': 'NASDAQ',
  'g': 11,
  'price': 105.58,
  'jan2': 84.22,
  'mcapB': 120.5637,
  'ath': 109.23,
  'fvMin': 97,
  'fvAvg': 112.27,
  'fvMax': 120,
  'an': 59,
  'peg': 2.22,
  'ps': 3.1,
  'fpe': 34.09,
  'pe': 59.85,
  'gm': 22.1,
  'pm': 3.77,
  'revB': 38.34,
  'revG': -1.4,
  'eps': 1.27,
  'epsG': 70.0,
  'roi': 9.05,
  'roe': -24.31,
  'fcfB': 3.64,
  'capexB': 1.34,
  'r40': 8.1,
  'budget': None,
  'lo52': 77.99,
  'hi52': 109.23,
  'pxd': '2026-08-07'},
 {'t': 'MCD',
  'name': "McDonald's",
  'ex': 'NYSE',
  'g': 11,
  'price': 274.48,
  'jan2': 305.29,
  'mcapB': 195.0222,
  'ath': 341.75,
  'fvMin': 250,
  'fvAvg': 327.29,
  'fvMax': 407,
  'an': 34,
  'peg': 3.05,
  'ps': 6.88,
  'fpe': 20.31,
  'pe': 21.8,
  'gm': 57.35,
  'pm': 31.73,
  'revB': 27.71,
  'revG': 6.3,
  'eps': 12.31,
  'epsG': 5.5,
  'roi': 17.76,
  'roe': None,
  'fcfB': 7.04,
  'capexB': 3.5,
  'r40': 31.7,
  'budget': None,
  'lo52': 260.96,
  'hi52': 341.75,
  'pxd': '2026-08-07'},
 {'t': 'KO',
  'name': 'Coca-Cola',
  'ex': 'NYSE',
  'g': 11,
  'price': 87.05,
  'jan2': 69.85,
  'mcapB': 374.5275,
  'ath': 81.92,
  'fvMin': 86,
  'fvAvg': 95.64,
  'fvMax': 104,
  'an': 48,
  'peg': 1.51,
  'ps': 7.65,
  'fpe': 25.26,
  'pe': 26.75,
  'gm': 61.89,
  'pm': 28.56,
  'revB': 49.28,
  'revG': 5.0,
  'eps': 3.18,
  'epsG': None,
  'roi': 13.91,
  'roe': 42.99,
  'fcfB': 12.56,
  'capexB': 2.07,
  'r40': 30.5,
  'budget': None,
  'lo52': 65.35,
  'hi52': 87.59,
  'pxd': '2026-08-07'},
 {'t': 'YUM',
  'name': 'Yum! Brands',
  'ex': 'NYSE',
  'g': 11,
  'price': 150.76,
  'jan2': 155.52,
  'mcapB': 41.5502,
  'ath': 170.14,
  'fvMin': 147,
  'fvAvg': 174.33,
  'fvMax': 200,
  'an': 26,
  'peg': 1.98,
  'ps': 4.96,
  'fpe': 23.45,
  'pe': 24.19,
  'gm': 45.31,
  'pm': 20.85,
  'revB': 8.72,
  'revG': 10.29,
  'eps': 6.49,
  'epsG': 14.0,
  'roi': 59.37,
  'roe': None,
  'fcfB': 1.68,
  'capexB': 0.4,
  'r40': 29.6,
  'budget': None,
  'lo52': 137.33,
  'hi52': 169.39,
  'pxd': '2026-08-07'},
 {'t': 'UNH',
  'name': 'UnitedHealth',
  'ex': 'NYSE',
  'g': 12,
  'price': 407.08,
  'jan2': 330.9,
  'mcapB': 369.6905,
  'ath': 461.62,
  'fvMin': 373,
  'fvAvg': 469.21,
  'fvMax': 529,
  'an': 52,
  'peg': 1.47,
  'ps': 0.85,
  'fpe': 19.99,
  'pe': 27.14,
  'gm': 19.51,
  'pm': 3.14,
  'revB': 450.13,
  'revG': 2.8,
  'eps': 15.53,
  'epsG': 16.2,
  'roi': 10.23,
  'roe': 14.15,
  'fcfB': 23.62,
  'capexB': 3.4,
  'r40': 8.0,
  'budget': None,
  'lo52': 234.6,
  'hi52': 461.62,
  'pxd': '2026-08-07'},
 {'t': 'JNJ',
  'name': 'Johnson & Johnson',
  'ex': 'NYSE',
  'g': 12,
  'price': 259.24,
  'jan2': 206.82,
  'mcapB': 623.9347,
  'ath': 259.27,
  'fvMin': 220,
  'fvAvg': 272.69,
  'fvMax': 305,
  'an': 40,
  'peg': 2.45,
  'ps': 6.37,
  'fpe': 21.36,
  'pe': 30.05,
  'gm': 68.1,
  'pm': 21.48,
  'revB': 97.93,
  'revG': 4.5,
  'eps': 8.63,
  'epsG': None,
  'roi': 19.79,
  'roe': None,
  'fcfB': 18.0,
  'capexB': 4.5,
  'r40': 22.9,
  'budget': None,
  'lo52': 162.78,
  'hi52': 269.43,
  'pxd': '2026-08-07'},
 {'t': 'TEM',
  'name': 'Tempus AI',
  'ex': 'NASDAQ',
  'g': 12,
  'price': 52.05,
  'jan2': 66.27,
  'mcapB': 9.3736,
  'ath': 104.32,
  'fvMin': 35,
  'fvAvg': 66.57,
  'fvMax': 100,
  'an': 16,
  'peg': None,
  'ps': 5.52,
  'fpe': None,
  'pe': None,
  'gm': 63.98,
  'pm': -17.77,
  'revB': 1.43,
  'revG': 50.44,
  'eps': -1.43,
  'epsG': None,
  'roi': -11.55,
  'roe': -67.44,
  'fcfB': -0.26,
  'capexB': 0.03,
  'r40': 32.0,
  'budget': None,
  'lo52': 41.73,
  'hi52': 104.32,
  'pxd': '2026-08-07'},
 {'t': 'HNGE',
  'name': 'Hinge Health',
  'ex': 'NYSE',
  'g': 12,
  'price': 89.33,
  'jan2': 45.56,
  'mcapB': 6.9849,
  'ath': 90.0,
  'fvMin': 65.0,
  'fvAvg': 92.67,
  'fvMax': 125.0,
  'an': 16,
  'peg': None,
  'ps': 8.64,
  'fpe': 28.79,
  'pe': 60.0,
  'gm': 84.54,
  'pm': 15.15,
  'revB': 0.72,
  'revG': 49.78,
  'eps': 1.34,
  'epsG': None,
  'roi': 15.64,
  'roe': 30.38,
  'fcfB': 0.29012,
  'capexB': 0.00067,
  'r40': 88.1,
  'budget': None,
  'lo52': 30.08,
  'hi52': 91.5,
  'pxd': '2026-08-07'},
 {'t': 'TMDX',
  'name': 'TransMedics Group',
  'ex': 'NASDAQ',
  'g': 12,
  'price': 83.89,
  'jan2': None,
  'mcapB': 2.9075,
  'ath': 156.0,
  'fvMin': 73,
  'fvAvg': 98.8,
  'fvMax': 125,
  'an': 12,
  'peg': None,
  'ps': 4.06,
  'fpe': 39.55,
  'pe': 19.63,
  'gm': 58.69,
  'pm': 22.69,
  'revB': 0.66847,
  'revG': None,
  'eps': 4.01,
  'epsG': None,
  'roi': 9.02,
  'roe': 36.28,
  'fcfB': 0.079,
  'capexB': 0.06689,
  'r40': None,
  'budget': None,
  'lo52': 60.11,
  'hi52': 156.0,
  'pxd': '2026-08-07'},
 {'t': 'CRSP',
  'name': 'CRISPR Therapeutics',
  'ex': 'NASDAQ',
  'g': 12,
  'price': 53.52,
  'jan2': None,
  'mcapB': 5.1717,
  'ath': 78.48,
  'fvMin': 44,
  'fvAvg': 87.22,
  'fvMax': 291,
  'an': 22,
  'peg': None,
  'ps': 372.85,
  'fpe': None,
  'pe': None,
  'gm': None,
  'pm': None,
  'revB': 0.01339,
  'revG': -64.8,
  'eps': -4.76,
  'epsG': None,
  'roi': -325.94,
  'roe': -26.09,
  'fcfB': -0.37133,
  'capexB': 0.00174,
  'r40': None,
  'budget': None,
  'lo52': 44.12,
  'hi52': 78.48,
  'pxd': '2026-08-07'},
 {'t': 'IREN',
  'name': 'IREN (Iris Energy)',
  'ex': 'NASDAQ',
  'g': 13,
  'price': 38.74,
  'jan2': 39.01,
  'mcapB': 13.7028,
  'ath': 76.87,
  'fvMin': 41,
  'fvAvg': 80.93,
  'fvMax': 126,
  'an': 16,
  'peg': None,
  'ps': 15.87,
  'fpe': None,
  'pe': 61.89,
  'gm': 68.4,
  'pm': 20.88,
  'revB': 0.76,
  'revG': 104.8,
  'eps': 0.54,
  'epsG': None,
  'roi': -5.92,
  'roe': 7.73,
  'fcfB': -2.32,
  'capexB': 2.71,
  'r40': -201.6,
  'budget': None,
  'lo52': 14.72,
  'hi52': 76.87,
  'pxd': '2026-08-10'},
 {'t': 'NBIS',
  'name': 'Nebius Group',
  'ex': 'NASDAQ',
  'g': 13,
  'price': 187.97,
  'jan2': 86.99,
  'mcapB': 48.1287,
  'ath': 299.86,
  'fvMin': 120,
  'fvAvg': 244.21,
  'fvMax': 380,
  'an': 16,
  'peg': None,
  'ps': 51.83,
  'fpe': None,
  'pe': 55.52,
  'gm': 72.06,
  'pm': 93.09,
  'revB': 0.88,
  'revG': 574.9,
  'eps': 3.2,
  'epsG': None,
  'roi': -9.33,
  'roe': 14.14,
  'fcfB': -3.16,
  'capexB': 6.0,
  'r40': 215.5,
  'budget': None,
  'lo52': 50.0,
  'hi52': 299.86,
  'pxd': '2026-08-07'},
 {'t': 'CRWV',
  'name': 'CoreWeave',
  'ex': 'NASDAQ',
  'g': 13,
  'price': 90.67,
  'jan2': 73.93,
  'mcapB': 49.4583,
  'ath': 183.98,
  'fvMin': 36,
  'fvAvg': 139.33,
  'fvMax': 303,
  'an': 36,
  'peg': None,
  'ps': 6.97,
  'fpe': None,
  'pe': None,
  'gm': 69.38,
  'pm': -25.57,
  'revB': 6.23,
  'revG': None,
  'eps': -3.15,
  'epsG': None,
  'roi': -0.31,
  'roe': -40.67,
  'fcfB': -10.62,
  'capexB': 16.6,
  'r40': None,
  'budget': None,
  'lo52': 60.55,
  'hi52': 153.2,
  'pxd': '2026-08-07'},
 {'t': 'APLD',
  'name': 'Applied Digital',
  'ex': 'NASDAQ',
  'g': 13,
  'price': 29.22,
  'jan2': 37.68,
  'mcapB': 8.5133,
  'ath': 47.28,
  'fvMin': 48,
  'fvAvg': 76.7,
  'fvMax': 106,
  'an': 10,
  'peg': None,
  'ps': 24.34,
  'fpe': None,
  'pe': None,
  'gm': 45.41,
  'pm': -59.47,
  'revB': 0.32,
  'revG': 112.04,
  'eps': -0.74,
  'epsG': None,
  'roi': -0.69,
  'roe': -5.56,
  'fcfB': -1.81,
  'capexB': 1.77,
  'r40': None,
  'budget': None,
  'lo52': 9.79,
  'hi52': 50.72,
  'pxd': '2026-08-07'},
 {'t': 'CIFR',
  'name': 'Cipher Mining',
  'ex': 'NASDAQ',
  'g': 13,
  'price': 17.18,
  'jan2': 16.63,
  'mcapB': 7.0277,
  'ath': 23.65,
  'fvMin': 23,
  'fvAvg': 32.53,
  'fvMax': 69,
  'an': 16,
  'peg': None,
  'ps': 43.63,
  'fpe': None,
  'pe': None,
  'gm': 12.37,
  'pm': -586.1,
  'revB': 0.19,
  'revG': 20.3,
  'eps': -2.79,
  'epsG': None,
  'roi': -0.24,
  'roe': -199.3,
  'fcfB': -1.02,
  'capexB': 0.95,
  'r40': None,
  'budget': None,
  'lo52': 4.55,
  'hi52': 30.14,
  'pxd': '2026-08-07'},
 {'t': 'HUT',
  'name': 'Hut 8 Corp',
  'ex': 'NASDAQ',
  'g': 13,
  'price': 88.59,
  'jan2': 60.04,
  'mcapB': 9.975,
  'ath': 124.83,
  'fvMin': 70,
  'fvAvg': 140.19,
  'fvMax': 226,
  'an': 16,
  'peg': None,
  'ps': 43.56,
  'fpe': None,
  'pe': None,
  'gm': 59.67,
  'pm': -109.77,
  'revB': 0.28,
  'revG': 185.05,
  'eps': -2.9,
  'epsG': None,
  'roi': -26.95,
  'roe': -27.4,
  'fcfB': -0.32,
  'capexB': 0.19,
  'r40': 71.5,
  'budget': None,
  'lo52': 18.68,
  'hi52': 140.8,
  'pxd': '2026-08-07'},
 {'t': 'NOK',
  'name': 'Nokia (ADR)',
  'ex': 'NYSE',
  'g': 13,
  'price': 9.36,
  'jan2': None,
  'mcapB': 52.48,
  'ath': 17.45,
  'fvMin': 8.5,
  'fvAvg': 15.02,
  'fvMax': 21,
  'an': 11,
  'peg': None,
  'ps': 2.2,
  'fpe': 20.97,
  'pe': 63.36,
  'gm': 45.5,
  'pm': 3.47,
  'revB': 23.3,
  'revG': 7.0,
  'eps': 0.14,
  'epsG': -35.3,
  'roi': 6.41,
  'roe': 3.45,
  'fcfB': 0.61683,
  'capexB': 0.69062,
  'r40': 9.65,
  'budget': None,
  'lo52': 4.0,
  'hi52': 17.45,
  'pxd': '2026-08-07'},
 {'t': 'CORZ',
  'name': 'Core Scientific',
  'ex': 'NASDAQ',
  'g': 13,
  'price': 21.16,
  'jan2': None,
  'mcapB': 6.8039,
  'ath': 30.46,
  'fvMin': 25,
  'fvAvg': 33.38,
  'fvMax': 42,
  'an': 14,
  'peg': None,
  'ps': 16.36,
  'fpe': 711.44,
  'pe': None,
  'gm': 28.35,
  'pm': None,
  'revB': 0.44031,
  'revG': None,
  'eps': -5.07,
  'epsG': None,
  'roi': -9.62,
  'roe': None,
  'fcfB': -0.95775,
  'capexB': -1.48,
  'r40': None,
  'budget': None,
  'lo52': 13.14,
  'hi52': 30.46,
  'pxd': '2026-08-07'},
 {'t': 'STRL',
  'name': 'Sterling Infrastructure',
  'ex': 'NASDAQ',
  'g': 13,
  'price': 547.06,
  'jan2': None,
  'mcapB': 16.73,
  'ath': 1005.68,
  'fvMin': 754,
  'fvAvg': 918.67,
  'fvMax': 1000,
  'an': 7,
  'peg': 0.69,
  'ps': 4.87,
  'fpe': 24.69,
  'pe': 39.44,
  'gm': 23.81,
  'pm': 12.55,
  'revB': 3.44,
  'revG': None,
  'eps': 13.87,
  'epsG': None,
  'roi': 41.69,
  'roe': 40.0,
  'fcfB': 0.482,
  'capexB': 0.1157,
  'r40': None,
  'budget': None,
  'lo52': 263.45,
  'hi52': 1005.68,
  'pxd': '2026-08-07'},
 {'t': 'KEEL',
  'name': 'Keel Infrastructure',
  'ex': 'NASDAQ',
  'g': 13,
  'price': 3.88,
  'jan2': None,
  'mcapB': 2.34,
  'ath': 7.37,
  'fvMin': 4.5,
  'fvAvg': 6.55,
  'fvMax': 10,
  'an': 11,
  'peg': None,
  'ps': 12.44,
  'fpe': None,
  'pe': None,
  'gm': -68.24,
  'pm': -230.59,
  'revB': 0.18814,
  'revG': 8.5,
  'eps': -0.72,
  'epsG': None,
  'roi': -20.1,
  'roe': -54.94,
  'fcfB': -0.41288,
  'capexB': 0.14044,
  'r40': None,
  'budget': None,
  'lo52': 1.17,
  'hi52': 7.37,
  'pxd': '2026-08-07'},
 {'t': 'V',
  'name': 'Visa',
  'ex': 'NYSE',
  'g': 14,
  'price': 362.5,
  'jan2': 349.77,
  'mcapB': 665.4111,
  'ath': 362.13,
  'fvMin': 350,
  'fvAvg': 411.18,
  'fvMax': 450,
  'an': 62,
  'peg': 2.11,
  'ps': 15.23,
  'fpe': 24.65,
  'pe': 31.33,
  'gm': 80.18,
  'pm': 50.78,
  'revB': 43.03,
  'revG': 14.37,
  'eps': 11.47,
  'epsG': 15.38,
  'roi': 34.22,
  'roe': 61.26,
  'fcfB': 21.19,
  'capexB': 1.57,
  'r40': 63.61,
  'budget': None,
  'lo52': 293.89,
  'hi52': 369.59,
  'pxd': '2026-08-07'},
 {'t': 'MA',
  'name': 'Mastercard',
  'ex': 'NYSE',
  'g': 14,
  'price': 562.95,
  'jan2': 575.54,
  'mcapB': 497.4137,
  'ath': 598.96,
  'fvMin': 550,
  'fvAvg': 644.24,
  'fvMax': 735,
  'an': 41,
  'peg': 1.62,
  'ps': 14.19,
  'fpe': 26.66,
  'pe': 30.6,
  'gm': 100.0,
  'pm': 46.34,
  'revB': 35.08,
  'revG': 14.07,
  'eps': 18.41,
  'epsG': 22.11,
  'roi': 95.21,
  'roe': 289.99,
  'fcfB': 15.97,
  'capexB': 1.46,
  'r40': 59.59,
  'budget': None,
  'lo52': 464.52,
  'hi52': 601.77,
  'pxd': '2026-08-07'},
 {'t': 'PYPL',
  'name': 'PayPal',
  'ex': 'NASDAQ',
  'g': 14,
  'price': 59.07,
  'jan2': 57.66,
  'mcapB': 50.5264,
  'ath': 77.98,
  'fvMin': 40,
  'fvAvg': 53.57,
  'fvMax': 70,
  'an': 70,
  'peg': 0.86,
  'ps': 1.51,
  'fpe': 10.15,
  'pe': 10.95,
  'gm': 45.75,
  'pm': 14.36,
  'revB': 33.73,
  'revG': 5.79,
  'eps': 5.34,
  'epsG': 19.24,
  'roi': 14.54,
  'roe': 24.42,
  'fcfB': 5.5,
  'capexB': 0.89,
  'r40': 22.1,
  'budget': None,
  'lo52': 38.46,
  'hi52': 79.21,
  'pxd': '2026-08-07'},
 {'t': 'HOOD',
  'name': 'Robinhood Markets',
  'ex': 'NASDAQ',
  'g': 14,
  'price': 93.29,
  'jan2': 115.27,
  'mcapB': 83.9961,
  'ath': 146.78,
  'fvMin': 100,
  'fvAvg': 122.19,
  'fvMax': 160,
  'an': 26,
  'peg': 2.82,
  'ps': 24.48,
  'fpe': 33.07,
  'pe': 38.89,
  'gm': 92.24,
  'pm': 39.56,
  'revB': 4.93,
  'revG': 32.0,
  'eps': 2.13,
  'epsG': 14.3,
  'roi': 9.24,
  'roe': 22.75,
  'fcfB': 3.01,
  'capexB': 0.02,
  'r40': 93.0,
  'budget': None,
  'lo52': 59.68,
  'hi52': 153.86,
  'pxd': '2026-08-07'},
 {'t': 'SOFI',
  'name': 'SoFi Technologies',
  'ex': 'NASDAQ',
  'g': 14,
  'price': 18.38,
  'jan2': 27.4,
  'mcapB': 23.5485,
  'ath': 29.72,
  'fvMin': 16,
  'fvAvg': 21.22,
  'fvMax': 29,
  'an': 27,
  'peg': 0.75,
  'ps': 4.03,
  'fpe': 25.55,
  'pe': 29.9,
  'gm': 83.51,
  'pm': 14.78,
  'revB': 4.31,
  'revG': 42.5,
  'eps': 0.48,
  'epsG': 50.0,
  'roi': 6.5,
  'roe': 7.11,
  'fcfB': -6.34,
  'capexB': 0.26,
  'r40': None,
  'budget': None,
  'lo52': 14.92,
  'hi52': 32.73,
  'pxd': '2026-08-07'},
 {'t': 'NU',
  'name': 'Nu Holdings',
  'ex': 'NYSE',
  'g': 14,
  'price': 13.84,
  'jan2': 17.46,
  'mcapB': 66.8519,
  'ath': 18.04,
  'fvMin': 10,
  'fvAvg': 17.94,
  'fvMax': 22,
  'an': 22,
  'peg': 0.46,
  'ps': 8.96,
  'fpe': 16.59,
  'pe': 21.74,
  'gm': None,
  'pm': 41.92,
  'revB': 7.59,
  'revG': 34.49,
  'eps': 0.65,
  'epsG': 47.51,
  'roi': 205.77,
  'roe': 30.05,
  'fcfB': -9.54,
  'capexB': 0.01,
  'r40': None,
  'budget': None,
  'lo52': 11.2,
  'hi52': 18.98,
  'pxd': '2026-08-07'},
 {'t': 'LMND',
  'name': 'Lemonade',
  'ex': 'NYSE',
  'g': 14,
  'price': 52.94,
  'jan2': 75.96,
  'mcapB': 4.0994,
  'ath': 183.26,
  'fvMin': 37.0,
  'fvAvg': 61.22,
  'fvMax': 92.0,
  'an': 11,
  'peg': None,
  'ps': 4.21,
  'fpe': None,
  'pe': None,
  'gm': 51.65,
  'pm': -14.19,
  'revB': 0.975,
  'revG': 62.31,
  'eps': -1.83,
  'epsG': None,
  'roi': -9.91,
  'roe': -26.96,
  'fcfB': 0.0081,
  'capexB': -0.0131,
  'r40': 63.1,
  'budget': None,
  'lo52': 36.13,
  'hi52': 99.9,
  'pxd': '2026-08-07'},
 {'t': 'CRWD',
  'name': 'CrowdStrike',
  'ex': 'NASDAQ',
  'g': 15,
  'price': 225.16,
  'jan2': 118.5,
  'mcapB': 229.2621,
  'ath': 225.16,
  'fvMin': 103.25,
  'fvAvg': 187.4,
  'fvMax': 250,
  'an': 52,
  'peg': 5.49,
  'ps': 40.59,
  'fpe': 155.49,
  'pe': None,
  'gm': 75.14,
  'pm': -0.6,
  'revB': 5.09,
  'revG': 23.2,
  'eps': -0.03,
  'epsG': None,
  'roi': -31.88,
  'roe': -0.25,
  'fcfB': 1.51,
  'capexB': 0.31,
  'r40': 52.7,
  'budget': 10000,
  'lo52': 85.68,
  'hi52': 225.16,
  'pxd': '2026-08-10'},
 {'t': 'PANW',
  'name': 'Palo Alto Networks',
  'ex': 'NASDAQ',
  'g': 15,
  'price': 385.04,
  'jan2': 184.81,
  'mcapB': 313.8065,
  'ath': 385.04,
  'fvMin': 162.34,
  'fvAvg': 325.37,
  'fvMax': 433,
  'an': 54,
  'peg': 6.03,
  'ps': 27.56,
  'fpe': 91.41,
  'pe': 346.81,
  'gm': 71.96,
  'pm': 7.95,
  'revB': 10.61,
  'revG': 19.5,
  'eps': 1.15,
  'epsG': -30.3,
  'roi': 3.65,
  'roe': 4.83,
  'fcfB': 3.79,
  'capexB': 0.42,
  'r40': 55.3,
  'budget': 10000,
  'lo52': 139.57,
  'hi52': 385.04,
  'pxd': '2026-08-10'},
 {'t': 'ZS',
  'name': 'Zscaler',
  'ex': 'NASDAQ',
  'g': 15,
  'price': 168.68,
  'jan2': 216.73,
  'mcapB': 27.2797,
  'ath': 336.99,
  'fvMin': 145,
  'fvAvg': 192.55,
  'fvMax': 250,
  'an': 46,
  'peg': 1.39,
  'ps': 7.25,
  'fpe': 32.08,
  'pe': None,
  'gm': 76.74,
  'pm': -2.44,
  'revB': 3.17,
  'revG': 25.43,
  'eps': -0.49,
  'epsG': None,
  'roi': -2.32,
  'roe': -3.71,
  'fcfB': 0.96,
  'capexB': 0.14,
  'r40': 55.82,
  'budget': None,
  'lo52': 114.63,
  'hi52': 336.99,
  'pxd': '2026-08-07'},
 {'t': 'FTNT',
  'name': 'Fortinet',
  'ex': 'NASDAQ',
  'g': 15,
  'price': 159.64,
  'jan2': 79.22,
  'mcapB': 116.9546,
  'ath': 170.35,
  'fvMin': 75,
  'fvAvg': 131.78,
  'fvMax': 215,
  'an': 44,
  'peg': 3.67,
  'ps': 15.7,
  'fpe': 48.73,
  'pe': 59.08,
  'gm': 80.3,
  'pm': 27.49,
  'revB': 7.11,
  'revG': 20.13,
  'eps': 2.58,
  'epsG': 28.57,
  'roi': 59.98,
  'roe': 132.39,
  'fcfB': 2.44,
  'capexB': 0.37,
  'r40': 54.45,
  'budget': None,
  'lo52': 70.12,
  'hi52': 170.35,
  'pxd': '2026-08-07'},
 {'t': 'NET',
  'name': 'Cloudflare',
  'ex': 'NYSE',
  'g': 15,
  'price': 300.27,
  'jan2': 182.78,
  'mcapB': 106.5823,
  'ath': 291.0,
  'fvMin': 136,
  'fvAvg': 259.84,
  'fvMax': 330,
  'an': 34,
  'peg': 4.58,
  'ps': 40.18,
  'fpe': 204.1,
  'pe': None,
  'gm': 72.58,
  'pm': -8.21,
  'revB': 2.51,
  'revG': 33.54,
  'eps': -0.59,
  'epsG': None,
  'roi': -2.51,
  'roe': -14.43,
  'fcfB': 0.342,
  'capexB': 0.292,
  'r40': 47.2,
  'budget': None,
  'lo52': 158.83,
  'hi52': 301.21,
  'pxd': '2026-08-07'},
 {'t': 'RBRK',
  'name': 'Rubrik',
  'ex': 'NYSE',
  'g': 15,
  'price': 90.07,
  'jan2': 75.47,
  'mcapB': 18.539,
  'ath': 103.0,
  'fvMin': 85.0,
  'fvAvg': 95.23,
  'fvMax': 115.0,
  'an': 27,
  'peg': None,
  'ps': 11.5,
  'fpe': 273.43,
  'pe': None,
  'gm': 80.58,
  'pm': -20.25,
  'revB': 1.425,
  'revG': 45.73,
  'eps': -1.45,
  'epsG': None,
  'roi': -80.59,
  'roe': None,
  'fcfB': 0.29388,
  'capexB': -0.03106,
  'r40': 66.36,
  'budget': None,
  'lo52': 42.25,
  'hi52': 99.75,
  'pxd': '2026-08-07'},
 {'t': 'PWR',
  'name': 'Quanta Services',
  'ex': 'NYSE',
  'g': 17,
  'price': 671.86,
  'jan2': 439.53,
  'mcapB': 101.0116,
  'ath': 785.12,
  'fvMin': 410.0,
  'fvAvg': 765.49,
  'fvMax': 976.0,
  'an': 30,
  'peg': 1.98,
  'ps': 3.17,
  'fpe': 38.49,
  'pe': 79.33,
  'gm': 15.46,
  'pm': 4.03,
  'revB': 32.91,
  'revG': 26.3,
  'eps': 8.74,
  'epsG': 35.14,
  'roi': 10.02,
  'roe': 15.34,
  'fcfB': 2.29,
  'capexB': -0.89046,
  'r40': 33.25,
  'budget': None,
  'lo52': 363.01,
  'hi52': 788.75,
  'pxd': '2026-08-07'},
 {'t': 'ETN',
  'name': 'Eaton',
  'ex': 'NYSE',
  'g': 17,
  'price': 444.96,
  'jan2': 323.21,
  'mcapB': 172.7732,
  'ath': 444.96,
  'fvMin': 321,
  'fvAvg': 455.79,
  'fvMax': 534,
  'an': 27,
  'peg': 2.96,
  'ps': 5.56,
  'fpe': 30.76,
  'pe': 42.28,
  'gm': 36.1,
  'pm': 12.75,
  'revB': 30.03,
  'revG': 15.53,
  'eps': 9.82,
  'epsG': -1.21,
  'roi': 12.39,
  'roe': 20.84,
  'fcfB': 3.94,
  'capexB': 1.02,
  'r40': 28.7,
  'budget': 10000,
  'lo52': 311.92,
  'hi52': 448.68,
  'pxd': '2026-08-10'},
 {'t': 'POWL',
  'name': 'Powell Industries',
  'ex': 'NASDAQ',
  'g': 17,
  'price': 211.56,
  'jan2': 117.41,
  'mcapB': 7.7066,
  'ath': 321.94,
  'fvMin': 252.0,
  'fvAvg': 316.25,
  'fvMax': 360.0,
  'an': 4,
  'peg': 2.86,
  'ps': 6.65,
  'fpe': 34.32,
  'pe': 40.47,
  'gm': 30.08,
  'pm': 16.49,
  'revB': 1.158,
  'revG': 7.04,
  'eps': 5.22,
  'epsG': 15.18,
  'roi': 38.38,
  'roe': 28.25,
  'fcfB': 0.19252,
  'capexB': -0.01075,
  'r40': 24.05,
  'budget': None,
  'lo52': 69.0,
  'hi52': 328.0,
  'pxd': '2026-08-07'},
 {'t': 'NVT',
  'name': 'nVent Electric',
  'ex': 'NYSE',
  'g': 17,
  'price': 164.69,
  'jan2': 106.32,
  'mcapB': 26.6565,
  'ath': 184.1,
  'fvMin': 131.0,
  'fvAvg': 200.4,
  'fvMax': 231.0,
  'an': 17,
  'peg': None,
  'ps': 5.47,
  'fpe': 28.1,
  'pe': 44.49,
  'gm': 36.97,
  'pm': 12.38,
  'revB': 4.834,
  'revG': 46.19,
  'eps': 3.65,
  'epsG': 4.27,
  'roi': 12.3,
  'roe': 15.75,
  'fcfB': 0.5777,
  'capexB': -0.1129,
  'r40': 58.14,
  'budget': None,
  'lo52': 85.72,
  'hi52': 184.64,
  'pxd': '2026-08-07'},
 {'t': 'HUBB',
  'name': 'Hubbell',
  'ex': 'NYSE',
  'g': 17,
  'price': 513.99,
  'jan2': 460.39,
  'mcapB': 27.1507,
  'ath': 556.18,
  'fvMin': 502.0,
  'fvAvg': 566.3,
  'fvMax': 630.0,
  'an': 12,
  'peg': 2.32,
  'ps': 4.26,
  'fpe': 23.22,
  'pe': 29.73,
  'gm': 35.34,
  'pm': 14.49,
  'revB': 6.224,
  'revG': 10.61,
  'eps': 16.9,
  'epsG': 9.46,
  'roi': 13.6,
  'roe': 24.44,
  'fcfB': 0.9014,
  'capexB': -0.1668,
  'r40': 25.09,
  'budget': None,
  'lo52': 403.82,
  'hi52': 565.5,
  'pxd': '2026-08-07'},
 {'t': 'GEV',
  'name': 'GE Vernova',
  'ex': 'NYSE',
  'g': 17,
  'price': 990.85,
  'jan2': 661.19,
  'mcapB': 263.8927,
  'ath': 1195.94,
  'fvMin': 836,
  'fvAvg': 1221.48,
  'fvMax': 1424,
  'an': 38,
  'peg': 1.81,
  'ps': 7.22,
  'fpe': 57.29,
  'pe': 30.95,
  'gm': 20.28,
  'pm': 23.81,
  'revB': 39.38,
  'revG': 10.3,
  'eps': 34.17,
  'epsG': 393.5,
  'roi': 35.65,
  'roe': 75.71,
  'fcfB': 7.53,
  'capexB': 1.49,
  'r40': 29.4,
  'budget': 10000,
  'lo52': 530.16,
  'hi52': 1195.94,
  'pxd': '2026-08-10'},
 {'t': 'MPWR',
  'name': 'Monolithic Power',
  'ex': 'NASDAQ',
  'g': 17,
  'price': 1381.1,
  'jan2': 927.61,
  'mcapB': 67.8752,
  'ath': 1714.09,
  'fvMin': 1500,
  'fvAvg': 1789.23,
  'fvMax': 2000,
  'an': 15,
  'peg': 1.23,
  'ps': 21.92,
  'fpe': 45.2,
  'pe': 87.01,
  'gm': 55.2,
  'pm': 24.5,
  'revB': 3.27,
  'revG': 28.67,
  'eps': 16.39,
  'epsG': -52.55,
  'roi': 31.84,
  'roe': 21.98,
  'fcfB': 0.63,
  'capexB': 0.2,
  'r40': 47.9,
  'budget': 5000,
  'lo52': 702.32,
  'hi52': 1714.09,
  'pxd': '2026-08-10'},
 {'t': 'HYLN',
  'name': 'Hyliion Holdings',
  'ex': 'NYSE American',
  'g': 17,
  'price': 3.87,
  'jan2': None,
  'mcapB': 0.69,
  'ath': 8.49,
  'fvMin': 5,
  'fvAvg': 7,
  'fvMax': 9,
  'an': 3,
  'peg': None,
  'ps': 121.07,
  'fpe': None,
  'pe': None,
  'gm': 6.33,
  'pm': -888.12,
  'revB': 0.00582,
  'revG': 191.19,
  'eps': -0.29,
  'epsG': None,
  'roi': -17.99,
  'roe': -25.22,
  'fcfB': -0.06355,
  'capexB': 0.01828,
  'r40': None,
  'budget': None,
  'lo52': 1.47,
  'hi52': 8.49,
  'pxd': '2026-08-07'},
 {'t': 'POWI',
  'name': 'Power Integrations',
  'ex': 'NASDAQ',
  'g': 17,
  'price': 64.59,
  'jan2': None,
  'mcapB': 3.6,
  'ath': 91.18,
  'fvMin': 65,
  'fvAvg': 77.5,
  'fvMax': 85,
  'an': 5,
  'peg': 1.5,
  'ps': 8.01,
  'fpe': 38.57,
  'pe': 146.68,
  'gm': 53.64,
  'pm': 5.58,
  'revB': 0.44937,
  'revG': 1.6,
  'eps': 0.44,
  'epsG': None,
  'roi': 7.85,
  'roe': 3.62,
  'fcfB': 0.07904,
  'capexB': 0.01904,
  'r40': 19.2,
  'budget': None,
  'lo52': 30.86,
  'hi52': 91.18,
  'pxd': '2026-08-07'},
 {'t': 'VRT',
  'name': 'Vertiv Holdings',
  'ex': 'NYSE',
  'g': 18,
  'price': 270.1,
  'jan2': 169.47,
  'mcapB': 103.9841,
  'ath': 379.94,
  'fvMin': 236,
  'fvAvg': 380.0,
  'fvMax': 500,
  'an': 27,
  'peg': 1.28,
  'ps': 10.26,
  'fpe': 42.38,
  'pe': 72.77,
  'gm': 37.15,
  'pm': 14.37,
  'revB': 10.84,
  'revG': 29.0,
  'eps': 3.98,
  'epsG': 131.4,
  'roi': 32.13,
  'roe': 45.1,
  'fcfB': 2.28,
  'capexB': 0.3,
  'r40': 50.0,
  'budget': 10000,
  'lo52': 118.7,
  'hi52': 379.93,
  'pxd': '2026-08-10'},
 {'t': 'NEE',
  'name': 'NextEra Energy',
  'ex': 'NYSE',
  'g': 18,
  'price': 84.65,
  'jan2': 79.89,
  'mcapB': 176.5571,
  'ath': 98.75,
  'fvMin': 55,
  'fvAvg': 99.1,
  'fvMax': 116,
  'an': 22,
  'peg': 2.81,
  'ps': 6.52,
  'fpe': 21.72,
  'pe': 20.18,
  'gm': 61.02,
  'pm': 32.4,
  'revB': 28.7,
  'revG': 10.74,
  'eps': 4.45,
  'epsG': -2.08,
  'roi': 3.14,
  'roe': 11.68,
  'fcfB': -16.57,
  'capexB': 30.37,
  'r40': 43.14,
  'budget': None,
  'lo52': 69.24,
  'hi52': 98.75,
  'pxd': '2026-08-07'},
 {'t': 'BE',
  'name': 'Bloom Energy',
  'ex': 'NYSE',
  'g': 18,
  'price': 219.34,
  'jan2': 98.69,
  'mcapB': 64.6004,
  'ath': 351.28,
  'fvMin': 70,
  'fvAvg': 286.2,
  'fvMax': 390,
  'an': 29,
  'peg': None,
  'ps': 15.24,
  'fpe': 52.23,
  'pe': 188.61,
  'gm': 31.65,
  'pm': 7.87,
  'revB': 3.113,
  'revG': 91.0,
  'eps': 0.88,
  'epsG': None,
  'roi': 22.6,
  'roe': 22.21,
  'fcfB': 0.625,
  'capexB': 0.113,
  'r40': 98.9,
  'budget': None,
  'lo52': 18.12,
  'hi52': 322.83,
  'pxd': '2026-08-07'},
 {'t': 'AIRJ',
  'name': 'AirJoule Technologies',
  'ex': 'NASDAQ',
  'g': 18,
  'price': 5.92,
  'jan2': 4.19,
  'mcapB': 0.4288,
  'ath': 29.27,
  'fvMin': 6.0,
  'fvAvg': 8.15,
  'fvMax': 12.0,
  'an': 5,
  'peg': None,
  'ps': None,
  'fpe': None,
  'pe': None,
  'gm': None,
  'pm': None,
  'revB': None,
  'revG': None,
  'eps': -1.19,
  'epsG': None,
  'roi': -6.14,
  'roe': -28.84,
  'fcfB': -0.00809,
  'capexB': -3.75e-05,
  'r40': None,
  'budget': None,
  'lo52': 2.22,
  'hi52': 6.75,
  'pxd': '2026-08-07'},
 {'t': 'FLNC',
  'name': 'Fluence Energy',
  'ex': 'NASDAQ',
  'g': 18,
  'price': 13.2,
  'jan2': None,
  'mcapB': 2.4304,
  'ath': 37.61,
  'fvMin': 9,
  'fvAvg': 18.67,
  'fvMax': 28,
  'an': 21,
  'peg': None,
  'ps': 0.94,
  'fpe': None,
  'pe': None,
  'gm': 9.36,
  'pm': -3.07,
  'revB': 2.63,
  'revG': 7.5,
  'eps': -0.61,
  'epsG': None,
  'roi': -22.66,
  'roe': -23.54,
  'fcfB': -0.12,
  'capexB': 0.01,
  'r40': 4.4,
  'budget': None,
  'lo52': 6.6,
  'hi52': 33.51,
  'pxd': '2026-08-07'},
 {'t': 'NRG',
  'name': 'NRG Energy',
  'ex': 'NYSE',
  'g': 18,
  'price': 118.13,
  'jan2': None,
  'mcapB': 25.2136,
  'ath': 189.96,
  'fvMin': 104,
  'fvAvg': 194.18,
  'fvMax': 267,
  'an': 17,
  'peg': 0.5,
  'ps': 0.76,
  'fpe': 11.78,
  'pe': 31.31,
  'gm': 18.8,
  'pm': 2.56,
  'revB': 33.13,
  'revG': 12.8,
  'eps': 3.87,
  'epsG': 76.2,
  'roi': 6.23,
  'roe': 23.77,
  'fcfB': 0.348,
  'capexB': 1.21,
  'r40': 13.85,
  'budget': None,
  'lo52': 112.5,
  'hi52': 189.96,
  'pxd': '2026-08-07'},
 {'t': 'MYRG',
  'name': 'MYR Group',
  'ex': 'NASDAQ',
  'g': 18,
  'price': 337.42,
  'jan2': None,
  'mcapB': 5.25,
  'ath': 503.57,
  'fvMin': 295,
  'fvAvg': 423.33,
  'fvMax': 564,
  'an': 7,
  'peg': 1.5,
  'ps': 1.31,
  'fpe': 26.26,
  'pe': 32.01,
  'gm': 12.49,
  'pm': 4.13,
  'revB': 4.01,
  'revG': 16.1,
  'eps': 10.54,
  'epsG': 121.2,
  'roi': 25.59,
  'roe': 24.71,
  'fcfB': 0.19336,
  'capexB': 0.10513,
  'r40': 20.92,
  'budget': None,
  'lo52': 171.51,
  'hi52': 503.57,
  'pxd': '2026-08-07'},
 {'t': 'MELI',
  'name': 'MercadoLibre',
  'ex': 'NASDAQ',
  'g': 19,
  'price': 1820.69,
  'jan2': 2178.41,
  'mcapB': 92.3103,
  'ath': 2560.05,
  'fvMin': 1750,
  'fvAvg': 2215.0,
  'fvMax': 2800,
  'an': 24,
  'peg': 1.54,
  'ps': 2.87,
  'fpe': 44.79,
  'pe': 47.57,
  'gm': 49.47,
  'pm': 6.04,
  'revB': 31.8,
  'revG': 42.11,
  'eps': 37.87,
  'epsG': -6.81,
  'roi': 16.68,
  'roe': 31.26,
  'fcfB': 11.82,
  'capexB': 1.34,
  'r40': 79.28,
  'budget': None,
  'lo52': 1495.0,
  'hi52': 2548.5,
  'pxd': '2026-08-07'},
 {'t': 'SHOP',
  'name': 'Shopify',
  'ex': 'NYSE',
  'g': 19,
  'price': 151.57,
  'jan2': 164.48,
  'mcapB': 197.1926,
  'ath': 170.83,
  'fvMin': 126,
  'fvAvg': 154.91,
  'fvMax': 191,
  'an': 64,
  'peg': 1.97,
  'ps': 11.96,
  'fpe': 58.82,
  'pe': 110.33,
  'gm': 47.97,
  'pm': 10.77,
  'revB': 12.37,
  'revG': 31.85,
  'eps': 1.02,
  'epsG': -17.89,
  'roi': 23.25,
  'roe': 11.31,
  'fcfB': 2.12,
  'capexB': 0.03,
  'r40': 49.0,
  'budget': None,
  'lo52': 94.0,
  'hi52': 182.19,
  'pxd': '2026-08-07'},
 {'t': 'AFRM',
  'name': 'Affirm Holdings',
  'ex': 'NASDAQ',
  'g': 19,
  'price': 75.25,
  'jan2': 81.8,
  'mcapB': 25.2027,
  'ath': 92.18,
  'fvMin': 55.1,
  'fvAvg': 90.5,
  'fvMax': 117,
  'an': 34,
  'peg': 0.76,
  'ps': 6.0,
  'fpe': 19.2,
  'pe': 66.57,
  'gm': 48.37,
  'pm': 9.63,
  'revB': 3.97,
  'revG': 31.58,
  'eps': 1.06,
  'epsG': None,
  'roi': 2.85,
  'roe': 11.49,
  'fcfB': 0.79,
  'capexB': 0.22,
  'r40': 51.4,
  'budget': None,
  'lo52': 42.1,
  'hi52': 100.0,
  'pxd': '2026-08-07'},
 {'t': 'UBER',
  'name': 'Uber Technologies',
  'ex': 'NYSE',
  'g': 19,
  'price': 75.02,
  'jan2': 85.44,
  'mcapB': 152.7114,
  'ath': 98.51,
  'fvMin': 72,
  'fvAvg': 102.25,
  'fvMax': 125,
  'an': 61,
  'peg': 0.51,
  'ps': 2.59,
  'fpe': 16.22,
  'pe': 14.87,
  'gm': 40.75,
  'pm': 17.34,
  'revB': 55.23,
  'revG': 16.68,
  'eps': 4.58,
  'epsG': -22.49,
  'roi': 19.18,
  'roe': 37.16,
  'fcfB': 10.12,
  'capexB': 0.308,
  'r40': 35.0,
  'budget': None,
  'lo52': 65.41,
  'hi52': 101.99,
  'pxd': '2026-08-07'},
 {'t': 'NFLX',
  'name': 'Netflix',
  'ex': 'NASDAQ',
  'g': 19,
  'price': 74.14,
  'jan2': None,
  'mcapB': 308.71,
  'ath': 126.71,
  'fvMin': 70,
  'fvAvg': 94.04,
  'fvMax': 135,
  'an': 51,
  'peg': 1.07,
  'ps': 6.38,
  'fpe': 21.4,
  'pe': 23.36,
  'gm': 49.12,
  'pm': 28.22,
  'revB': 48.37,
  'revG': 13.3,
  'eps': 3.17,
  'epsG': None,
  'roi': 32.43,
  'roe': 49.54,
  'fcfB': 11.15,
  'capexB': 0.81883,
  'r40': 36.4,
  'budget': None,
  'lo52': 65.08,
  'hi52': 126.71,
  'pxd': '2026-08-07'},
 {'t': 'BKNG',
  'name': 'Booking Holdings',
  'ex': 'NASDAQ',
  'g': 19,
  'price': 214.42,
  'jan2': None,
  'mcapB': 161.11,
  'ath': 231.8,
  'fvMin': 188,
  'fvAvg': 237.81,
  'fvMax': 301,
  'an': 38,
  'peg': 1.16,
  'ps': 5.7,
  'fpe': 19.25,
  'pe': 23.7,
  'gm': 87.23,
  'pm': 25.53,
  'revB': 28.24,
  'revG': 9.0,
  'eps': 9.05,
  'epsG': None,
  'roi': 92.45,
  'roe': None,
  'fcfB': 9.54,
  'capexB': 0.32,
  'r40': 42.8,
  'budget': None,
  'lo52': 150.14,
  'hi52': 231.8,
  'pxd': '2026-08-07'},
 {'t': 'WDAY',
  'name': 'Workday',
  'ex': 'NASDAQ',
  'g': 19,
  'price': 179.64,
  'jan2': None,
  'mcapB': 44.37,
  'ath': 249.85,
  'fvMin': 92,
  'fvAvg': 168.64,
  'fvMax': 275,
  'an': 41,
  'peg': 0.76,
  'ps': 4.5,
  'fpe': 16.15,
  'pe': 55.94,
  'gm': 75.77,
  'pm': 8.6,
  'revB': 9.85,
  'revG': 13.5,
  'eps': 3.21,
  'epsG': None,
  'roi': 12.62,
  'roe': 10.86,
  'fcfB': 2.97,
  'capexB': 0.206,
  'r40': 43.7,
  'budget': None,
  'lo52': 110.36,
  'hi52': 249.85,
  'pxd': '2026-08-07'},
 {'t': 'SOUN',
  'name': 'SoundHound AI',
  'ex': 'NASDAQ',
  'g': 19,
  'price': 8.02,
  'jan2': None,
  'mcapB': 3.7381,
  'ath': 22.17,
  'fvMin': 8,
  'fvAvg': 12.86,
  'fvMax': 17,
  'an': 7,
  'peg': None,
  'ps': 15.19,
  'fpe': None,
  'pe': None,
  'gm': 42.29,
  'pm': -67.45,
  'revB': 0.2032,
  'revG': 54.6,
  'eps': -0.38,
  'epsG': None,
  'roi': -28.31,
  'roe': -32.24,
  'fcfB': -0.14238,
  'capexB': 0.02787,
  'r40': None,
  'budget': None,
  'lo52': 5.65,
  'hi52': 22.17,
  'pxd': '2026-08-07'},
 {'t': 'SE',
  'name': 'Sea Limited',
  'ex': 'NYSE',
  'g': 19,
  'price': 113.43,
  'jan2': 133.52,
  'mcapB': 69.4765,
  'ath': 196.05,
  'fvMin': 101,
  'fvAvg': 142.26,
  'fvMax': 195,
  'an': 29,
  'peg': 1.28,
  'ps': 2.38,
  'fpe': 24.2,
  'pe': 39.17,
  'gm': 44.27,
  'pm': 6.36,
  'revB': 25.19,
  'revG': 40.54,
  'eps': 2.54,
  'epsG': 78.87,
  'roi': 25.59,
  'roe': 14.86,
  'fcfB': 4.67,
  'capexB': 0.65,
  'r40': 59.08,
  'budget': None,
  'lo52': 77.05,
  'hi52': 199.3,
  'pxd': '2026-08-07'},
 {'t': 'CAT',
  'name': 'Caterpillar',
  'ex': 'NYSE',
  'g': 20,
  'price': 842.19,
  'jan2': None,
  'mcapB': 387.13,
  'ath': 1073.46,
  'fvMin': 575,
  'fvAvg': 972.95,
  'fvMax': 1225,
  'an': 28,
  'peg': 1.61,
  'ps': 5.18,
  'fpe': 29.32,
  'pe': 36.29,
  'gm': 29.71,
  'pm': 14.51,
  'revB': 74.73,
  'revG': 18.4,
  'eps': 23.21,
  'epsG': 18.0,
  'roi': 18.3,
  'roe': 56.97,
  'fcfB': 8.99,
  'capexB': 4.58,
  'r40': 30.4,
  'budget': None,
  'lo52': 405.46,
  'hi52': 1073.46,
  'pxd': '2026-08-07'},
 {'t': 'DE',
  'name': 'Deere & Co',
  'ex': 'NYSE',
  'g': 20,
  'price': 620.83,
  'jan2': None,
  'mcapB': 167.59,
  'ath': 674.19,
  'fvMin': 500,
  'fvAvg': 648.03,
  'fvMax': 812,
  'an': 24,
  'peg': 2.51,
  'ps': 3.54,
  'fpe': 31.25,
  'pe': 35.18,
  'gm': 25.96,
  'pm': 10.1,
  'revB': 47.34,
  'revG': 4.3,
  'eps': 17.65,
  'epsG': -14.6,
  'roi': 5.93,
  'roe': 18.35,
  'fcfB': 3.77,
  'capexB': 4.17,
  'r40': 12.3,
  'budget': None,
  'lo52': 433.0,
  'hi52': 674.19,
  'pxd': '2026-08-07'},
 {'t': 'GE',
  'name': 'GE Aerospace',
  'ex': 'NYSE',
  'g': 20,
  'price': 370.08,
  'jan2': None,
  'mcapB': 383.98,
  'ath': 388.84,
  'fvMin': 347,
  'fvAvg': 404.9,
  'fvMax': 455,
  'an': 22,
  'peg': 2.55,
  'ps': 7.58,
  'fpe': 44.09,
  'pe': 43.62,
  'gm': 31.05,
  'pm': 17.72,
  'revB': 50.64,
  'revG': 21.7,
  'eps': 8.49,
  'epsG': 18.4,
  'roi': 31.03,
  'roe': 48.23,
  'fcfB': 8.4,
  'capexB': 1.4,
  'r40': 38.3,
  'budget': None,
  'lo52': 263.8,
  'hi52': 388.84,
  'pxd': '2026-08-07'},
 {'t': 'MMM',
  'name': '3M Co',
  'ex': 'NYSE',
  'g': 20,
  'price': 182.9,
  'jan2': None,
  'mcapB': 94.33,
  'ath': 184.9,
  'fvMin': 120,
  'fvAvg': 181.85,
  'fvMax': 219,
  'an': 18,
  'peg': 2.16,
  'ps': 3.75,
  'fpe': 19.86,
  'pe': 32.53,
  'gm': 39.41,
  'pm': 11.9,
  'revB': 25.18,
  'revG': None,
  'eps': 5.62,
  'epsG': None,
  'roi': 36.58,
  'roe': 81.89,
  'fcfB': 3.99,
  'capexB': 0.914,
  'r40': None,
  'budget': None,
  'lo52': 139.34,
  'hi52': 184.9,
  'pxd': '2026-08-07'}]

POS = {'GOOGL': {'budget': 60000, 'cost': 55324.68, 'shares': 205},
 'AMZN': {'budget': 50000, 'cost': 48600.75, 'shares': 209},
 'MSFT': {'budget': 60000, 'cost': 52878.96, 'shares': 117},
 'META': {'budget': 50000, 'cost': 39239.51, 'shares': 59.5},
 'AAPL': {'budget': 25000, 'cost': 11437.61, 'shares': 46},
 'NVDA': {'budget': 80000, 'cost': 77340.96, 'shares': 411},
 'TSM': {'budget': 50000, 'cost': 37850.87, 'shares': 107.5},
 'AVGO': {'budget': 40000, 'cost': 28571.53, 'shares': 84},
 'AMD': {'budget': 25000, 'cost': 13789.22, 'shares': 55},
 'MRVL': {'budget': 25000, 'cost': 7872.66, 'shares': 48.5},
 'INTC': {'budget': 5000, 'cost': 2029.6, 'shares': 45},
 'QCOM': {'budget': 5000, 'cost': 1959.96, 'shares': 11},
 'ARM': {'budget': None, 'cost': None, 'shares': None},
 'AEHR': {'budget': None, 'cost': None, 'shares': None},
 'ASML': {'budget': 30000, 'cost': 16394.06, 'shares': 13},
 'AMAT': {'budget': 20000, 'cost': 8162.07, 'shares': 34},
 'LRCX': {'budget': 15000, 'cost': 4100.12, 'shares': 22},
 'KLAC': {'budget': 10000, 'cost': 3510.69, 'shares': 18},
 'AMKR': {'budget': 5000, 'cost': 3454.62, 'shares': 55},
 'MU': {'budget': 30000, 'cost': 20655.29, 'shares': 33.45},
 'SKHY': {'budget': 10000, 'cost': 1913.26, 'shares': 15},
 'SNDK': {'budget': None, 'cost': None, 'shares': None},
 'WDC': {'budget': None, 'cost': None, 'shares': None},
 'STX': {'budget': None, 'cost': None, 'shares': None},
 'DELL': {'budget': None, 'cost': None, 'shares': None},
 'HPE': {'budget': None, 'cost': None, 'shares': None},
 'SMCI': {'budget': None, 'cost': None, 'shares': None},
 'ANET': {'budget': 20000, 'cost': 15585.57, 'shares': 98},
 'CRDO': {'budget': 15000, 'cost': 7700.99, 'shares': 54},
 'LITE': {'budget': 20000, 'cost': 7610.18, 'shares': 9.5},
 'ALAB': {'budget': 15000, 'cost': 3805.47, 'shares': 15},
 'APH': {'budget': 10000, 'cost': 2004.4, 'shares': 16},
 'GLW': {'budget': None, 'cost': None, 'shares': None},
 'FN': {'budget': 5000, 'cost': 1604.56, 'shares': 3},
 'AAOI': {'budget': 5000, 'cost': 1338.56, 'shares': 10},
 'ETN': {'budget': 10000, 'cost': 7879.55, 'shares': 19},
 'VST': {'budget': 10000, 'cost': 6078.05, 'shares': 42},
 'CEG': {'budget': 10000, 'cost': 4040.98, 'shares': 15},
 'VRT': {'budget': 10000, 'cost': 3524.44, 'shares': 14},
 'GEV': {'budget': 10000, 'cost': 2147.3, 'shares': 2},
 'BWXT': {'budget': 5000, 'cost': 1994.06, 'shares': 10},
 'MPWR': {'budget': 5000, 'cost': 1538.56, 'shares': 1},
 'OKLO': {'budget': None, 'cost': None, 'shares': None},
 'SMR': {'budget': None, 'cost': None, 'shares': None},
 'NNE': {'budget': None, 'cost': None, 'shares': None},
 'CCJ': {'budget': None, 'cost': None, 'shares': None},
 'TE': {'budget': None, 'cost': None, 'shares': None},
 'EOSE': {'budget': None, 'cost': None, 'shares': None},
 'ENPH': {'budget': None, 'cost': None, 'shares': None},
 'NEE': {'budget': None, 'cost': None, 'shares': None},
 'BE': {'budget': None, 'cost': None, 'shares': None},
 'XE': {'budget': None, 'cost': None, 'shares': None},
 'ISRG': {'budget': 20000, 'cost': 16381.68, 'shares': 38},
 'CGNX': {'budget': 10000, 'cost': 4067.62, 'shares': 62},
 'TER': {'budget': 10000, 'cost': 3567.37, 'shares': 10},
 'TSLA': {'budget': 10000, 'cost': 4304.62, 'shares': 10},
 'OUST': {'budget': None, 'cost': None, 'shares': None},
 'ROK': {'budget': None, 'cost': None, 'shares': None},
 'RKLB': {'budget': 10000, 'cost': 1942.58, 'shares': 23},
 'ASTS': {'budget': None, 'cost': None, 'shares': None},
 'LUNR': {'budget': None, 'cost': None, 'shares': None},
 'SPCX': {'budget': None, 'cost': None, 'shares': None},
 'PLTR': {'budget': 10000, 'cost': 5533.09, 'shares': 40},
 'AVAV': {'budget': 5000, 'cost': 3758.3, 'shares': 18},
 'ONDS': {'budget': None, 'cost': 2030.54, 'shares': 200},
 'IONQ': {'budget': None, 'cost': None, 'shares': None},
 'ORCL': {'budget': 10000, 'cost': 8313.62, 'shares': 50},
 'ZETA': {'budget': 5000, 'cost': 2004.33, 'shares': 111},
 'CDNS': {'budget': 5000, 'cost': 2007.28, 'shares': 6},
 'CRM': {'budget': 5000, 'cost': 3749.91, 'shares': 21},
 'IBM': {'budget': 5000, 'cost': 1713.07, 'shares': 6},
 'ADBE': {'budget': None, 'cost': None, 'shares': None},
 'NOW': {'budget': None, 'cost': None, 'shares': None},
 'SNOW': {'budget': None, 'cost': None, 'shares': None},
 'DDOG': {'budget': None, 'cost': None, 'shares': None},
 'MDB': {'budget': None, 'cost': None, 'shares': None},
 'SBUX': {'budget': None, 'cost': None, 'shares': None},
 'MCD': {'budget': None, 'cost': None, 'shares': None},
 'KO': {'budget': None, 'cost': None, 'shares': None},
 'YUM': {'budget': None, 'cost': None, 'shares': None},
 'UNH': {'budget': None, 'cost': None, 'shares': None},
 'JNJ': {'budget': None, 'cost': None, 'shares': None},
 'TEM': {'budget': None, 'cost': None, 'shares': None},
 'IREN': {'budget': None, 'cost': 1735.16, 'shares': 45},
 'NBIS': {'budget': None, 'cost': None, 'shares': None},
 'CRWV': {'budget': None, 'cost': None, 'shares': None},
 'APLD': {'budget': None, 'cost': None, 'shares': None},
 'CIFR': {'budget': None, 'cost': None, 'shares': None},
 'HUT': {'budget': None, 'cost': None, 'shares': None},
 'V': {'budget': None, 'cost': None, 'shares': None},
 'MA': {'budget': None, 'cost': None, 'shares': None},
 'PYPL': {'budget': None, 'cost': None, 'shares': None},
 'HOOD': {'budget': None, 'cost': None, 'shares': None},
 'SOFI': {'budget': None, 'cost': None, 'shares': None},
 'AFRM': {'budget': None, 'cost': None, 'shares': None},
 'NU': {'budget': None, 'cost': None, 'shares': None},
 'SHOP': {'budget': None, 'cost': None, 'shares': None},
 'MELI': {'budget': None, 'cost': None, 'shares': None},
 'SE': {'budget': None, 'cost': None, 'shares': None},
 'UBER': {'budget': None, 'cost': None, 'shares': None},
 'CRWD': {'budget': 10000, 'cost': 3914.12, 'shares': 40},
 'PANW': {'budget': 10000, 'cost': 3760.62, 'shares': 22},
 'ZS': {'budget': None, 'cost': None, 'shares': None},
 'NET': {'budget': None, 'cost': None, 'shares': None},
 'FTNT': {'budget': None, 'cost': None, 'shares': None},
 'POET': {'budget': None, 'cost': None, 'shares': None},
 'AMPX': {'budget': None, 'cost': None, 'shares': None},
 'NOK': {'budget': None, 'cost': None, 'shares': None},
 'APP': {'budget': None, 'cost': None, 'shares': None},
 'COHR': {'budget': 5000, 'cost': 1328.56, 'shares': 4},
 'LMND': {'budget': None, 'cost': None, 'shares': None},
 'HNGE': {'budget': None, 'cost': None, 'shares': None},
 'AIRJ': {'budget': None, 'cost': None, 'shares': None},
 'RBRK': {'budget': None, 'cost': None, 'shares': None},
 'PL': {'budget': None, 'cost': None, 'shares': None},
 'PWR': {'budget': None, 'cost': None, 'shares': None},
 'POWL': {'budget': None, 'cost': None, 'shares': None},
 'NVT': {'budget': None, 'cost': None, 'shares': None},
 'HUBB': {'budget': None, 'cost': None, 'shares': None},
 'VEEV': {'budget': None, 'cost': None, 'shares': None},
 'CORZ': {'budget': None, 'cost': None, 'shares': None},
 'FLNC': {'budget': None, 'cost': None, 'shares': None},
 'LWLG': {'budget': None, 'cost': None, 'shares': None},
 'AXTI': {'budget': None, 'cost': None, 'shares': None},
 'AXON': {'budget': None, 'cost': None, 'shares': None},
 'TEAM': {'budget': None, 'cost': None, 'shares': None},
 'TMDX': {'budget': None, 'cost': None, 'shares': None},
 'CRSP': {'budget': None, 'cost': None, 'shares': None},
 'NRG': {'budget': None, 'cost': None, 'shares': None},
 'SOUN': {'budget': None, 'cost': None, 'shares': None},
 'UUUU': {'budget': None, 'cost': None, 'shares': None},
 'LEU': {'budget': None, 'cost': None, 'shares': None},
 'MYRG': {'budget': None, 'cost': None, 'shares': None},
 'STRL': {'budget': None, 'cost': None, 'shares': None},
 'FSLR': {'budget': None, 'cost': None, 'shares': None},
 'ON': {'budget': None, 'cost': None, 'shares': None},
 'KEEL': {'budget': None, 'cost': None, 'shares': None},
 'SIDU': {'budget': None, 'cost': None, 'shares': None},
 'HYLN': {'budget': None, 'cost': None, 'shares': None},
 'POWI': {'budget': None, 'cost': None, 'shares': None},
 'RCAT': {'budget': None, 'cost': None, 'shares': None},
 'LMT': {'budget': None, 'cost': None, 'shares': None},
 'RTX': {'budget': None, 'cost': None, 'shares': None},
 'NOC': {'budget': None, 'cost': None, 'shares': None},
 'NFLX': {'budget': None, 'cost': None, 'shares': None},
 'BKNG': {'budget': None, 'cost': None, 'shares': None},
 'WDAY': {'budget': None, 'cost': None, 'shares': None},
 'CAT': {'budget': None, 'cost': None, 'shares': None},
 'DE': {'budget': None, 'cost': None, 'shares': None},
 'GE': {'budget': None, 'cost': None, 'shares': None},
 'MMM': {'budget': None, 'cost': None, 'shares': None},
 'MCHP': {'budget': None, 'cost': None, 'shares': None},
 'NXPI': {'budget': None, 'cost': None, 'shares': None},
 'CSCO': {'budget': None, 'cost': None, 'shares': None},
 'CIEN': {'budget': None, 'cost': None, 'shares': None},
 'MTSI': {'budget': None, 'cost': None, 'shares': None},
 'VIAV': {'budget': None, 'cost': None, 'shares': None},
 'TSEM': {'budget': None, 'cost': None, 'shares': None},
 'GFS': {'budget': None, 'cost': None, 'shares': None},
 'FORM': {'budget': None, 'cost': None, 'shares': None},
 'COHU': {'budget': None, 'cost': None, 'shares': None}}

TRADES = [{'date': 'Jul 21, 2026',
  'action': 'BUY',
  't': 'MU',
  'shares': 2,
  'price': 921.01,
  'amount': 1842.02,
  'est': False,
  'note': 'Add per buy plan - avg cost now $585.49 incl fees (29.45 sh)'},
 {'date': 'Jul 21, 2026',
  'action': 'SELL',
  't': 'NBIS',
  'shares': 27,
  'price': 194.85,
  'amount': 5261.0,
  'est': False,
  'cost_released': 3856.65,
  'note': 'Full exit per plan - actual fills'},
 {'date': 'Jul 21, 2026',
  'action': 'SELL',
  't': 'IREN',
  'shares': 45,
  'price': 40.76,
  'amount': 1834.0,
  'est': False,
  'cost_released': 1735.17,
  'note': 'Tranche 1 trim - 45 sh remain; GTC $45 sell WITHDRAWN Jul 24 (not filled), still flagged To Be Sold'},
 {'date': 'Jul 24, 2026',
  'action': 'BUY',
  't': 'GOOGL',
  'shares': 7,
  'price': 319.38,
  'amount': 2235.64,
  'est': False,
  'note': 'Add on post-earnings capex dip - 205 sh total, avg cost $269.88'},
 {'date': 'Jul 22, 2026',
  'action': 'BUY',
  't': 'AMZN',
  'shares': 8,
  'price': 244.0,
  'amount': 1952.0,
  'est': False,
  'note': 'Limit 244.20, filled 244.00 - 202 sh total, $3,405 budget room left'},
 {'date': 'Jul 25, 2026',
  'action': 'BUY',
  't': 'NVDA',
  'shares': 9,
  'price': 211.0599,
  'amount': 1899.54,
  'est': False,
  'note': 'Limit 211.50, filled 211.06 - 392 sh total, avg cost $187.22 incl fees; budget left $6,610'},
 {'date': 'Jul 27, 2026',
  'action': 'BUY',
  't': 'MU',
  'shares': 2,
  'price': 869.16,
  'amount': 1738.32,
  'est': False,
  'note': 'Dad pick #6 - limit 870, filled 869.16 (dip from ~$990); 31.45 sh total, avg cost $603.80 incl fees; budget '
          'left $11,011. ORCL div +$39.18 same day'},
 {'date': 'Jul 28, 2026',
  'action': 'BUY',
  't': 'NVDA',
  'shares': 10,
  'price': 195.895,
  'amount': 1967.51,
  'est': False,
  'note': 'Dad pick, unadvised - filled overnight Jul 28 BKK; 10 sh @ $195.895 = $1,958.95 + $8.56 fees. Contract was '
          'still unsettled on the Jul 30 POEMS screen, which is why cash had already dropped but the trade was never '
          'logged. 402 sh total, avg cost $187.4561 incl fees; budget left $4,642.65'},
 {'date': 'Jul 29, 2026',
  'action': 'BUY',
  't': 'SKHY',
  'shares': 15,
  'price': 126.9791,
  'amount': 1913.26,
  'est': False,
  'note': 'Dad pick - limit 127.00, filled 126.9791 (order 6239421); 15 sh, avg cost $127.55 incl fees; budget left '
          '$8,087. NB bought at a ~33% premium to Seoul-parity value'},
 {'date': 'Jul 31, 2026',
  'action': 'BUY',
  't': 'MU',
  'shares': 2,
  'price': 828.6263,
  'amount': 1665.81,
  'est': False,
  'note': 'Dad pick - contract 396719/400/01, filled Jul 31 @ $828.6263, settles Aug 3. 33.45 sh total, avg cost '
          '$617.4975 incl fees; budget left $9,344.71'},
 {'date': 'Aug 3, 2026',
  'action': 'BUY',
  't': 'AMZN',
  'shares': 7,
  'price': 285.3599,
  'amount': 2006.08,
  'est': False,
  'note': 'Claude pick (AI-applier thesis) - limit 285.50 day order 65487, filled 285.3599 Aug 3; 7 sh = $1,997.52 + '
          '$8.56 fees. 209 sh total, avg cost $232.5395 incl fees; budget left $1,399.25'},
 {'date': 'Aug 4, 2026',
  'action': 'BUY',
  't': 'FN',
  'shares': 3,
  'price': 532.0,
  'amount': 1604.56,
  'est': False,
  'note': '3 sh @ $532.00 + $8.56 fee. New position, 3 sh, avg cost $534.8533.'},
 {'date': 'Aug 4, 2026',
  'action': 'BUY',
  't': 'LITE',
  'shares': 2,
  'price': 854.68,
  'amount': 1717.92,
  'est': False,
  'note': '2 sh @ $854.68 + $8.56 fee. 9.5 sh total, avg cost $801.0716.'},
 {'date': 'Aug 4, 2026',
  'action': 'BUY',
  't': 'META',
  'shares': 3.5,
  'price': 583.24,
  'amount': 2049.88,
  'est': False,
  'note': '3 + 0.5 sh @ $583.24 (one order, partial fill) + $8.54 fee. 59.5 sh total, avg cost $659.4876.'},
 {'date': 'Aug 4, 2026',
  'action': 'BUY',
  't': 'COHR',
  'shares': 4,
  'price': 330.0,
  'amount': 1328.56,
  'est': False,
  'note': '4 sh @ $330.00 + $8.56 fee. New position and new ticker (Photonic & Optic Fiber).'},
 {'date': 'Aug 4, 2026',
  'action': 'BUY',
  't': 'AAOI',
  'shares': 10,
  'price': 133.0,
  'amount': 1338.56,
  'est': False,
  'note': '10 sh @ $133.00 + $8.56 fee. New position, 10 sh, avg cost $133.856.'},
 {'date': 'Aug 5, 2026',
  'action': 'BUY',
  't': 'TSM',
  'shares': 4.5,
  'price': 416.273,
  'amount': 1873.23,
  'est': False,
  'note': 'Limit 418.00 day order filled at 416.273 (order O9636080581412011, 10:36 PM BKK / 11:36 AM ET). Cost basis '
          'incl $8.56 commission = $1881.79. Avg TSM cost now $352.10 over 107.5 sh.'},
 {'date': 'Aug 6, 2026',
  'action': 'BUY',
  't': 'NVDA',
  'shares': 9,
  'price': 219.45,
  'amount': 1975.05,
  'est': False,
  'note': 'Limit 219.45 day order filled at limit (order 290919, 10:49 PM BKK / 11:49 AM ET). NVDA 402 -> 411 sh, avg '
          'cost 187.46 -> 188.16. Budget 80,000 now 96.7% used; 2,667.60 headroom left.'},
 {'date': 'Aug 6, 2026',
  'action': 'BUY',
  't': 'CRM',
  'shares': 10,
  'price': 185.328,
  'amount': 1853.28,
  'est': False,
  'note': 'Limit 185.56 day order filled 185.328 (order 290805, 10:48 PM BKK / 11:48 AM ET). CRM 11 -> 21 sh, avg cost '
          '171.64 -> 178.16. Bought after the post-Q2 -5.1% drop.'},
 {'date': 'Aug 7, 2026',
  'action': 'BUY',
  't': 'ISRG',
  'shares': 5,
  'price': 377.595,
  'amount': 1887.98,
  'est': False,
  'note': 'Limit 378 day order filled 377.595 (order 368452, Aug 7 2026 11:04 PM BKK). ISRG 33 -> 38 sh, cost '
          '14,485.14 -> 16,381.68 incl 8.56 commission, avg cost 438.94 -> 431.10. Budget 20,000 now 81.9% used '
          '(3,618.32 headroom). Bought near the 52-week low ~379 vs a 603.88 high.'},
 {'date': 'Aug 10, 2026',
  'action': 'BUY',
  't': 'MSFT',
  'shares': 4,
  'price': 512.99,
  'amount': 2060.52,
  'est': False,
  'note': 'Filled during the Aug 10 US session (POEMS intraday screen stamped Aug 11, 12:20 PM BKK). 4 sh; cash debit '
          '2,060.52 INCLUDES the 8.56 commission, so 512.99/sh. Aug 10 range was 502.27-513.73 close 506.06 - a '
          'near-the-high fill. MSFT 113 -> 117 sh, cost 50,818.44 -> 52,878.96, avg 449.72 -> 451.96. Budget 60,000 '
          'now 88.1% used (7,121.04 headroom).'},
 {'date': 'Aug 10, 2026',
  'action': 'BUY',
  't': 'ANET',
  'shares': 10,
  'price': 192.7,
  'amount': 1935.56,
  'est': False,
  'note': 'Filled during the Aug 10 US session. 10 sh; cash debit 1,935.56 INCLUDES the 8.56 commission, so 192.70/sh. '
          'Aug 10 range 187.03-195.70 close 191.52. ANET 88 -> 98 sh, cost 13,650.01 -> 15,585.57, avg 155.11 -> '
          '159.04. Budget 20,000 now 77.9% used (4,414.43 headroom).'}]

SELLPLAN = ['IREN']

CASH = {'currency': 'USD', 'amount': 7330.98, 'asof': 'Aug 11, 2026', 'broker': 'POEMS (Phillip Securities)'}

REALIZED = [{'t': 'NBIS', 'g': 13, 'amount': 1404.52},
 {'t': 'IREN', 'g': 13, 'amount': 98.52},
 {'t': 'CRWV', 'g': 13, 'amount': -786.02},
 {'t': 'ORCL', 'g': 10, 'amount': -1961.73},
 {'t': 'IONQ', 'g': 9, 'amount': -772.46},
 {'t': 'SNPS', 'g': 10, 'amount': -263.2},
 {'t': 'NOW', 'g': 10, 'amount': -401.51},
 {'t': 'TTD', 'g': 10, 'amount': -421.11},
 {'t': 'MELI', 'g': 19, 'amount': 272.72},
 {'t': 'APP', 'g': 10, 'amount': -393.16},
 {'t': 'SOFI', 'g': 14, 'amount': -152.4},
 {'t': 'HOOD', 'g': 14, 'amount': 897.82}]

TOTALS = {'investedEverUSD': 600423.57,
 'investedEverTHB': 19365760,
 'soldCostUSD': 34025.62,
 'soldCostTHB': 1143440,
 'realizedUSD': -2478.01,
 'realizedTHB': -83170,
 'fx': 33.75,
 'inceptionDate': 'Jul 15, 2025'}

SOLD = [{'t': 'NBIS',
  'qty': 27,
  'costUSD': 3856.65,
  'costTHB': 129891.8,
  'glUSD': 1404.51,
  'glTHB': 47304.07,
  'g': 13,
  'lastDate': '2026-07-21'},
 {'t': 'HOOD',
  'qty': 25,
  'costUSD': 1933.56,
  'costTHB': 64909.61,
  'glUSD': 897.82,
  'glTHB': 30139.82,
  'g': 14,
  'lastDate': '2026-07-16'},
 {'t': 'MELI',
  'qty': 1,
  'costUSD': 1582.36,
  'costTHB': 53119.83,
  'glUSD': 272.72,
  'glTHB': 9155.21,
  'g': 19,
  'lastDate': '2026-07-16'},
 {'t': 'IREN',
  'qty': 45,
  'costUSD': 1735.17,
  'costTHB': 58440.36,
  'glUSD': 98.52,
  'glTHB': 3318.32,
  'g': 13,
  'lastDate': '2026-07-21'},
 {'t': 'SOFI',
  'qty': 105,
  'costUSD': 2006.51,
  'costTHB': 67358.56,
  'glUSD': -152.4,
  'glTHB': -5116.08,
  'g': 14,
  'lastDate': '2026-07-16'},
 {'t': 'SNPS',
  'qty': 4,
  'costUSD': 1934.56,
  'costTHB': 64943.18,
  'glUSD': -263.2,
  'glTHB': -8835.62,
  'g': 10,
  'lastDate': '2026-07-16'},
 {'t': 'APP',
  'qty': 4,
  'costUSD': 2060.56,
  'costTHB': 69193.6,
  'glUSD': -393.16,
  'glTHB': -13202.31,
  'g': 10,
  'lastDate': '2026-07-17'},
 {'t': 'NOW',
  'qty': 37,
  'costUSD': 4081.76,
  'costTHB': 137024.61,
  'glUSD': -401.51,
  'glTHB': -13478.62,
  'g': 10,
  'lastDate': '2026-07-16'},
 {'t': 'TTD',
  'qty': 80,
  'costUSD': 1951.7,
  'costTHB': 65518.57,
  'glUSD': -421.11,
  'glTHB': -14136.66,
  'g': 10,
  'lastDate': '2026-07-16'},
 {'t': 'IONQ',
  'qty': 81,
  'costUSD': 3569.26,
  'costTHB': 119998.38,
  'glUSD': -772.46,
  'glTHB': -25969.96,
  'g': 9,
  'lastDate': '2026-07-20'},
 {'t': 'CRWV',
  'qty': 17,
  'costUSD': 1997.56,
  'costTHB': 67078.06,
  'glUSD': -786.02,
  'glTHB': -26394.55,
  'g': 13,
  'lastDate': '2026-07-17'},
 {'t': 'ORCL',
  'qty': 44,
  'costUSD': 7315.98,
  'costTHB': 245963.36,
  'glUSD': -1961.73,
  'glTHB': -65953.47,
  'g': 10,
  'lastDate': '2026-07-20'}]

MONTHLY = [{'ym': '2025-07',
  'buyUSD': 19750.54,
  'buyLots': 4,
  'sellUSD': 0.0,
  'sellLots': 0,
  'realizedUSD': 0.0,
  'realizedTHB': 0,
  'byStock': [{'t': 'NVDA', 'buy': 10266.29, 'sell': 0.0, 'real': 0.0},
              {'t': 'GOOGL', 'buy': 9484.25, 'sell': 0.0, 'real': 0.0}],
  'byGroup': [{'g': 0, 'buy': 9484.25, 'sell': 0.0, 'real': 0.0}, {'g': 1, 'buy': 10266.29, 'sell': 0.0, 'real': 0.0}]},
 {'ym': '2025-08',
  'buyUSD': 36743.8,
  'buyLots': 9,
  'sellUSD': 0.0,
  'sellLots': 0,
  'realizedUSD': 0.0,
  'realizedTHB': 0,
  'byStock': [{'t': 'AMZN', 'buy': 8463.05, 'sell': 0.0, 'real': 0.0},
              {'t': 'MSFT', 'buy': 6099.24, 'sell': 0.0, 'real': 0.0},
              {'t': 'GOOGL', 'buy': 5675.18, 'sell': 0.0, 'real': 0.0},
              {'t': 'META', 'buy': 4509.63, 'sell': 0.0, 'real': 0.0},
              {'t': 'NVDA', 'buy': 4171.91, 'sell': 0.0, 'real': 0.0},
              {'t': 'AAPL', 'buy': 4088.73, 'sell': 0.0, 'real': 0.0},
              {'t': 'ASML', 'buy': 3736.06, 'sell': 0.0, 'real': 0.0}],
  'byGroup': [{'g': 0, 'buy': 28835.83, 'sell': 0.0, 'real': 0.0},
              {'g': 1, 'buy': 4171.91, 'sell': 0.0, 'real': 0.0},
              {'g': 2, 'buy': 3736.06, 'sell': 0.0, 'real': 0.0}]},
 {'ym': '2025-09',
  'buyUSD': 34133.89,
  'buyLots': 8,
  'sellUSD': 0.0,
  'sellLots': 0,
  'realizedUSD': 0.0,
  'realizedTHB': 0,
  'byStock': [{'t': 'ISRG', 'buy': 8949.11, 'sell': 0.0, 'real': 0.0},
              {'t': 'META', 'buy': 4569.76, 'sell': 0.0, 'real': 0.0},
              {'t': 'TSM', 'buy': 4200.97, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMZN', 'buy': 4148.86, 'sell': 0.0, 'real': 0.0},
              {'t': 'NVDA', 'buy': 4124.81, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMAT', 'buy': 4083.72, 'sell': 0.0, 'real': 0.0},
              {'t': 'MSFT', 'buy': 4056.66, 'sell': 0.0, 'real': 0.0}],
  'byGroup': [{'g': 0, 'buy': 12775.28, 'sell': 0.0, 'real': 0.0},
              {'g': 1, 'buy': 8325.78, 'sell': 0.0, 'real': 0.0},
              {'g': 2, 'buy': 4083.72, 'sell': 0.0, 'real': 0.0},
              {'g': 6, 'buy': 8949.11, 'sell': 0.0, 'real': 0.0}]},
 {'ym': '2025-10',
  'buyUSD': 20924.7,
  'buyLots': 5,
  'sellUSD': 0.0,
  'sellLots': 0,
  'realizedUSD': 0.0,
  'realizedTHB': 0,
  'byStock': [{'t': 'MSFT', 'buy': 8281.69, 'sell': 0.0, 'real': 0.0},
              {'t': 'META', 'buy': 4311.21, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMZN', 'buy': 4227.03, 'sell': 0.0, 'real': 0.0},
              {'t': 'GOOGL', 'buy': 4104.77, 'sell': 0.0, 'real': 0.0}],
  'byGroup': [{'g': 0, 'buy': 20924.7, 'sell': 0.0, 'real': 0.0}]},
 {'ym': '2025-11',
  'buyUSD': 17712.19,
  'buyLots': 6,
  'sellUSD': 0.0,
  'sellLots': 0,
  'realizedUSD': 0.0,
  'realizedTHB': 0,
  'byStock': [{'t': 'META', 'buy': 8873.95, 'sell': 0.0, 'real': 0.0},
              {'t': 'NVDA', 'buy': 3625.12, 'sell': 0.0, 'real': 0.0},
              {'t': 'GOOGL', 'buy': 3188.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'TSM', 'buy': 2024.56, 'sell': 0.0, 'real': 0.0}],
  'byGroup': [{'g': 0, 'buy': 12062.51, 'sell': 0.0, 'real': 0.0}, {'g': 1, 'buy': 5649.68, 'sell': 0.0, 'real': 0.0}]},
 {'ym': '2025-12',
  'buyUSD': 19329.84,
  'buyLots': 9,
  'sellUSD': 0.0,
  'sellLots': 0,
  'realizedUSD': 0.0,
  'realizedTHB': 0,
  'byStock': [{'t': 'AVGO', 'buy': 4426.52, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMZN', 'buy': 4184.12, 'sell': 0.0, 'real': 0.0},
              {'t': 'MSFT', 'buy': 2416.06, 'sell': 0.0, 'real': 0.0},
              {'t': 'GOOGL', 'buy': 2190.46, 'sell': 0.0, 'real': 0.0},
              {'t': 'LRCX', 'buy': 2108.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'NVDA', 'buy': 2065.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'ORCL', 'buy': 1938.56, 'sell': 0.0, 'real': 0.0}],
  'byGroup': [{'g': 0, 'buy': 8790.64, 'sell': 0.0, 'real': 0.0},
              {'g': 1, 'buy': 6492.08, 'sell': 0.0, 'real': 0.0},
              {'g': 2, 'buy': 2108.56, 'sell': 0.0, 'real': 0.0},
              {'g': 10, 'buy': 1938.56, 'sell': 0.0, 'real': 0.0}]},
 {'ym': '2026-01',
  'buyUSD': 75906.87,
  'buyLots': 36,
  'sellUSD': 0.0,
  'sellLots': 0,
  'realizedUSD': 0.0,
  'realizedTHB': 0,
  'byStock': [{'t': 'NVDA', 'buy': 14665.32, 'sell': 0.0, 'real': 0.0},
              {'t': 'TSM', 'buy': 12433.01, 'sell': 0.0, 'real': 0.0},
              {'t': 'MSFT', 'buy': 10646.8, 'sell': 0.0, 'real': 0.0},
              {'t': 'AVGO', 'buy': 10449.8, 'sell': 0.0, 'real': 0.0},
              {'t': 'GOOGL', 'buy': 6320.48, 'sell': 0.0, 'real': 0.0},
              {'t': 'MU', 'buy': 5875.68, 'sell': 0.0, 'real': 0.0},
              {'t': 'ASML', 'buy': 2868.28, 'sell': 0.0, 'real': 0.0},
              {'t': 'TSLA', 'buy': 2246.06, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMZN', 'buy': 2195.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'META', 'buy': 2168.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'INTC', 'buy': 2029.6, 'sell': 0.0, 'real': 0.0},
              {'t': 'ORCL', 'buy': 2016.16, 'sell': 0.0, 'real': 0.0},
              {'t': 'LRCX', 'buy': 1991.56, 'sell': 0.0, 'real': 0.0}],
  'byGroup': [{'g': 0, 'buy': 21331.4, 'sell': 0.0, 'real': 0.0},
              {'g': 1, 'buy': 39577.73, 'sell': 0.0, 'real': 0.0},
              {'g': 2, 'buy': 4859.84, 'sell': 0.0, 'real': 0.0},
              {'g': 3, 'buy': 5875.68, 'sell': 0.0, 'real': 0.0},
              {'g': 5, 'buy': 2246.06, 'sell': 0.0, 'real': 0.0},
              {'g': 10, 'buy': 2016.16, 'sell': 0.0, 'real': 0.0}]},
 {'ym': '2026-02',
  'buyUSD': 56458.99,
  'buyLots': 30,
  'sellUSD': 0.0,
  'sellLots': 0,
  'realizedUSD': 0.0,
  'realizedTHB': 0,
  'byStock': [{'t': 'NVDA', 'buy': 9438.8, 'sell': 0.0, 'real': 0.0},
              {'t': 'GOOGL', 'buy': 6328.33, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMD', 'buy': 5992.08, 'sell': 0.0, 'real': 0.0},
              {'t': 'MSFT', 'buy': 4342.12, 'sell': 0.0, 'real': 0.0},
              {'t': 'AVGO', 'buy': 4168.12, 'sell': 0.0, 'real': 0.0},
              {'t': 'PLTR', 'buy': 3597.36, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMZN', 'buy': 3546.3, 'sell': 0.0, 'real': 0.0},
              {'t': 'ASML', 'buy': 2752.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'TSLA', 'buy': 2058.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'CRWD', 'buy': 2055.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'AVAV', 'buy': 1676.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'IONQ', 'buy': 1566.96, 'sell': 0.0, 'real': 0.0},
              {'t': 'AAPL', 'buy': 1562.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMKR', 'buy': 1508.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'IREN', 'buy': 1493.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'KLAC', 'buy': 1474.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'TSM', 'buy': 1460.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'VRT', 'buy': 1435.88, 'sell': 0.0, 'real': 0.0}],
  'byGroup': [{'g': 0, 'buy': 15779.31, 'sell': 0.0, 'real': 0.0},
              {'g': 1, 'buy': 21059.56, 'sell': 0.0, 'real': 0.0},
              {'g': 2, 'buy': 5735.68, 'sell': 0.0, 'real': 0.0},
              {'g': 5, 'buy': 2058.56, 'sell': 0.0, 'real': 0.0},
              {'g': 8, 'buy': 5273.92, 'sell': 0.0, 'real': 0.0},
              {'g': 9, 'buy': 1566.96, 'sell': 0.0, 'real': 0.0},
              {'g': 13, 'buy': 1493.56, 'sell': 0.0, 'real': 0.0},
              {'g': 15, 'buy': 2055.56, 'sell': 0.0, 'real': 0.0},
              {'g': 18, 'buy': 1435.88, 'sell': 0.0, 'real': 0.0}]},
 {'ym': '2026-03',
  'buyUSD': 64002.08,
  'buyLots': 32,
  'sellUSD': 0.0,
  'sellLots': 0,
  'realizedUSD': 0.0,
  'realizedTHB': 0,
  'byStock': [{'t': 'NVDA', 'buy': 7631.39, 'sell': 0.0, 'real': 0.0},
              {'t': 'MSFT', 'buy': 7608.08, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMZN', 'buy': 5899.83, 'sell': 0.0, 'real': 0.0},
              {'t': 'AVGO', 'buy': 5883.84, 'sell': 0.0, 'real': 0.0},
              {'t': 'META', 'buy': 5566.2, 'sell': 0.0, 'real': 0.0},
              {'t': 'ORCL', 'buy': 3902.97, 'sell': 0.0, 'real': 0.0},
              {'t': 'CRDO', 'buy': 3881.37, 'sell': 0.0, 'real': 0.0},
              {'t': 'ISRG', 'buy': 3772.32, 'sell': 0.0, 'real': 0.0},
              {'t': 'LITE', 'buy': 2123.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'GOOGL', 'buy': 2088.64, 'sell': 0.0, 'real': 0.0},
              {'t': 'VRT', 'buy': 2088.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'MU', 'buy': 2033.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'BWXT', 'buy': 1994.06, 'sell': 0.0, 'real': 0.0},
              {'t': 'TSM', 'buy': 1981.96, 'sell': 0.0, 'real': 0.0},
              {'t': 'MRVL', 'buy': 1944.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'HOOD', 'buy': 1933.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'CRWD', 'buy': 1863.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMD', 'buy': 1804.06, 'sell': 0.0, 'real': 0.0}],
  'byGroup': [{'g': 0, 'buy': 21162.75, 'sell': 0.0, 'real': 0.0},
              {'g': 1, 'buy': 19245.81, 'sell': 0.0, 'real': 0.0},
              {'g': 3, 'buy': 2033.56, 'sell': 0.0, 'real': 0.0},
              {'g': 4, 'buy': 6004.93, 'sell': 0.0, 'real': 0.0},
              {'g': 6, 'buy': 3772.32, 'sell': 0.0, 'real': 0.0},
              {'g': 10, 'buy': 3902.97, 'sell': 0.0, 'real': 0.0},
              {'g': 14, 'buy': 1933.56, 'sell': 0.0, 'real': 0.0},
              {'g': 15, 'buy': 1863.56, 'sell': 0.0, 'real': 0.0},
              {'g': 16, 'buy': 1994.06, 'sell': 0.0, 'real': 0.0},
              {'g': 18, 'buy': 2088.56, 'sell': 0.0, 'real': 0.0}]},
 {'ym': '2026-04',
  'buyUSD': 57480.27,
  'buyLots': 30,
  'sellUSD': 0.0,
  'sellLots': 0,
  'realizedUSD': 0.0,
  'realizedTHB': 0,
  'byStock': [{'t': 'ETN', 'buy': 4129.92, 'sell': 0.0, 'real': 0.0},
              {'t': 'PANW', 'buy': 3760.62, 'sell': 0.0, 'real': 0.0},
              {'t': 'MSFT', 'buy': 3724.76, 'sell': 0.0, 'real': 0.0},
              {'t': 'META', 'buy': 3543.83, 'sell': 0.0, 'real': 0.0},
              {'t': 'TSM', 'buy': 2211.7, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMZN', 'buy': 2031.12, 'sell': 0.0, 'real': 0.0},
              {'t': 'CDNS', 'buy': 2007.28, 'sell': 0.0, 'real': 0.0},
              {'t': 'SOFI', 'buy': 2006.51, 'sell': 0.0, 'real': 0.0},
              {'t': 'NVDA', 'buy': 2004.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'ZETA', 'buy': 2004.34, 'sell': 0.0, 'real': 0.0},
              {'t': 'MRVL', 'buy': 2003.41, 'sell': 0.0, 'real': 0.0},
              {'t': 'NOW', 'buy': 2001.1, 'sell': 0.0, 'real': 0.0},
              {'t': 'CRWV', 'buy': 1997.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'ANET', 'buy': 1994.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMD', 'buy': 1979.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'IREN', 'buy': 1976.77, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMKR', 'buy': 1946.06, 'sell': 0.0, 'real': 0.0},
              {'t': 'SNPS', 'buy': 1934.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'ALAB', 'buy': 1930.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'NBIS', 'buy': 1918.09, 'sell': 0.0, 'real': 0.0},
              {'t': 'GOOGL', 'buy': 1888.36, 'sell': 0.0, 'real': 0.0},
              {'t': 'CRM', 'buy': 1888.07, 'sell': 0.0, 'real': 0.0},
              {'t': 'ORCL', 'buy': 1874.3, 'sell': 0.0, 'real': 0.0},
              {'t': 'AVGO', 'buy': 1718.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'MPWR', 'buy': 1538.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'ASML', 'buy': 1465.56, 'sell': 0.0, 'real': 0.0}],
  'byGroup': [{'g': 0, 'buy': 11188.07, 'sell': 0.0, 'real': 0.0},
              {'g': 1, 'buy': 9917.79, 'sell': 0.0, 'real': 0.0},
              {'g': 2, 'buy': 3411.62, 'sell': 0.0, 'real': 0.0},
              {'g': 4, 'buy': 3925.12, 'sell': 0.0, 'real': 0.0},
              {'g': 10, 'buy': 11709.65, 'sell': 0.0, 'real': 0.0},
              {'g': 13, 'buy': 5892.42, 'sell': 0.0, 'real': 0.0},
              {'g': 14, 'buy': 2006.51, 'sell': 0.0, 'real': 0.0},
              {'g': 15, 'buy': 3760.62, 'sell': 0.0, 'real': 0.0},
              {'g': 17, 'buy': 5668.48, 'sell': 0.0, 'real': 0.0}]},
 {'ym': '2026-05',
  'buyUSD': 68940.1,
  'buyLots': 36,
  'sellUSD': 0.0,
  'sellLots': 0,
  'realizedUSD': 0.0,
  'realizedTHB': 0,
  'byStock': [{'t': 'GOOGL', 'buy': 5881.88, 'sell': 0.0, 'real': 0.0},
              {'t': 'ANET', 'buy': 5815.25, 'sell': 0.0, 'real': 0.0},
              {'t': 'NVDA', 'buy': 5620.81, 'sell': 0.0, 'real': 0.0},
              {'t': 'TSM', 'buy': 4148.12, 'sell': 0.0, 'real': 0.0},
              {'t': 'AAPL', 'buy': 3820.35, 'sell': 0.0, 'real': 0.0},
              {'t': 'ETN', 'buy': 3749.63, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMZN', 'buy': 3740.6, 'sell': 0.0, 'real': 0.0},
              {'t': 'GEV', 'buy': 2147.3, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMAT', 'buy': 2054.39, 'sell': 0.0, 'real': 0.0},
              {'t': 'ONDS', 'buy': 2030.54, 'sell': 0.0, 'real': 0.0},
              {'t': 'CEG', 'buy': 2016.3, 'sell': 0.0, 'real': 0.0},
              {'t': 'APH', 'buy': 2004.4, 'sell': 0.0, 'real': 0.0},
              {'t': 'IONQ', 'buy': 2002.3, 'sell': 0.0, 'real': 0.0},
              {'t': 'VST', 'buy': 1997.93, 'sell': 0.0, 'real': 0.0},
              {'t': 'QCOM', 'buy': 1959.96, 'sell': 0.0, 'real': 0.0},
              {'t': 'TTD', 'buy': 1951.7, 'sell': 0.0, 'real': 0.0},
              {'t': 'NBIS', 'buy': 1938.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'PLTR', 'buy': 1935.73, 'sell': 0.0, 'real': 0.0},
              {'t': 'ORCL', 'buy': 1919.31, 'sell': 0.0, 'real': 0.0},
              {'t': 'CRDO', 'buy': 1853.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'META', 'buy': 1814.4, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMD', 'buy': 1811.36, 'sell': 0.0, 'real': 0.0},
              {'t': 'TER', 'buy': 1724.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'MU', 'buy': 1710.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'MSFT', 'buy': 1708.24, 'sell': 0.0, 'real': 0.0},
              {'t': 'MELI', 'buy': 1582.36, 'sell': 0.0, 'real': 0.0}],
  'byGroup': [{'g': 0, 'buy': 16965.47, 'sell': 0.0, 'real': 0.0},
              {'g': 1, 'buy': 13540.25, 'sell': 0.0, 'real': 0.0},
              {'g': 2, 'buy': 2054.39, 'sell': 0.0, 'real': 0.0},
              {'g': 3, 'buy': 1710.56, 'sell': 0.0, 'real': 0.0},
              {'g': 4, 'buy': 9673.21, 'sell': 0.0, 'real': 0.0},
              {'g': 6, 'buy': 1724.56, 'sell': 0.0, 'real': 0.0},
              {'g': 8, 'buy': 3966.27, 'sell': 0.0, 'real': 0.0},
              {'g': 9, 'buy': 2002.3, 'sell': 0.0, 'real': 0.0},
              {'g': 10, 'buy': 3871.01, 'sell': 0.0, 'real': 0.0},
              {'g': 13, 'buy': 1938.56, 'sell': 0.0, 'real': 0.0},
              {'g': 16, 'buy': 4014.23, 'sell': 0.0, 'real': 0.0},
              {'g': 17, 'buy': 5896.93, 'sell': 0.0, 'real': 0.0},
              {'g': 19, 'buy': 1582.36, 'sell': 0.0, 'real': 0.0}]},
 {'ym': '2026-06',
  'buyUSD': 72759.17,
  'buyLots': 37,
  'sellUSD': 0.0,
  'sellLots': 0,
  'realizedUSD': 0.0,
  'realizedTHB': 0,
  'byStock': [{'t': 'TSM', 'buy': 7509.37, 'sell': 0.0, 'real': 0.0},
              {'t': 'ANET', 'buy': 5902.03, 'sell': 0.0, 'real': 0.0},
              {'t': 'NVDA', 'buy': 5827.6, 'sell': 0.0, 'real': 0.0},
              {'t': 'MU', 'buy': 5822.22, 'sell': 0.0, 'real': 0.0},
              {'t': 'GOOGL', 'buy': 4140.2, 'sell': 0.0, 'real': 0.0},
              {'t': 'CGNX', 'buy': 4067.62, 'sell': 0.0, 'real': 0.0},
              {'t': 'ORCL', 'buy': 3978.3, 'sell': 0.0, 'real': 0.0},
              {'t': 'MRVL', 'buy': 3924.69, 'sell': 0.0, 'real': 0.0},
              {'t': 'LITE', 'buy': 3768.7, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMD', 'buy': 2202.16, 'sell': 0.0, 'real': 0.0},
              {'t': 'VST', 'buy': 2089.06, 'sell': 0.0, 'real': 0.0},
              {'t': 'NOW', 'buy': 2080.66, 'sell': 0.0, 'real': 0.0},
              {'t': 'KLAC', 'buy': 2036.13, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMAT', 'buy': 2023.96, 'sell': 0.0, 'real': 0.0},
              {'t': 'ASML', 'buy': 1994.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'CRDO', 'buy': 1966.06, 'sell': 0.0, 'real': 0.0},
              {'t': 'AAPL', 'buy': 1965.97, 'sell': 0.0, 'real': 0.0},
              {'t': 'TER', 'buy': 1961.06, 'sell': 0.0, 'real': 0.0},
              {'t': 'RKLB', 'buy': 1942.58, 'sell': 0.0, 'real': 0.0},
              {'t': 'AVGO', 'buy': 1924.68, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMZN', 'buy': 1924.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'ALAB', 'buy': 1874.91, 'sell': 0.0, 'real': 0.0},
              {'t': 'META', 'buy': 1832.09, 'sell': 0.0, 'real': 0.0}],
  'byGroup': [{'g': 0, 'buy': 9862.82, 'sell': 0.0, 'real': 0.0},
              {'g': 1, 'buy': 21388.5, 'sell': 0.0, 'real': 0.0},
              {'g': 2, 'buy': 6054.65, 'sell': 0.0, 'real': 0.0},
              {'g': 3, 'buy': 5822.22, 'sell': 0.0, 'real': 0.0},
              {'g': 4, 'buy': 13511.7, 'sell': 0.0, 'real': 0.0},
              {'g': 6, 'buy': 6028.68, 'sell': 0.0, 'real': 0.0},
              {'g': 7, 'buy': 1942.58, 'sell': 0.0, 'real': 0.0},
              {'g': 10, 'buy': 6058.96, 'sell': 0.0, 'real': 0.0},
              {'g': 16, 'buy': 2089.06, 'sell': 0.0, 'real': 0.0}]},
 {'ym': '2026-07',
  'buyUSD': 34632.83,
  'buyLots': 18,
  'sellUSD': 31547.61,
  'sellLots': 13,
  'realizedUSD': -2478.01,
  'realizedTHB': -83170,
  'byStock': [{'t': 'AMZN', 'buy': 6233.64, 'sell': 0.0, 'real': 0.0},
              {'t': 'NVDA', 'buy': 4025.07, 'sell': 0.0, 'real': 0.0},
              {'t': 'ASML', 'buy': 3577.04, 'sell': 0.0, 'real': 0.0},
              {'t': 'MU', 'buy': 3516.39, 'sell': 0.0, 'real': 0.0},
              {'t': 'AVAV', 'buy': 2081.74, 'sell': 0.0, 'real': 0.0},
              {'t': 'APP', 'buy': 2060.56, 'sell': 1667.4, 'real': -393.16},
              {'t': 'CEG', 'buy': 2024.68, 'sell': 0.0, 'real': 0.0},
              {'t': 'VST', 'buy': 1991.06, 'sell': 0.0, 'real': 0.0},
              {'t': 'MSFT', 'buy': 1934.7, 'sell': 0.0, 'real': 0.0},
              {'t': 'SKHY', 'buy': 1913.26, 'sell': 0.0, 'real': 0.0},
              {'t': 'GOOGL', 'buy': 1797.92, 'sell': 0.0, 'real': 0.0},
              {'t': 'ISRG', 'buy': 1763.71, 'sell': 0.0, 'real': 0.0},
              {'t': 'IBM', 'buy': 1713.07, 'sell': 0.0, 'real': 0.0},
              {'t': 'ORCL', 'buy': 0.0, 'sell': 5354.25, 'real': -1961.73},
              {'t': 'NBIS', 'buy': 0.0, 'sell': 5261.16, 'real': 1404.51},
              {'t': 'NOW', 'buy': 0.0, 'sell': 3680.25, 'real': -401.51},
              {'t': 'HOOD', 'buy': 0.0, 'sell': 2831.38, 'real': 897.82},
              {'t': 'IONQ', 'buy': 0.0, 'sell': 2796.8, 'real': -772.46},
              {'t': 'MELI', 'buy': 0.0, 'sell': 1855.08, 'real': 272.72},
              {'t': 'SOFI', 'buy': 0.0, 'sell': 1854.11, 'real': -152.4},
              {'t': 'IREN', 'buy': 0.0, 'sell': 1833.69, 'real': 98.52},
              {'t': 'SNPS', 'buy': 0.0, 'sell': 1671.36, 'real': -263.2},
              {'t': 'TTD', 'buy': 0.0, 'sell': 1530.59, 'real': -421.11},
              {'t': 'CRWV', 'buy': 0.0, 'sell': 1211.54, 'real': -786.02}],
  'byGroup': [{'g': 0, 'buy': 9966.26, 'sell': 0.0, 'real': 0.0},
              {'g': 1, 'buy': 4025.07, 'sell': 0.0, 'real': 0.0},
              {'g': 2, 'buy': 3577.04, 'sell': 0.0, 'real': 0.0},
              {'g': 3, 'buy': 5429.65, 'sell': 0.0, 'real': 0.0},
              {'g': 6, 'buy': 1763.71, 'sell': 0.0, 'real': 0.0},
              {'g': 8, 'buy': 2081.74, 'sell': 0.0, 'real': 0.0},
              {'g': 9, 'buy': 1713.07, 'sell': 2796.8, 'real': -772.46},
              {'g': 10, 'buy': 2060.56, 'sell': 13903.85, 'real': -3440.71},
              {'g': 13, 'buy': 0.0, 'sell': 8306.39, 'real': 717.01},
              {'g': 14, 'buy': 0.0, 'sell': 4685.49, 'real': 745.42},
              {'g': 16, 'buy': 4015.74, 'sell': 0.0, 'real': 0.0},
              {'g': 19, 'buy': 0.0, 'sell': 1855.08, 'real': 272.72}]},
 {'ym': '2026-08',
  'buyUSD': 21648.3,
  'buyLots': 12,
  'sellUSD': 0.0,
  'sellLots': 0,
  'realizedUSD': 0.0,
  'realizedTHB': 0,
  'byStock': [{'t': 'MSFT', 'buy': 2060.52, 'sell': 0.0, 'real': 0.0},
              {'t': 'META', 'buy': 2049.88, 'sell': 0.0, 'real': 0.0},
              {'t': 'AMZN', 'buy': 2006.08, 'sell': 0.0, 'real': 0.0},
              {'t': 'NVDA', 'buy': 1983.61, 'sell': 0.0, 'real': 0.0},
              {'t': 'ANET', 'buy': 1935.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'ISRG', 'buy': 1887.98, 'sell': 0.0, 'real': 0.0},
              {'t': 'TSM', 'buy': 1873.23, 'sell': 0.0, 'real': 0.0},
              {'t': 'CRM', 'buy': 1861.84, 'sell': 0.0, 'real': 0.0},
              {'t': 'LITE', 'buy': 1717.92, 'sell': 0.0, 'real': 0.0},
              {'t': 'FN', 'buy': 1604.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'AAOI', 'buy': 1338.56, 'sell': 0.0, 'real': 0.0},
              {'t': 'COHR', 'buy': 1328.56, 'sell': 0.0, 'real': 0.0}],
  'byGroup': [{'g': 0, 'buy': 6116.48, 'sell': 0.0, 'real': 0.0},
              {'g': 1, 'buy': 3856.84, 'sell': 0.0, 'real': 0.0},
              {'g': 4, 'buy': 7925.16, 'sell': 0.0, 'real': 0.0},
              {'g': 6, 'buy': 1887.98, 'sell': 0.0, 'real': 0.0},
              {'g': 10, 'buy': 1861.84, 'sell': 0.0, 'real': 0.0}]}]

THBFX = {'GOOGL': 32.2446,
 'NVDA': 32.084,
 'AMZN': 32.3066,
 'AAPL': 32.4331,
 'ASML': 32.372,
 'MSFT': 32.0119,
 'META': 32.2596,
 'AMAT': 32.1085,
 'ISRG': 32.1878,
 'TSM': 31.9974,
 'AVGO': 31.6503,
 'ORCL': 32.2708,
 'LRCX': 31.4491,
 'TSLA': 31.5761,
 'MU': 32.1376,
 'INTC': 30.96,
 'AMD': 32.0515,
 'PLTR': 31.79,
 'IREN': 31.993,
 'AMKR': 31.7261,
 'KLAC': 32.1216,
 'VRT': 31.4889,
 'CRWD': 31.9405,
 'AVAV': 32.3497,
 'IONQ': 31.9649,
 'LITE': 32.5803,
 'CRDO': 32.4342,
 'HOOD': 32.16,
 'BWXT': 32.42,
 'MRVL': 32.5019,
 'PANW': 32.5467,
 'NBIS': 32.3339,
 'CRM': 32.02,
 'ETN': 32.3317,
 'NOW': 32.3068,
 'CRWV': 31.9,
 'ZETA': 32.0,
 'SOFI': 32.22,
 'CDNS': 32.32,
 'SNPS': 32.48,
 'MPWR': 32.71,
 'ANET': 32.6011,
 'ALAB': 32.4697,
 'QCOM': 32.53,
 'TTD': 32.12,
 'MELI': 32.25,
 'APH': 32.34,
 'VST': 32.9329,
 'GEV': 32.66,
 'TER': 32.6972,
 'ONDS': 32.54,
 'CEG': 33.0112,
 'CGNX': 33.0687,
 'RKLB': 33.31,
 'IBM': 33.2,
 'APP': 33.44,
 'SKHY': 33.75}

M3 = {'GOOGL': {'pfcf': 72.54, 'ev': 21.6, 'de': 0.18},
 'AMZN': {'pfcf': None, 'ev': 16.66, 'de': 0.4},
 'MSFT': {'pfcf': 43.88, 'ev': 14.98, 'de': 0.3},
 'META': {'pfcf': 39.9, 'ev': 14.85, 'de': 0.43},
 'AAPL': {'pfcf': 35.15, 'ev': 28.23, 'de': 0.78},
 'ORCL': {'pfcf': None, 'ev': 16.33, 'de': 3.89},
 'NVDA': {'pfcf': 42.14, 'ev': 30.07, 'de': 0.07},
 'TSM': {'pfcf': 54.38, 'ev': 18.5, 'de': 0.15},
 'AVGO': {'pfcf': 57.14, 'ev': 45.56, 'de': 0.74},
 'AMD': {'pfcf': 94.5, 'ev': 120.07, 'de': 0.05},
 'MRVL': {'pfcf': 110.19, 'ev': 69.19, 'de': 0.29},
 'INTC': {'pfcf': None, 'ev': 55.86, 'de': 0.36},
 'QCOM': {'pfcf': 14.81, 'ev': 14.66, 'de': 0.56},
 'ARM': {'pfcf': 309.19, 'ev': 281.38, 'de': 0.06},
 'ASML': {'pfcf': 60.95, 'ev': 44.88, 'de': 0.09},
 'AMAT': {'pfcf': 82.31, 'ev': 47.31, 'de': 0.3},
 'CRWD': {'pfcf': 127.47, 'ev': 3171.48, 'de': 0.18},
 'PANW': {'pfcf': 69.75, 'ev': 121.08, 'de': 0.08},
 'LRCX': {'pfcf': 66.5, 'ev': 50.75, 'de': 0.35},
 'KLAC': {'pfcf': 69.72, 'ev': 48.14, 'de': 0.93},
 'AMKR': {'pfcf': 99.27, 'ev': 13.7, 'de': 0.35},
 'MU': {'pfcf': 41.4, 'ev': 15.54, 'de': 0.06},
 'SKHY': {'pfcf': 33.37, 'ev': 15.36, 'de': 0.13},
 'STX': {'pfcf': 54.45, 'ev': 39.12, 'de': 1.65},
 'SNDK': {'pfcf': 53.1, 'ev': 41.43, 'de': 0.02},
 'WDC': {'pfcf': 66.05, 'ev': 48.46, 'de': 0.18},
 'ANET': {'pfcf': 46.22, 'ev': 50.19, 'de': None},
 'CRDO': {'pfcf': 107.31, 'ev': 88.02, 'de': 0.01},
 'LITE': {'pfcf': 566.23, 'ev': 127.15, 'de': 1.11},
 'ALAB': {'pfcf': 224.04, 'ev': 238.13, 'de': 0.03},
 'APH': {'pfcf': 41.84, 'ev': 25.45, 'de': 1.33},
 'GLW': {'pfcf': 89.93, 'ev': 37.08, 'de': 0.8},
 'FN': {'pfcf': 401.79, 'ev': 36.4, 'de': 0.0},
 'AAOI': {'pfcf': None, 'ev': None, 'de': 0.18},
 'ETN': {'pfcf': 42.37, 'ev': 29.24, 'de': 1.02},
 'VST': {'pfcf': 31.18, 'ev': 11.21, 'de': 3.29},
 'VRT': {'pfcf': 50.71, 'ev': 48.86, 'de': 0.77},
 'CEG': {'pfcf': 87.42, 'ev': 15.05, 'de': 0.65},
 'GEV': {'pfcf': 35.65, 'ev': 87.4, 'de': 0.21},
 'BWXT': {'pfcf': 48.56, 'ev': 36.6, 'de': 1.51},
 'MPWR': {'pfcf': 113.97, 'ev': 75.6, 'de': 0.01},
 'ISRG': {'pfcf': None, 'ev': 26.22, 'de': None},
 'CGNX': {'pfcf': 48.03, 'ev': 42.06, 'de': 0.04},
 'TSLA': {'pfcf': 209.58, 'ev': 108.16, 'de': 0.11},
 'TER': {'pfcf': 104.55, 'ev': 49.53, 'de': 0.03},
 'OUST': {'pfcf': None, 'ev': None, 'de': 0.04},
 'ROK': {'pfcf': 38.08, 'ev': 27.74, 'de': 1.13},
 'RKLB': {'pfcf': None, 'ev': None, 'de': 0.06},
 'ASTS': {'pfcf': None, 'ev': None, 'de': 1.26},
 'SPCX': {'pfcf': None, 'ev': 320.35, 'de': 0.31},
 'PLTR': {'pfcf': 90.16, 'ev': 144.09, 'de': 0.02},
 'AVAV': {'pfcf': None, 'ev': 39.99, 'de': 0.19},
 'ONDS': {'pfcf': None, 'ev': None, 'de': 0.02},
 'MDB': {'pfcf': 40.54, 'ev': None, 'de': 0.02},
 'IONQ': {'pfcf': None, 'ev': None, 'de': 0.01},
 'ZETA': {'pfcf': 26.94, 'ev': 55.94, 'de': 0.21},
 'ADBE': {'pfcf': 9.19, 'ev': 9.74, 'de': 0.61},
 'NOW': {'pfcf': 23.83, 'ev': 32.43, 'de': 0.68},
 'SNOW': {'pfcf': 80.87, 'ev': None, 'de': 1.43},
 'DDOG': {'pfcf': 69.04, 'ev': None, 'de': 0.29},
 'CDNS': {'pfcf': 64.96, 'ev': 51.84, 'de': 0.47},
 'CRM': {'pfcf': 9.11, 'ev': 12.73, 'de': 1.24},
 'IBM': {'pfcf': 14.01, 'ev': 14.24, 'de': 1.89},
 'SBUX': {'pfcf': 32.81, 'ev': 27.81, 'de': None},
 'MCD': {'pfcf': 26.6, 'ev': 16.21, 'de': None},
 'UNH': {'pfcf': 16.59, 'ev': 17.99, 'de': 0.69},
 'IREN': {'pfcf': None, 'ev': 112.14, 'de': 1.49},
 'NBIS': {'pfcf': None, 'ev': None, 'de': 1.32},
 'CRWV': {'pfcf': None, 'ev': 25.78, 'de': 7.39},
 'JNJ': {'pfcf': 34.7, 'ev': 25.14, 'de': 0.68},
 'KO': {'pfcf': 27.8, 'ev': 24.51, 'de': 1.25},
 'YUM': {'pfcf': 25.76, 'ev': 18.26, 'de': None},
 'TEM': {'pfcf': None, 'ev': None, 'de': 3.25},
 'OKLO': {'pfcf': None, 'ev': None, 'de': 0.0},
 'SMR': {'pfcf': None, 'ev': None, 'de': 0.01},
 'NNE': {'pfcf': None, 'ev': None, 'de': 0.0},
 'CCJ': {'pfcf': 94.99, 'ev': 68.18, 'de': 0.17},
 'TE': {'pfcf': None, 'ev': 2.09, 'de': 1.78},
 'EOSE': {'pfcf': None, 'ev': 1.55, 'de': None},
 'ENPH': {'pfcf': 33.34, 'ev': 4.52, 'de': 0.56},
 'NEE': {'pfcf': None, 'ev': 294.58, 'de': 1.62},
 'BE': {'pfcf': 75.97, 'ev': 118.17, 'de': 1.71},
 'XE': {'pfcf': None, 'ev': None, 'de': 0.03},
 'V': {'pfcf': 31.63, 'ev': 24.66, 'de': 0.67},
 'MA': {'pfcf': 31.17, 'ev': 23.58, 'de': 4.4},
 'PYPL': {'pfcf': 9.0, 'ev': 7.43, 'de': 0.58},
 'HOOD': {'pfcf': 28.24, 'ev': 29.41, 'de': 0.23},
 'SOFI': {'pfcf': None, 'ev': None, 'de': 0.3},
 'AFRM': {'pfcf': 30.31, 'ev': 49.35, 'de': 2.4},
 'NU': {'pfcf': None, 'ev': None, 'de': None},
 'SHOP': {'pfcf': 69.76, 'ev': 84.45, 'de': 0.01},
 'MELI': {'pfcf': 7.73, 'ev': 24.88, 'de': 1.7},
 'SE': {'pfcf': 12.85, 'ev': 25.02, 'de': 0.28},
 'UBER': {'pfcf': 14.14, 'ev': 20.39, 'de': 0.52},
 'APLD': {'pfcf': None, 'ev': None, 'de': 1.1},
 'CIFR': {'pfcf': None, 'ev': None, 'de': 9.7},
 'HUT': {'pfcf': None, 'ev': None, 'de': 0.25},
 'DELL': {'pfcf': 29.94, 'ev': 22.13, 'de': None},
 'HPE': {'pfcf': 15.83, 'ev': 15.74, 'de': 0.84},
 'SMCI': {'pfcf': None, 'ev': 17.12, 'de': 1.21},
 'LUNR': {'pfcf': None, 'ev': None, 'de': 0.62},
 'AEHR': {'pfcf': None, 'ev': None, 'de': 0.05},
 'ZS': {'pfcf': 23.89, 'ev': 98.99, 'de': 0.79},
 'NET': {'pfcf': 295.26, 'ev': None, 'de': 2.18},
 'FTNT': {'pfcf': 45.84, 'ev': 40.05, 'de': 0.57},
 'POET': {'pfcf': None, 'ev': None, 'de': 0.02},
 'AMPX': {'pfcf': None, 'ev': None, 'de': 0.06},
 'NOK': {'pfcf': 83.09, 'ev': 24.02, 'de': 0.16},
 'APP': {'pfcf': 30.73, 'ev': 28.07, 'de': 1.63},
 'COHR': {'pfcf': None, 'ev': 61.74, 'de': 0.31},
 'LMND': {'pfcf': 506.44, 'ev': None, 'de': 0.41},
 'HNGE': {'pfcf': 21.44, 'ev': 222.24, 'de': 0.02},
 'AIRJ': {'pfcf': None, 'ev': None, 'de': 0.0},
 'RBRK': {'pfcf': 55.76, 'ev': None, 'de': None},
 'PL': {'pfcf': 174.79, 'ev': None, 'de': 1.1},
 'PWR': {'pfcf': 45.54, 'ev': 35.97, 'de': 0.68},
 'POWL': {'pfcf': 40.0, 'ev': 29.92, 'de': None},
 'NVT': {'pfcf': 45.74, 'ev': 26.27, 'de': 0.41},
 'HUBB': {'pfcf': 29.44, 'ev': 21.23, 'de': 1.42},
 'VEEV': {'pfcf': 21.15, 'ev': 27.6, 'de': 0.01},
 'CORZ': {'pfcf': None, 'ev': None, 'de': None},
 'FLNC': {'pfcf': None, 'ev': None, 'de': 0.96},
 'LWLG': {'pfcf': None, 'ev': None, 'de': 0.03},
 'AXTI': {'pfcf': None, 'ev': 1042.39, 'de': 0.09},
 'AXON': {'pfcf': 315.27, 'ev': 324.07, 'de': 0.5},
 'TEAM': {'pfcf': 21.19, 'ev': 185.08, 'de': 1.16},
 'TMDX': {'pfcf': 34.32, 'ev': 26.33, 'de': 1.67},
 'CRSP': {'pfcf': None, 'ev': None, 'de': 0.45},
 'NRG': {'pfcf': 72.09, 'ev': 12.68, 'de': 4.83},
 'SOUN': {'pfcf': None, 'ev': None, 'de': 0.01},
 'UUUU': {'pfcf': None, 'ev': None, 'de': 0.85},
 'LEU': {'pfcf': None, 'ev': 179.64, 'de': 1.39},
 'MYRG': {'pfcf': 27.17, 'ev': 17.56, 'de': 0.09},
 'STRL': {'pfcf': 34.71, 'ev': 23.96, 'de': 0.24},
 'FSLR': {'pfcf': 19.1, 'ev': 11.37, 'de': 0.02},
 'ON': {'pfcf': 21.06, 'ev': 22.24, 'de': 0.65},
 'KEEL': {'pfcf': None, 'ev': None, 'de': 3.12},
 'SIDU': {'pfcf': None, 'ev': None, 'de': 0.01},
 'HYLN': {'pfcf': None, 'ev': None, 'de': 0.02},
 'POWI': {'pfcf': 45.53, 'ev': 80.37, 'de': 0.0},
 'RCAT': {'pfcf': None, 'ev': None, 'de': 0.03},
 'LMT': {'pfcf': 15.55, 'ev': 14.06, 'de': 2.34},
 'RTX': {'pfcf': 26.35, 'ev': 22.25, 'de': 0.57},
 'NOC': {'pfcf': 22.27, 'ev': 15.6, 'de': 0.95},
 'NFLX': {'pfcf': 27.68, 'ev': 21.47, 'de': 0.55},
 'BKNG': {'pfcf': 16.89, 'ev': 16.69, 'de': None},
 'WDAY': {'pfcf': 14.93, 'ev': 31.89, 'de': 0.57},
 'CAT': {'pfcf': 43.04, 'ev': 27.56, 'de': 2.33},
 'DE': {'pfcf': 44.48, 'ev': 19.72, 'de': 2.35},
 'GE': {'pfcf': 45.73, 'ev': 37.22, 'de': 1.13},
 'MMM': {'pfcf': 23.67, 'ev': 16.99, 'de': 4.38},
 'MCHP': {'pfcf': 41.4, 'ev': 34.78, 'de': 0.84},
 'NXPI': {'pfcf': 21.54, 'ev': 13.66, 'de': 0.93},
 'CSCO': {'pfcf': 40.6, 'ev': 29.59, 'de': 0.68},
 'CIEN': {'pfcf': 70.11, 'ev': 86.96, 'de': 0.55},
 'MTSI': {'pfcf': 116.32, 'ev': 85.36, 'de': 0.27},
 'VIAV': {'pfcf': 210.77, 'ev': 91.17, 'de': 0.44},
 'TSEM': {'pfcf': 94.71, 'ev': 98.21, 'de': 0.05},
 'GFS': {'pfcf': 34.5, 'ev': 12.61, 'de': 0.14},
 'FORM': {'pfcf': 67.86, 'ev': 58.2, 'de': 0.03},
 'COHU': {'pfcf': 69.66, 'ev': 254.16, 'de': 0.43}}

METRICS3 = M3  # alias: per-ticker pfcf / ev (EV/EBITDA) / de (Debt/Equity)

def rows():
    return STOCKS

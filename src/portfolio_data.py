...
ASOF = ('Aug 31, 2026 - close. Daily price refresh: 178 of 181 tickers brought to the Monday Aug 31, 2026 close (price, '
 "mcapB, pxd; mcapB scaled by each ticker's exact price ratio). METHOD per playbook: stockanalysis.com quote page "
 'taken only when stamped "Aug 31, 2026, 4:00 PM EDT" AND its stated previous close tied to the history table\'s Aug '
 '28 close; history-table fallback (sanity-checked in-range, normal volume) where the quote page was frozen intraday: '
 'COHU 46.10, VPG 61.69, TSEM 202.84, HNGE 92.56, CORZ 16.30, GLW 148.73, NXPI 224.64. Google Finance (explicit '
 '"Closed: Aug 31, 4:00 PM GMT-4" stamps) used for AMBQ 58.31 and HYLN 3.47 where both stockanalysis pages were stale. '
 'Moves >=6% independently confirmed: TE -7.10% (Google Finance) and XE +7.14% (Google Finance); both passed with '
 '--allow. DISCARDED internally-impossible stockanalysis /history/ rows: OKTA (close 173.12 > high 172.87), WULF '
 '(close 14.86 < low 14.88), AMBA (close 70.34 < low 70.58), plus ~25 partial-session history rows where the 4:00 PM '
 'quote page was taken instead (largest gaps: MPWR 1267.77 vs 1254.42, RBRK 93.04 vs 91.71, FN 412.93 vs 410.91, SNOW '
 '331.43 vs 325.99). UNRESOLVED, left at their verified Aug 28 closes with pxd 2026-08-28: CLFD 29.53 and POWI 51.51 '
 '(both quote pages frozen intraday all evening, history tables weeks stale, every second source cached Aug 28 or '
 'older); CRCL 87.14 - its Aug 31 close printed 95.55 (+9.65%) on both stockanalysis pages in exact agreement, but no '
 'independent second source was reachable before the session search budget ran out, and the playbook forbids shipping '
 'an unconfirmed 6%+ move, so it is held back deliberately and will look 9.65% stale until the next run confirms it. '
 'Fundamentals, M3, POS, TRADES untouched (owned by other jobs). | Sep 1, 2026 - MANUAL FULL REFRESH MERGED WITH THE '
 "SCHEDULED JOB'S RUN; ELEVEN TICKERS RECONCILED AND EIGHT 52-WEEK BOUNDS CORRECTED. TWO INDEPENDENT RUNS OF THE SAME "
 'REFRESH WERE COMPARED TICKER BY TICKER. The scheduled daily job (commit b700f11) reached 178 of 181. A separate '
 "manual run, launched from the chat at the user's request, reached 181 of 181 with two independent Aug-31-stamped "
 "sources each. THE TWO RUNS AGREE EXACTLY ON 170 OF 181 TICKERS - the strongest corroboration this board's pricing "
 "has ever had, since neither run saw the other's output. This entry records the eleven that differed and how each was "
 'resolved. THREE THE SCHEDULED JOB COULD NOT RESOLVE, NOW CLOSED: CLFD 29.53 (pxd 2026-08-28) -> 29.01, POWI 51.51 '
 "(pxd 2026-08-28) -> 51.38, CRCL 87.14 (pxd 2026-08-28) -> 95.55. ⚠️ CRCL IS THE NOTABLE ONE AND THE JOB'S CAUTION "
 'WAS CORRECT AT THE TIME: the job SAW 95.55 on both stockanalysis pages in exact agreement but could not reach an '
 'independent second source before its search budget ran out, and the playbook forbids shipping an unconfirmed 6%-plus '
 'move, so it deliberately held the stale Aug 28 price. The manual run supplied the missing independence - MarketBeat '
 "stamped '08/31/2026 04:00 PM Eastern' and stockevents - AND a dated news article that independently quotes CRCL 'at "
 "roughly $95.55'. CRCL 95.55 is therefore now FOUR-WAY confirmed and the +9.65% move is sourced to bitcoin breaking "
 'above $71,000 lifting the crypto-linked equities. This is exactly how the two-run design is supposed to work: the '
 'job refused to guess, and the second run finished the evidence. ONE GENUINE CORRECTION: TSEM 202.84 -> 205.05. The '
 'manual run explicitly DISCARDED 202.84 because MarketBeat at 4:00 PM and stockevents both printed 205.05, and '
 "because the stockanalysis history row's OWN high and low (211.15 / 202.61) agreed with MarketBeat while only its "
 'close did not - a single-field error, not a different session. SEVEN CENT-LEVEL DISAGREEMENTS, both values recorded, '
 'never averaged, manual run adopted: EOSE 3.215 vs 3.22, GE 335.59 vs 335.71, OSS 10.41 vs 10.42, PENG 49.04 vs '
 '49.08, ROK 424.43 vs 424.68, SPCX 143.69 vs 143.73, VPG 61.69 vs 61.81. ⚠️ TO BE CLEAR: on these seven BOTH runs are '
 'defensible and neither is demonstrably wrong. The manual values were adopted on PROCESS grounds - that run had the '
 "wider source set and a documented two-source-agreement rule - not because the job's figures were shown to be in "
 "error. None of the seven is a held position, so the broker's mark, which is the standing tie-breaker for cent-level "
 'conflicts, was not available. ⚠️ EIGHT 52-WEEK BOUNDS CORRECTED - A METHOD ERROR IN THE SCHEDULED JOB, WORTH FIXING '
 'IN THE JOB ITSELF. Where a close breached the stored 52-week range, the job reset the bound to the CLOSING price: '
 'hi52 ZETA 30.97, TEAM 194.17, HNGE 92.56, CRWD 231.00, FTNT 170.93, OKTA 173.04; lo52 EOSE 3.21, NRG 110.12. That is '
 "internally impossible in a different way - A 52-WEEK HIGH CAN NEVER SIT BELOW THAT SAME SESSION'S INTRADAY HIGH. "
 'Corrected to the sourced Aug 31 session extremes: ZETA 31.05, TEAM 197.30, HNGE 92.69, CRWD 233.88, FTNT 172.51, '
 "OKTA 174.08, EOSE 3.18, NRG 108.54. These remain 'AT LEAST' values rather than verified 52-week extremes - each is "
